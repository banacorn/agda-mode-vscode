...
let getInstalledTargetsAndPersistThem = async (findCommands, globalStorageUri: VSCode.Uri.t) => {
  // from `agdaMode.connection.paths` in the settings
  let dict = {
    let pairs =
      await Config.Connection.getAgdaPaths()
      ->Array.map(async uri => {
        let target = await Connection__Target.fromURI(uri)
  ...
  let addAgdaOrALS = async (folderURI, fileName) => {
    ...
    switch await Connection__Target.fromRawPath(VSCode.Uri.fsPath(executablePath)) {
    | Ok(target) =>
