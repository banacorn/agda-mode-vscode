...
let decorateHole = (editor: TextEditor.t, (start, end_): (int, int), index: int) => {
  let document = TextEditor.document(editor)
  let backgroundRange = VSRange.make(
...
let decorateHighlightings = (editor: TextEditor.t, highlightings: array<Highlighting.t>): (
  array<(Editor.Decoration.t, array<VSRange.t>)>,
  array<srcLoc>,
) => {
  ...
  let aspects: array<(Highlighting.Aspect.t, VSRange.t)> = highlightings->Array.map(((
    range,
    aspects,
    _,
  )) =>
  ...
  let srcLocs: array<srcLoc> =
    highlightings->Array.keepMap(((range, _, source)) =>
      source->Option.map(((filepath, offset)) => {range: range, filepath: filepath, offset: offset})
    )
...
> =>
  Js.Array.find(
    ...
    Workspace.openTextDocumentWithFileName(srcLoc.filepath)->Promise.map(document => {
