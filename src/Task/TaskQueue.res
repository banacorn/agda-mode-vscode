...
let getNextTask = self =>
  ...
  | (Pending(list{x, ...view}), _, _) =>
    self.view = Pending(view)
    Some(x)
  | (Closing(resolve, list{x, ...view}), _, _) =>
    self.view = Closing(resolve, view)
    Some(x)
  // stuck on View
  | (Pending(list{}), _, _) => None
  // trigger the ending promise
  | (Closing(resolve, list{}), _, _) =>
    self.view = Free
    resolve()
    None
  // still have tasks in the Agda queue
  | (Free, Pending(list{x, ...agda}), _) =>
    self.agda = Pending(agda)
    Some(x)
  | (Free, Closing(resolve, list{x, ...agda}), _) =>
    self.agda = Closing(resolve, agda)
    Some(x)
  // stuck on Agda
  | (Free, Pending(list{}), _) => None
  // trigger the ending promise
  | (Free, Closing(resolve, list{}), _) =>
    self.agda = Free
    resolve()
    None
  // still have tasks in the Main queue
  | (Free, Free, list{x, ...main}) =>
    self.main = main
    Some(x)
  // stuck on Main
  | (Free, Free, list{}) => None
  ...
  if self.busy->Option.isNone {
    ...
    | Some(task) =>
      ...
      | AgdaRequest(_) =>
        ...
        self.execute(self, task)->Promise.get(keepRunning => // and start executing tasks
          if keepRunning {
            // and keep running
            kickStart(self)
          }
...
let addToTheFront = (self, tasks) => {
  ...
  | (Closing(resolver, view), _) => self.view = Closing(resolver, List.concat(tasks, view))
  | (Free, Pending(agda)) => self.agda = Pending(List.concat(tasks, agda))
  | (Free, Closing(resolver, agda)) => self.agda = Closing(resolver, List.concat(tasks, agda))
...
let toString = (taskToString, {main, agda, view}) => {
  ...
  let agda = switch agda {
  | Free => ""
  | Pending(agda) => "Agda " ++ Util.Pretty.list(List.map(agda, taskToString))
  | Closing(_, agda) => "Agda# " ++ Util.Pretty.list(List.map(agda, taskToString))
  }
  let view = switch view {
  | Free => ""
  | Pending(view) => "View " ++ Util.Pretty.list(List.map(view, taskToString))
  | Closing(_, view) => "View# " ++ Util.Pretty.list(List.map(view, taskToString))
  }
  main ++ ("\n" ++ (agda ++ ("\n" ++ view)))
}
...
module Agda = {
  ...
  let addToTheBack = (self, tasks) =>
    ...
    | Free =>
      ...
      kickStart(self)
    | Closing(_, _) =>
      Js.log("[ panic ] Cannot add task to the Agda task queue after it's been marked closed")
    | Pending(agda) =>
...
module View = {
  ...
  let addToTheBack = (self, tasks) =>
    ...
    | Free =>
      ...
      kickStart(self)
    | Closing(_, _) =>
      Js.log("[ panic ] Cannot add task to the View task queue after it's been marked closed")
    | Pending(view) =>
