...
//   let displayConnectionError = error => WithStateP(
//     _ => {
//       let (header, body) = Connection.Error.toString(error)
//       display(state, Error("Connection Error: " ++ header), Plain(body))->Promise.map(() => list{})
//     },
//   )
...
//         WithStateP(
//           _ =>
//             display(state, Error("Internal Parse Error"), Plain(body))->Promise.map(() => list{}),
//         ),
//       }
//       addToAgdaQueue(tasks)
...
//   | DispatchCommand(command) => Handle__Command.handle(state, command)->Promise.map(tasks => {
