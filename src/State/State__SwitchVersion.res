...
let switchAgdaVersion = async (state: State.t) => {
  ...
  switch await Connection.make(state.memento, Config.Connection.getAgdaPaths(), ["als", "agda"], platform, State__Request.askUserAboutDownloadPolicy, State__Request.downloadLatestALS(state)) {
  | Ok(conn) =>
