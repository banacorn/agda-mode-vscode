...
| F8 | Low | process destroy state machine never stabilizes at `Destroyed` | `test/tests/Connection/Test__Connection__Process.res` | [x] | [x] |
| F9 | Medium | same-owner fast-path allows concurrent (not only nested) execution | `test/tests/Test__Registry__Connection.res` | [x] | [x] |
