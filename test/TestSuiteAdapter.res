...
let run = () => {
  ...
  let testsRoot =
    dirname->Option.mapWithDefault("./", dirname => Node.Path.resolve(dirname, "tests"))

  Js.Promise.make((~resolve, ~reject) => Glob.glob("**/*.js", {cwd: testsRoot}, (err, files) =>
      switch Js.Nullable.toOption(err) {
      | Some(err) => reject(. err)
      | None =>
        // Add files to the test suite
        files->Array.forEach(file => mocha->Mocha.addFile(Node.Path.resolve(testsRoot, file)))

        // Run the mocha test
        switch mocha->Mocha.run(failures =>
          if failures > 0 {
