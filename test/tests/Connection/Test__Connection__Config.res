...
describe("Config.Connection paths", () => {
  ...
  describe("UI-triggered additions", () => {
    ...
    describe("Switch version UI selection", () => {
      Async.it("should add path to config when user selects existing path not in config", async () => {
        let initialConfig = [userAgda.contents]
        let selectedPath = alternativeAgda.contents // not in initial config
        
        await Config.Connection.setAgdaPaths(Chan.make(), initialConfig)
        let listener = Log.collect(logChannel)
      ...
      Async.it("should not modify config when user selects path already in config", async () => {
        ...
        // Since Config.Connection.addAgdaPath has built-in deduplication,
        // we can test this directly without full UI simulation
        await Config.Connection.addAgdaPath(logChannel, selectedPath)
      ...
      Async.it("should add new path to config when user selects different path", async () => {
        let initialConfig = [userAgda.contents]
        ...
        // Test the core config addition logic that SwitchVersion UI uses
        await Config.Connection.addAgdaPath(logChannel, selectedPath)
    ...
    describe("Download action selection", () => {
      Async.it("should add downloaded path to config when user downloads new ALS", async () => {
        let initialConfig = [userAgda.contents]
        await Config.Connection.setAgdaPaths(Chan.make(), initialConfig)
        let listener = Log.collect(logChannel)
        
        // Simulate successful download by using Connection.make with FromDownloads source
        let result = await makeConnection([], platformWithDiscovery) // empty config forces download fallback
      ...
      Async.it("should add downloaded path to config when user uses already downloaded ALS", async () => {
        let initialConfig = [userAgda.contents] 
        await Config.Connection.setAgdaPaths(Chan.make(), initialConfig)
    ...
    describe("Non-endpoint selections", () => {
      Async.it("should not modify config when user selects open folder action", async () => {
        let initialConfig = [userAgda.contents]
        await Config.Connection.setAgdaPaths(Chan.make(), initialConfig)
        let listener = Log.collect(logChannel)
      ...
      Async.it("should handle separator items correctly", async () => {
        let initialConfig = [userAgda.contents]
        await Config.Connection.setAgdaPaths(Chan.make(), initialConfig)
        let listener = Log.collect(logChannel)
