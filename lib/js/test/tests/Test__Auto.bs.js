// Generated by ReScript, PLEASE EDIT WITH CARE
'use strict';

var Curry = require("rescript/lib/js/curry.js");
var Assert = require("assert");
var OS$AgdaModeVscode = require("../../src/Util/OS.bs.js");
var Util$AgdaModeVscode = require("../../src/Util/Util.bs.js");
var Goals$AgdaModeVscode = require("../../src/Goals.bs.js");
var Test__Util$AgdaModeVscode = require("./Test__Util.bs.js");
var State__Connection$AgdaModeVscode = require("../../src/State/State__Connection.bs.js");

function run(normalization) {
  it("should be responded with correct responses 1", (async function () {
          var ctx = await Test__Util$AgdaModeVscode.AgdaMode.makeAndLoad("Auto.agda");
          var goal = Goals$AgdaModeVscode.getGoalByIndex(ctx.state.goals, 0);
          var responses = goal !== undefined ? await State__Connection$AgdaModeVscode.sendRequestAndCollectResponses(ctx.state, {
                  TAG: "Auto",
                  _0: normalization,
                  _1: goal
                }) : [];
          var filteredResponses = responses.filter(Test__Util$AgdaModeVscode.filteredResponse);
          var version = ctx.state.agdaVersion;
          if (version !== undefined) {
            if (Util$AgdaModeVscode.Version.gte(version, "2.7.0")) {
              return Curry._3(Assert.deepStrictEqual, filteredResponses, [
                          {
                            TAG: "GiveAction",
                            _0: 0,
                            _1: {
                              TAG: "GiveString",
                              _0: "n"
                            }
                          },
                          {
                            TAG: "InteractionPoints",
                            _0: [1]
                          }
                        ], undefined);
            } else {
              return Curry._3(Assert.deepStrictEqual, filteredResponses, [
                          {
                            TAG: "GiveAction",
                            _0: 0,
                            _1: {
                              TAG: "GiveString",
                              _0: "n"
                            }
                          },
                          {
                            TAG: "DisplayInfo",
                            _0: {
                              TAG: "AllGoalsWarnings",
                              _0: "*All Goals*",
                              _1: "?1 : ℕ\n"
                            }
                          },
                          {
                            TAG: "InteractionPoints",
                            _0: [1]
                          }
                        ], undefined);
            }
          } else {
            Assert.fail("No Agda version found");
            return ;
          }
        }));
  it("should be responded with correct responses 2", (async function () {
          var ctx = await Test__Util$AgdaModeVscode.AgdaMode.makeAndLoad("Auto.agda");
          var goal = Goals$AgdaModeVscode.getGoalByIndex(ctx.state.goals, 1);
          var responses = goal !== undefined ? await State__Connection$AgdaModeVscode.sendRequestAndCollectResponses(ctx.state, {
                  TAG: "Auto",
                  _0: normalization,
                  _1: goal
                }) : [];
          var filteredResponses = responses.filter(Test__Util$AgdaModeVscode.filteredResponse);
          var version = ctx.state.agdaVersion;
          if (version !== undefined) {
            if (Util$AgdaModeVscode.Version.gte(version, "2.7.0")) {
              return Curry._3(Assert.deepStrictEqual, filteredResponses, [
                          {
                            TAG: "GiveAction",
                            _0: 1,
                            _1: {
                              TAG: "GiveString",
                              _0: "n"
                            }
                          },
                          {
                            TAG: "InteractionPoints",
                            _0: [0]
                          }
                        ], undefined);
            } else {
              return Curry._3(Assert.deepStrictEqual, filteredResponses, [
                          {
                            TAG: "GiveAction",
                            _0: 1,
                            _1: {
                              TAG: "GiveString",
                              _0: "m"
                            }
                          },
                          {
                            TAG: "DisplayInfo",
                            _0: {
                              TAG: "AllGoalsWarnings",
                              _0: "*All Goals*",
                              _1: "?0 : ℕ\n"
                            }
                          },
                          {
                            TAG: "InteractionPoints",
                            _0: [0]
                          }
                        ], undefined);
            }
          } else {
            Assert.fail("No Agda version found");
            return ;
          }
        }));
}

describe("agda-mode.auto", (function () {
        this.timeout(OS$AgdaModeVscode.onUnix ? 4000 : 20000);
        describe("AsIs", (function () {
                run("AsIs");
              }));
        describe("Simplified", (function () {
                run("Simplified");
              }));
        describe("Normalised", (function () {
                run("Normalised");
              }));
        describe("HeadNormal", (function () {
                run("HeadNormal");
              }));
      }));

exports.run = run;
/*  Not a pure module */
