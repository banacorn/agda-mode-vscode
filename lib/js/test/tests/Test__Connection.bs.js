...
function probeKeyForLocalPath(path) {
  var match = Connection__URI$AgdaModeVscode.parse(path);
  return match._1.fsPath;
}

function findProbeByLocalPath(errors, path) {
  var expectedKey = normalizeLocalPathForAssert(probeKeyForLocalPath(path));
  return Core__Array.findMap(Object.entries(errors.probes), (function (param) {
                if (normalizeLocalPathForAssert(param[0]) === expectedKey) {
                  return param[1];
                }
...
describe("Connection", (function () {
        this.timeout(OS$AgdaModeVscode.onUnix ? 10000 : 30000);
        ...
        describe("`make`", (function () {
                ...
                it("should return Establish error for non-existent path", (async function () {
                        ...
                        var match = findProbeByLocalPath(result._0, nonExistentPath);
                        if (match !== undefined) {
                ...
                it("should return Establish error for unrecognized executable", (async function () {
                        ...
                        if (result.TAG === "Ok") {
                          ...
                          var match = result._0.probes[probeKeyForLocalPath(mockPath)];
                          if (match !== undefined) {
                            var error = match[0];
                ...
                it("should handle error construction correctly for probe errors", (async function () {
                        ...
                        var match = findProbeByLocalPath(errors, invalidPath);
                        if (match !== undefined) {
                          if (match[0].TAG === "CannotDetermineAgdaOrALS") {
                            return ;
                          }
        ...
        describe("`fromPathsOrCommands`", (function () {
                describe("paths", (function () {
                        ...
                        it("should preserve FromConfig as probe source for resource candidates", (async function () {
                                ...
                                var probeKey = probeKeyForLocalPath(path);
                                var match = result._0.probes[probeKey];
                                if (match !== undefined) {
                ...
                describe("commands", (function () {
                        ...
                        it("should rewrite command candidate source to FromCommandLookup when resolved command fails to connect", (async function () {
                                ...
                                var match = Connection__URI$AgdaModeVscode.parse("/definitely/not/a/real/agda");
                                var resolvedProbeKey = match._1.fsPath;
                                var match$1 = result._0.probes[resolvedProbeKey];
                                if (match$1 !== undefined) {
                                  return Curry._3(Assert.deepStrictEqual, match$1[1], {
        ...
        describe("make with logging", (function () {
                ...
                it("should log connection events when using real agda command", (async function () {
                        ...
                        var platformDeps = Desktop$AgdaModeVscode.make();
                        var globalStorageUri = Vscode.Uri.file("/tmp/test-storage");
                        var result = await Connection$AgdaModeVscode.makeWithFallback(platformDeps, memento, globalStorageUri, [], logChannel);
                        var loggedEvents = listener(Log$AgdaModeVscode.isConnection);
                        if (result.TAG !== "Ok") {
                ...
                it("should not log connection events when connection fails", (async function () {
                        ...
                        var globalStorageUri = Vscode.Uri.file("/tmp/test-storage");
                        var match = await Connection$AgdaModeVscode.makeWithFallback(platformDeps, memento, globalStorageUri, ["/nonexistent/path"], logChannel);
                        if (match.TAG === "Ok") {
                ...
                it.skip("should log connection events even when falling back to downloads", (async function () {
                        ...
                        var platformDeps = Desktop$AgdaModeVscode.make();
                        var globalStorageUri = Vscode.Uri.file("/tmp/test-storage");
                        var match = await Connection$AgdaModeVscode.makeWithFallback(platformDeps, memento, globalStorageUri, ["/invalid/path"], logChannel);
                        if (match.TAG !== "Ok") {
                          return Curry._3(Assert.deepStrictEqual, loggedEvents, [], undefined);
                        }
        ...
        describe("make fromPaths scenarios", (function () {
                it("should prioritize valid paths over download fallback", (async function () {
                        ...
                        var platformDeps = Desktop$AgdaModeVscode.make();
                        var globalStorageUri = Vscode.Uri.file("/tmp/test-storage");
                        var result = await Connection$AgdaModeVscode.makeWithFallback(platformDeps, memento, globalStorageUri, [agdaMockPath], logChannel);
                        var establishFlowEvents = collectEstablishFlow(listener);
                        var loggedEvents = listener(Log$AgdaModeVscode.isConnection);
                        if (result.TAG === "Ok") {
                          var connection = result._0;
        ...
        describe("Connection contract coverage", (function () {
                ...
                it("should update connection.paths but not PreferredCandidate after automatic fallback download", (async function () {
                        ...
                        var result = await Connection$AgdaModeVscode.makeWithFallback(platform, memento, Vscode.Uri.file("/tmp/test-storage"), ["/invalid/path"], logChannel);
                        if (result.TAG === "Ok") {
                          var establishFlowEvents = collectEstablishFlow(listener);
                          Curry._3(Assert.deepStrictEqual, establishFlowEvents, [
                                "ConfigCandidatesPlanned:1",
                ...
                it("should prepend downloaded path to connection.paths on automatic fallback", (async function () {
                        ...
                        var memento = Memento$AgdaModeVscode.make(undefined);
                        var platform = Mock$AgdaModeVscode.Platform.makeWithSuccessfulDownload(downloadedAgda.contents);
                        var result = await Connection$AgdaModeVscode.makeWithFallback(platform, memento, Vscode.Uri.file("/tmp/test-storage"), existingPaths, logChannel);
                        if (result.TAG === "Ok") {
                          Curry._3(Assert.deepStrictEqual, Connection$AgdaModeVscode.getPath(result._0), downloadedAgda.contents, undefined);
                ...
                it("should return managed-download error when cached native has no same-release WASM fallback", (async function () {
                        ...
                        switch (errors.TAG) {
                          case "Establish" :
                              var normalizedNativeKey = Vscode.Uri.file(cachedNativePath).fsPath;
                              var match = errors._0.probes[normalizedNativeKey];
                              if (match !== undefined) {
                ...
                it("should not resolve selected channel when cached managed native has no same-release WASM fallback", (async function () {
                        ...
                        switch (errors.TAG) {
                          case "Establish" :
                              ...
                              var normalizedNativeKey = Vscode.Uri.file(cachedNativePath).fsPath;
                              var match = errors$1.probes[normalizedNativeKey];
                              if (match !== undefined) {
                        ...
                        return await withTempStorage("agda-managed-local-wasm-fallback-", (async function (storagePath, globalStorageUri) {
                                      ...
                                      switch (errors.TAG) {
                                        case "Establish" :
                                            ...
                                            var normalizedWasmKey = Vscode.Uri.file(cachedWasmPath).fsPath;
                                            var match$1 = errors$1.probes[normalizedWasmKey];
                                            if (match$1 !== undefined) {
                ...
                it("should canonicalize Windows file URI shape via VSCode.Uri.file when parsing command/resource paths", (function () {
                        if (OS$AgdaModeVscode.onUnix) {
                          return ;
                        }
                        var windowsPath = "D:\\resolved\\bin\\agda.exe";
                        var result = Connection__URI$AgdaModeVscode.parse(windowsPath);
                        Curry._3(Assert.deepStrictEqual, result._1.toString(), Vscode.Uri.file(windowsPath).toString(), undefined);
                      }));
