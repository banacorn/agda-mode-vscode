...
describe("Connection", (function () {
        ...
        describe("`fromDownloads`", (function () {
                ...
                it("should fall back to DevALS (WASM) when LatestALS download fails on desktop", (async function () {
                        var agdaMockPath = await Test__Util$AgdaModeVscode.Endpoint.Agda.mock("2.8.0", "agda-mock-wasm-fallback");
                        await Config$AgdaModeVscode.Connection.DownloadPolicy.set("Undecided");
                        var checkedCache = {
                          contents: false
                        };
                        ...
                        var mockPlatformDeps = Mock$AgdaModeVscode.Platform.makeWithLatestALSFailureAndDevALSSuccess(agdaMockPath, checkedCache, checkedDownload);
                        ...
                        if (result.TAG === "Ok") {
                          ...
                          switch (connection.TAG) {
                            case "Agda" :
                                ...
                                Curry._3(Assert.deepStrictEqual, connection._2, "2.8.0", undefined);
                                break;
                            case "ALS" :
                            case "ALSWASM" :
                                Assert.fail("Expected Agda connection from DevALS fallback");
                                break;
                            
                          }
                        } else {
                          Assert.fail("Expected DevALS fallback to succeed, but fromDownloads only tries LatestALS on desktop (WASM fallback not implemented yet)");
                        }
                        try {
