...
describe("Tokens", (function () {
        this.timeout(10000);
        Test__Util$AgdaModeVscode.Agda.make(undefined, "GotoDefinition.agda");
        describe("GotoDefinition.agda", (function () {
                it("should produce 28 tokens", (async function () {
                        var context = await Test__Util$AgdaModeVscode.Agda.make(undefined, "GotoDefinition.agda");
                        var match = await Test__Util$AgdaModeVscode.Agda.load(context);
                        var tokens = Belt_Array.map(Tokens$AgdaModeVscode.toArray(match[1].tokens), (function (param) {
                                return Editor$AgdaModeVscode.$$Range.toString(param[1]) + " " + Tokens$AgdaModeVscode.Token.toString(param[0]);
                              }));
                        return Curry._3(Assert.deepEqual, 28, tokens.length, undefined);
                      }));
