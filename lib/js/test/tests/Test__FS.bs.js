...
describe("FS", (function () {
        ...
        describe("deleteRecursive", (function () {
                it("should delete non-empty directory successfully", (async function () {
                        var tempDir = Nodepath.join(Nodeos.tmpdir(), "fs-delete-recursive-" + String(Date.now() | 0));
                        var file1 = Nodepath.join(tempDir, "file1.txt");
                        var file2 = Nodepath.join(tempDir, "file2.txt");
                        await Nodefs.promises.mkdir(tempDir, {
                              recursive: true,
                              mode: 511
                            });
                        Nodefs.writeFileSync(file1, Buffer.from("content1"));
                        Nodefs.writeFileSync(file2, Buffer.from("content2"));
                        Assert.ok(Nodefs.existsSync(tempDir));
                        Assert.ok(Nodefs.existsSync(file1));
                        Assert.ok(Nodefs.existsSync(file2));
                        var uri = Vscode.Uri.file(tempDir);
                        var result = await FS$AgdaModeVscode.deleteRecursive(uri);
                        if (result.TAG === "Ok") {
                          Assert.ok(!Nodefs.existsSync(tempDir));
                          Assert.ok(!Nodefs.existsSync(file1));
                          Assert.ok(!Nodefs.existsSync(file2));
                ...
                it("should delete nested directory structure successfully", (async function () {
                        var tempDir = Nodepath.join(Nodeos.tmpdir(), "fs-delete-nested-" + String(Date.now() | 0));
                        var subDir = Nodepath.join(tempDir, "subdir");
                        var file1 = Nodepath.join(tempDir, "file1.txt");
                        var file2 = Nodepath.join(subDir, "file2.txt");
                        await Nodefs.promises.mkdir(subDir, {
                              recursive: true,
                              mode: 511
                            });
                        Nodefs.writeFileSync(file1, Buffer.from("content1"));
                        Nodefs.writeFileSync(file2, Buffer.from("content2"));
                        Assert.ok(Nodefs.existsSync(tempDir));
                        Assert.ok(Nodefs.existsSync(subDir));
                        Assert.ok(Nodefs.existsSync(file1));
                        Assert.ok(Nodefs.existsSync(file2));
                        var uri = Vscode.Uri.file(tempDir);
                        var result = await FS$AgdaModeVscode.deleteRecursive(uri);
                ...
                it("should delete empty directory successfully", (async function () {
                        var tempDir = Nodepath.join(Nodeos.tmpdir(), "fs-delete-recursive-empty-" + String(Date.now() | 0));
                        await Nodefs.promises.mkdir(tempDir, {
                              recursive: true,
                              mode: 511
                            });
                ...
                it("should succeed when trying to delete non-existent directory (idempotent)", (async function () {
                        var nonExistentDir = Nodepath.join(Nodeos.tmpdir(), "non-existent-recursive-" + String(Date.now() | 0));
                        Assert.ok(!Nodefs.existsSync(nonExistentDir));
                        var uri = Vscode.Uri.file(nonExistentDir);
                        var result = await FS$AgdaModeVscode.deleteRecursive(uri);
                        if (result.TAG === "Ok") {
                          Assert.ok(!Nodefs.existsSync(nonExistentDir));
                          return ;
