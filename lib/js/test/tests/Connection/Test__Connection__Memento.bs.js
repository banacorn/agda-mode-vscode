...
describe("Memento.PickedConnection", (function () {
        ...
        describe("Memento update behavior", (function () {
                ...
                it("should set memento to bare command name when auto discovery succeeds", (async function () {
                        var memento = Memento$AgdaModeVscode.make(undefined);
                        await Config$AgdaModeVscode.Connection.setAgdaPaths(logChannel, []);
                        var result = await Connection$AgdaModeVscode.makeWithFallback(platform, memento, Vscode.Uri.file("/tmp/test"), [], ["agda"], logChannel);
                        if (result.TAG === "Ok") {
                          var actualPath = Connection$AgdaModeVscode.getPath(result._0);
                          var mementoPath = Memento$AgdaModeVscode.Module.PickedConnection.get(memento);
                          Curry._3(Assert.deepStrictEqual, actualPath, systemAgda.contents, undefined);
                          return Curry._3(Assert.deepStrictEqual, mementoPath, "agda", undefined);
                        }
                        Assert.fail("Connection should succeed");
                      }));
                it("should not overwrite existing picked connection when auto discovery succeeds", (async function () {
                        var previousPick = "/nonexistent/picked-agda-path";
                        var memento = Memento$AgdaModeVscode.make(undefined);
                        await Memento$AgdaModeVscode.Module.PickedConnection.set(memento, previousPick);
                        await Config$AgdaModeVscode.Connection.setAgdaPaths(logChannel, []);
                        var result = await Connection$AgdaModeVscode.makeWithFallback(platform, memento, Vscode.Uri.file("/tmp/test"), [], ["agda"], logChannel);
                        if (result.TAG === "Ok") {
                          var actualPath = Connection$AgdaModeVscode.getPath(result._0);
                          var mementoPath = Memento$AgdaModeVscode.Module.PickedConnection.get(memento);
                          Curry._3(Assert.deepStrictEqual, actualPath, systemAgda.contents, undefined);
                          return Curry._3(Assert.deepStrictEqual, mementoPath, previousPick, undefined);
                        }
                        Assert.fail("Connection should succeed");
                      }));
              }));
