...
var Connection__Target__Agda__Error$AgdaModeVscode = require("../../src/Connection/Target/Agda/Connection__Target__Agda__Error.bs.js");
...
function activateExtension() {
  ...
  var disposables = [];
  var channels$1 = Main$AgdaModeVscode.activateWithoutContext(disposables, extensionPath, globalStorageUri);
...
function toString$1(x) {
  if (x.TAG === "ALSConnectionError") {
    return x._0.map(Connection__Resolver$AgdaModeVscode.$$Error.toString).join(",");
  }
  var match = Connection__Target__Agda__Error$AgdaModeVscode.toString(x._0);
  return "AgdaConnectionError: " + match[0] + ": " + match[1];
}
...
async function exists(command) {
  var match = await Connection__Resolver$AgdaModeVscode.searchUntilSuccess([{
          TAG: "FromCommand",
          _0: command,
  ...
  var msg = match[1].map(Connection__Resolver$AgdaModeVscode.$$Error.toString).join(",");
  throw {
        RE_EXN_ID: "Failure",
