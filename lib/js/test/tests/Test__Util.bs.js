...
async function load(self) {
  ...
  throw {
        RE_EXN_ID: "Failure",
        _1: "Cannot load " + self.filepath,
        Error: new Error()
      };
}

async function $$case(self, cursorAndPayload, state) {
  ...
  if (cursorAndPayload !== undefined) {
    ...
    var succeed = await Editor$AgdaModeVscode.$$Text.insert(state.document, cursor, cursorAndPayload[1]);
    if (!succeed) {
      throw {
            RE_EXN_ID: "Failure",
  ...
  var match = Util$AgdaModeVscode.Promise_.pending();
  var resolve = match[1];
  var destructor = Chan$AgdaModeVscode.on(state.channels.commandHandled, (function (command) {
