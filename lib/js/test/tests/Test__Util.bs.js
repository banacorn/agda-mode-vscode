// Generated by ReScript, PLEASE EDIT WITH CARE
'use strict';

var Caml = require("rescript/lib/js/caml.js");
var Diff = require("diff");
var Js_exn = require("rescript/lib/js/js_exn.js");
var Assert = require("assert");
var Vscode = require("vscode");
var Nodefs = require("node:fs");
var Nodeos = require("node:os");
var Caml_obj = require("rescript/lib/js/caml_obj.js");
var Nodepath = require("node:path");
var Caml_option = require("rescript/lib/js/caml_option.js");
var Core__Option = require("@rescript/core/lib/js/src/Core__Option.bs.js");
var Caml_exceptions = require("rescript/lib/js/caml_exceptions.js");
var FS$AgdaModeVscode = require("../../src/FS.bs.js");
var OS$AgdaModeVscode = require("../../src/Util/OS.bs.js");
var Caml_js_exceptions = require("rescript/lib/js/caml_js_exceptions.js");
var Log$AgdaModeVscode = require("../../src/State/Log.bs.js");
var Chan$AgdaModeVscode = require("../../src/Util/Chan.bs.js");
var Util$AgdaModeVscode = require("../../src/Util/Util.bs.js");
var Editor$AgdaModeVscode = require("../../src/Editor.bs.js");
var Parser$AgdaModeVscode = require("../../src/Parser/Parser.bs.js");
var Command$AgdaModeVscode = require("../../src/Command.bs.js");
var Desktop$AgdaModeVscode = require("../../src/Main/Desktop.bs.js");
var Memento$AgdaModeVscode = require("../../src/Memento.bs.js");
var Connection$AgdaModeVscode = require("../../src/Connection/Connection.bs.js");
var Connection__URI$AgdaModeVscode = require("../../src/Connection/Shared/Connection__URI.bs.js");
var Connection__Error$AgdaModeVscode = require("../../src/Connection/Shared/Connection__Error.bs.js");

var Exn = /* @__PURE__ */Caml_exceptions.create("Test__Util-AgdaModeVscode.Exn");

var $$TextEncoder = {};

var $$TextDecoder = {};

function toString(uint8Array) {
  return new TextDecoder().decode(uint8Array);
}

var TypedArray = {
  toString: toString
};

function open_(fileName) {
  return Vscode.window.showTextDocument(Vscode.Uri.file(fileName), undefined);
}

async function read(fileName) {
  var editor = await Vscode.window.showTextDocument(Vscode.Uri.file(fileName), undefined);
  var $$document = editor.document;
  return $$document.getText(undefined);
}

async function write(fileName, content) {
  var editor = await Vscode.window.showTextDocument(Vscode.Uri.file(fileName), undefined);
  var $$document = editor.document;
  var lineCount = $$document.lineCount;
  var replaceRange = new Vscode.Range(new Vscode.Position(0, 0), new Vscode.Position(lineCount, 0));
  var succeed = await Editor$AgdaModeVscode.$$Text.replace($$document, replaceRange, content);
  if (succeed) {
    await $$document.save();
    return ;
  }
  throw {
        RE_EXN_ID: "Failure",
        _1: "Failed to write to " + fileName,
        Error: new Error()
      };
}

var $$File = {
  open_: open_,
  read: read,
  write: write
};

function getExpectedFilename(baseFilename) {
  if (OS$AgdaModeVscode.onUnix) {
    return baseFilename + ".out";
  } else {
    return baseFilename + ".win.out";
  }
}

var ExpectedFiles = {
  getExpectedFilename: getExpectedFilename
};

var runner = (function(f) {



    var tmp



    try {



      var result = f();



      tmp = {



        TAG: 0,



        _0: result,



        [Symbol.for("name")]: "Ok"



      };



    }



    catch (raw_exn){



      tmp = 



        {



          TAG: 1,



          _0: raw_exn,



          [Symbol.for("name")]: "Error"



        };



    }



    return $$Promise.resolved(tmp);



  });

function toAbsolute(filepath) {
  return Nodepath.resolve(__dirname, filepath);
}

var extensionUri = Vscode.Uri.file(Nodepath.resolve(__dirname, "../../../../"));

var globalStorageUri = Vscode.Uri.file(Nodepath.resolve(__dirname, "../../../../test/globalStoragePath"));

function asset(filepath) {
  return Parser$AgdaModeVscode.Filepath.toString(Parser$AgdaModeVscode.Filepath.make(Vscode.Uri.joinPath(extensionUri, "test/tests/assets", filepath).fsPath));
}

var Path = {
  toAbsolute: toAbsolute,
  extensionUri: extensionUri,
  globalStorageUri: globalStorageUri,
  asset: asset
};

var activationSingleton = {
  contents: undefined
};

var activatedLogUri = {
  contents: undefined
};

async function activateExtension(candidate) {
  var channels = activationSingleton.contents;
  if (channels !== undefined) {
    return channels;
  }
  var ext = Vscode.extensions.getExtension("banacorn.agda-mode");
  var extension;
  if (ext !== undefined) {
    extension = Caml_option.valFromOption(ext);
  } else {
    throw {
          RE_EXN_ID: "Failure",
          _1: "Extension banacorn.agda-mode not found",
          Error: new Error()
        };
  }
  var $$exports = await extension.activate();
  await Memento$AgdaModeVscode.Module.PreferredCandidate.set($$exports.memento, candidate);
  activationSingleton.contents = $$exports.channels;
  activatedLogUri.contents = Caml_option.some($$exports.logUri);
  return $$exports.channels;
}

async function activateExtensionAndOpenFile(fileName, candidate) {
  var channels = await activateExtension(candidate);
  var editor = await Vscode.window.showTextDocument(Vscode.Uri.file(fileName), undefined);
  return [
          editor,
          channels
        ];
}

async function openNoLoad(path) {
  var match = await activateExtensionAndOpenFile(asset(path), undefined);
  await Vscode.commands.executeCommand("agda-mode.input-symbol[Activate]");
  await Vscode.commands.executeCommand("agda-mode.escape");
  return [
          match[0],
          match[1]
        ];
}

async function staleAndRefreshEditor(path, awayPath) {
  var fileName = asset(awayPath);
  await Vscode.window.showTextDocument(Vscode.Uri.file(fileName), undefined);
  var fileName$1 = asset(path);
  return await Vscode.window.showTextDocument(Vscode.Uri.file(fileName$1), undefined);
}

function wait(ms) {
  return new Promise((function (resolve, param) {
                setTimeout(resolve, ms);
              }));
}

async function createStorageUri(prefixOpt) {
  var prefix = prefixOpt !== undefined ? prefixOpt : "agda-test-storage";
  var storagePath = Nodepath.join(Nodeos.tmpdir(), prefix + "-" + String(Date.now() | 0));
  await Nodefs.promises.mkdir(storagePath, {
        recursive: true,
        mode: 511
      });
  return Vscode.Uri.file(storagePath);
}

async function withStorage(prefix, f) {
  var globalStorageUri = await createStorageUri(prefix);
  var result;
  try {
    await f(globalStorageUri);
    result = undefined;
  }
  catch (raw_exn){
    result = Caml_js_exceptions.internalToOCamlException(raw_exn);
  }
  await FS$AgdaModeVscode.deleteRecursive(globalStorageUri);
  if (result === undefined) {
    return ;
  }
  throw result;
}

function locate() {
  var logUri = activatedLogUri.contents;
  if (logUri !== undefined) {
    var logDir = Caml_option.valFromOption(logUri).fsPath;
    var candidate = Nodepath.join(Nodepath.dirname(logDir), "exthost.log");
    if (Nodefs.existsSync(candidate)) {
      return candidate;
    } else {
      Assert.fail("Test__Util.ExtHostLog: could not find exthost.log -- observed logUri.fsPath: " + (logDir + (", expected candidate: " + candidate)));
      return "";
    }
  }
  Assert.fail("Test__Util.ExtHostLog: extension has not been activated yet -- call activateExtension (or activateExtensionAndOpenFile) first");
  return "";
}

function countLinesContaining(phrase) {
  var path = locate();
  var content = Nodefs.readFileSync(path).toString();
  return content.split("\n").filter(function (line) {
              return line.includes(phrase);
            }).length;
}

var ExtHostLog = {
  locate: locate,
  countLinesContaining: countLinesContaining
};

function normalize(string) {
  return string.trim().replace(/\r\n|\n/g, "\n");
}

function unlinesWith(f, xs) {
  return Util$AgdaModeVscode.$$String.unlines(xs.map(f));
}

function breakInput(input, breakpoints) {
  var breakpoints$p = [0].concat(breakpoints);
  return breakpoints$p.map(function (x, i) {
                var next = breakpoints$p[i + 1 | 0];
                if (next !== undefined) {
                  return [
                          x,
                          next
                        ];
                } else {
                  return [
                          x,
                          input.length
                        ];
                }
              }).map(function (param) {
              return input.substring(param[0], param[1]);
            });
}

var Strings = {
  normalize: normalize,
  unlinesWith: unlinesWith,
  breakInput: breakInput
};

function getValue(x) {
  return x._0;
}

function fromChangeObject(obj) {
  if (obj.added) {
    return {
            TAG: "Added",
            _0: obj.value
          };
  } else if (obj.removed) {
    return {
            TAG: "Removed",
            _0: obj.value
          };
  } else {
    return {
            TAG: "NoChange",
            _0: obj.value
          };
  }
}

function wordsWithSpace(a, b) {
  return Diff.diffWordsWithSpace(a, b).map(fromChangeObject);
}

function firstChange(diffs) {
  var count = {
    contents: 0
  };
  var change = {
    contents: undefined
  };
  diffs.forEach(function (diff) {
        if (!Core__Option.isNone(change.contents)) {
          return ;
        }
        switch (diff.TAG) {
          case "Added" :
              change.contents = {
                TAG: "Added",
                _0: diff._0
              };
              return ;
          case "Removed" :
              change.contents = {
                TAG: "Removed",
                _0: diff._0
              };
              return ;
          case "NoChange" :
              count.contents = count.contents + diff._0.length | 0;
              return ;
          
        }
      });
  return Core__Option.map(change.contents, (function (change) {
                return [
                        change,
                        count.contents
                      ];
              }));
}

var Diff$1 = {
  getValue: getValue,
  fromChangeObject: fromChangeObject,
  wordsWithSpace: wordsWithSpace,
  firstChange: firstChange
};

function getGoldenFilepathsSync(directoryPath) {
  return [];
}

var FileMissing = /* @__PURE__ */Caml_exceptions.create("Test__Util-AgdaModeVscode.Golden.FileMissing");

function toString$1(param) {
  return "Golden file at: " + param._0 + "\nExpected: \n" + param._2 + "\nActual: \n" + param._1;
}

function map(param, f) {
  return {
          TAG: "Golden",
          _0: param._0,
          _1: f(param._1),
          _2: param._2
        };
}

async function readFile(filepath) {
  var filepath$1 = Nodepath.resolve(__dirname, filepath);
  var error = await FS$AgdaModeVscode.readFile(Vscode.Uri.file(filepath$1 + ".in"));
  var inFile;
  if (error.TAG === "Ok") {
    inFile = new TextDecoder().decode(error._0);
  } else {
    throw {
          RE_EXN_ID: "Failure",
          _1: "Cannot read file " + filepath$1 + ".in: " + error._0,
          Error: new Error()
        };
  }
  var error$1 = await FS$AgdaModeVscode.readFile(Vscode.Uri.file(filepath$1 + ".out"));
  var outFile;
  if (error$1.TAG === "Ok") {
    outFile = new TextDecoder().decode(error$1._0);
  } else {
    throw {
          RE_EXN_ID: "Failure",
          _1: "Cannot read file " + filepath$1 + ".out: " + error$1._0,
          Error: new Error()
        };
  }
  return {
          TAG: "Golden",
          _0: filepath$1,
          _1: inFile,
          _2: outFile
        };
}

function compare(param) {
  var actual = normalize(param._1);
  var expected = normalize(param._2);
  Core__Option.forEach(firstChange(wordsWithSpace(actual, expected)), (function (param) {
          var count = param[1];
          var diff = param[0];
          var value = diff._0;
          var change = value.length > 100 ? value.substring(0, 100) + " ..." : value;
          var expected$1 = expected.substring(Caml.int_max(0, count - 50 | 0), ((Caml.int_max(0, count - 50 | 0) + 50 | 0) + value.length | 0) + 50 | 0);
          var actual$1 = actual.substring(Caml.int_max(0, count - 50 | 0), ((Caml.int_max(0, count - 50 | 0) + 50 | 0) + value.length | 0) + 50 | 0);
          var message = function (change) {
            return "\n\nchange => " + change + "\n\nexpected => " + expected$1 + "\n\nactual   => " + actual$1;
          };
          switch (diff.TAG) {
            case "Added" :
            case "Removed" :
                Assert.fail(message(change));
                return ;
            case "NoChange" :
                return ;
            
          }
        }));
}

var Golden = {
  Diff: Diff$1,
  getGoldenFilepathsSync: getGoldenFilepathsSync,
  FileMissing: FileMissing,
  toString: toString$1,
  map: map,
  readFile: readFile,
  compare: compare
};

async function versionGTE(command, expectedVersion) {
  var platformDeps = Desktop$AgdaModeVscode.make();
  var _error = await Connection$AgdaModeVscode.fromPathsOrCommands(platformDeps, [[
          command,
          "FromConfig"
        ]], undefined, undefined);
  if (_error.TAG !== "Ok") {
    return false;
  }
  var match = _error._0;
  switch (match.TAG) {
    case "Agda" :
        return Util$AgdaModeVscode.Version.gte(match._2, expectedVersion);
    case "ALS" :
        return Util$AgdaModeVscode.Version.gte(match._2.agdaVersion, expectedVersion);
    case "ALSWASM" :
        return Util$AgdaModeVscode.Version.gte(match._3.agdaVersion, expectedVersion);
    
  }
}

async function commandExists(command) {
  var platformDeps = Desktop$AgdaModeVscode.make();
  var errors = await Connection$AgdaModeVscode.fromPathsOrCommands(platformDeps, [[
          command,
          "FromConfig"
        ]], undefined, undefined);
  if (errors.TAG === "Ok") {
    return Connection$AgdaModeVscode.getPath(errors._0);
  }
  throw {
        RE_EXN_ID: "Failure",
        _1: Connection__Error$AgdaModeVscode.Establish.toString(errors._0),
        Error: new Error()
      };
}

async function makeAndLoad(filepath) {
  var rawFilepath = Vscode.Uri.joinPath(extensionUri, "test/tests/assets", filepath).fsPath;
  var candidate = await commandExists("agda");
  var load = async function (channels, filepath) {
    var match = Util$AgdaModeVscode.Promise_.pending();
    var resolve = match[1];
    var disposable = Chan$AgdaModeVscode.on(channels.commandHandled, (function (command) {
            if (command === "Load") {
              return resolve();
            }
            
          }));
    await Vscode.window.showTextDocument(Vscode.Uri.file(filepath), undefined);
    var match$1 = await Vscode.commands.executeCommand("agda-mode.load");
    if (match$1 !== undefined) {
      if (match$1.TAG === "Ok") {
        await match[0];
        disposable();
        return match$1._0;
      }
      var match$2 = Connection__Error$AgdaModeVscode.toString(match$1._0);
      throw {
            RE_EXN_ID: "Failure",
            _1: match$2[0] + "\n" + match$2[1],
            Error: new Error()
          };
    }
    throw {
          RE_EXN_ID: "Failure",
          _1: "Cannot load " + filepath,
          Error: new Error()
        };
  };
  var channels = await activateExtension(candidate);
  var state = await load(channels, rawFilepath);
  return {
          filepath: rawFilepath,
          channels: channels,
          state: state
        };
}

async function quit(self) {
  await Vscode.window.showTextDocument(Vscode.Uri.file(self.filepath), undefined);
  var destroyCompleted = Log$AgdaModeVscode.on(self.channels.log, (function (log) {
          if (log.TAG === "Others" && log._0 === "State.destroy: Connection released, destruction complete") {
            return true;
          } else {
            return false;
          }
        }));
  var match = await Vscode.commands.executeCommand("agda-mode.quit");
  if (match !== undefined && match.TAG !== "Ok") {
    var match$1 = Connection__Error$AgdaModeVscode.toString(match._0);
    throw {
          RE_EXN_ID: "Failure",
          _1: match$1[0] + "\n" + match$1[1],
          Error: new Error()
        };
  }
  return await destroyCompleted;
}

async function $$case(self, cursor, payload) {
  var editor = await Vscode.window.showTextDocument(Vscode.Uri.file(self.filepath), undefined);
  var succeed = await Editor$AgdaModeVscode.$$Text.insert(self.state.document, cursor, payload);
  if (!succeed) {
    throw {
          RE_EXN_ID: "Failure",
          _1: "Failed to insert text",
          Error: new Error()
        };
  }
  Editor$AgdaModeVscode.Cursor.set(editor, cursor);
  var match = Util$AgdaModeVscode.Promise_.pending();
  var resolve = match[1];
  var destructor = Chan$AgdaModeVscode.on(self.state.channels.commandHandled, (function (command) {
          if (typeof command !== "object" && command === "Load") {
            return resolve();
          }
          
        }));
  var match$1 = await Vscode.commands.executeCommand("agda-mode.case");
  if (match$1 !== undefined) {
    if (match$1.TAG === "Ok") {
      await match[0];
      destructor();
      self.state = match$1._0;
      return ;
    }
    var match$2 = Connection__Error$AgdaModeVscode.toString(match$1._0);
    throw {
          RE_EXN_ID: "Failure",
          _1: match$2[0] + "\n" + match$2[1],
          Error: new Error()
        };
  }
  throw {
        RE_EXN_ID: "Failure",
        _1: "Cannot case split " + self.filepath,
        Error: new Error()
      };
}

async function execute(self, command) {
  var match = Util$AgdaModeVscode.Promise_.pending();
  var resolve = match[1];
  var destructor = Chan$AgdaModeVscode.on(self.state.channels.commandHandled, (function (handledCommand) {
          if (Caml_obj.equal(handledCommand, command)) {
            return resolve();
          }
          
        }));
  if (!Command$AgdaModeVscode.names.some(function (param) {
          return Caml_obj.equal(param[0], command);
        })) {
    throw {
          RE_EXN_ID: "Failure",
          _1: "Command " + Command$AgdaModeVscode.toString(command) + " is not registered",
          Error: new Error()
        };
  }
  var match$1 = await Vscode.commands.executeCommand("agda-mode." + Command$AgdaModeVscode.toKeybinding(command));
  if (match$1 !== undefined) {
    if (match$1.TAG === "Ok") {
      await match[0];
      destructor();
      self.state = match$1._0;
      return ;
    }
    var match$2 = Connection__Error$AgdaModeVscode.toString(match$1._0);
    throw {
          RE_EXN_ID: "Failure",
          _1: match$2[0] + "\n" + match$2[1],
          Error: new Error()
        };
  }
  throw {
        RE_EXN_ID: "Failure",
        _1: "Cannot execute command " + Command$AgdaModeVscode.toString(command) + " in " + self.filepath,
        Error: new Error()
      };
}

async function execute$1(self, command, cursor, payload) {
  var editor = await Vscode.window.showTextDocument(Vscode.Uri.file(self.filepath), undefined);
  if (cursor !== undefined) {
    var cursor$1 = Caml_option.valFromOption(cursor);
    if (payload !== undefined) {
      await Editor$AgdaModeVscode.$$Text.insert(self.state.document, cursor$1, payload);
    }
    Editor$AgdaModeVscode.Cursor.set(editor, cursor$1);
  }
  return await execute(self, command);
}

function nextGoal(__x) {
  return execute$1(__x, "NextGoal", undefined, undefined);
}

function previousGoal(__x) {
  return execute$1(__x, "PreviousGoal", undefined, undefined);
}

var AgdaMode = {
  versionGTE: versionGTE,
  commandExists: commandExists,
  makeAndLoad: makeAndLoad,
  quit: quit,
  $$case: $$case,
  execute: execute$1,
  nextGoal: nextGoal,
  previousGoal: previousGoal
};

function filteredResponse(response) {
  if (typeof response !== "object") {
    switch (response) {
      case "ClearRunningInfo" :
      case "ClearHighlighting" :
      case "CompleteHighlightingAndMakePromptReappear" :
          return false;
      default:
        return true;
    }
  } else {
    switch (response.TAG) {
      case "HighlightingInfoDirect" :
      case "HighlightingInfoIndirect" :
      case "Status" :
      case "RunningInfo" :
          return false;
      default:
        return true;
    }
  }
}

async function mock(version, name) {
  var match = OS$AgdaModeVscode.onUnix ? [
      name,
      "#!/bin/sh\necho 'Agda version " + version + "'\nexit 0"
    ] : [
      name + ".bat",
      "@echo Agda version " + version
    ];
  var content = match[1];
  var match$1 = Connection__URI$AgdaModeVscode.parse(Nodepath.join(Nodeos.tmpdir(), match[0]));
  var tempFile = match$1._1.fsPath;
  try {
    Nodefs.writeFileSync(tempFile, Buffer.from(content));
    if (OS$AgdaModeVscode.onUnix) {
      try {
        await Nodefs.promises.chmod(tempFile, 493);
      }
      catch (raw_obj){
        var obj = Caml_js_exceptions.internalToOCamlException(raw_obj);
        if (obj.RE_EXN_ID === Js_exn.$$Error) {
          var errorMsg = Core__Option.getOr(obj._1.message, "unknown chmod error");
          throw {
                RE_EXN_ID: "Failure",
                _1: "Cannot chmod mock executable at " + tempFile + ": " + errorMsg,
                Error: new Error()
              };
        }
        throw obj;
      }
    }
    return tempFile;
  }
  catch (raw_obj$1){
    var obj$1 = Caml_js_exceptions.internalToOCamlException(raw_obj$1);
    if (obj$1.RE_EXN_ID === Js_exn.$$Error) {
      var errorMsg$1 = Core__Option.getOr(obj$1._1.message, "unknown error");
      var detailedError = "Got error when trying to construct a mock for Agda:\n  - Target file: " + tempFile + "\n  - Platform: " + (
        OS$AgdaModeVscode.onUnix ? "Unix" : "Windows"
      ) + "\n  - Temp directory: " + Nodeos.tmpdir() + "\n  - Error: " + errorMsg$1 + "\n  - Content length: " + String(content.length) + "\n  - Content: " + content;
      throw {
            RE_EXN_ID: "Failure",
            _1: detailedError,
            Error: new Error()
          };
    }
    var detailedError$1 = "Got error when trying to construct a mock for Agda:\n  - Target file: " + tempFile + "\n  - Platform: " + (
      OS$AgdaModeVscode.onUnix ? "Unix" : "Windows"
    ) + "\n  - Temp directory: " + Nodeos.tmpdir() + "\n  - Error: unknown error type\n  - Content: " + content;
    throw {
          RE_EXN_ID: "Failure",
          _1: detailedError$1,
          Error: new Error()
        };
  }
}

async function destroy(target) {
  try {
    Nodefs.unlinkSync(target);
    return ;
  }
  catch (raw_obj){
    var obj = Caml_js_exceptions.internalToOCamlException(raw_obj);
    if (obj.RE_EXN_ID === Js_exn.$$Error) {
      console.warn("Warning: Cannot delete mock Agda executable: " + Core__Option.getOr(obj._1.message, "unknown error"));
      return ;
    } else {
      return ;
    }
  }
}

var Agda = {
  mock: mock,
  destroy: destroy
};

async function mock$1(alsVersion, agdaVersion, name) {
  var output = "Agda v" + agdaVersion + " Language Server v" + alsVersion;
  var match = OS$AgdaModeVscode.onUnix ? [
      name,
      "#!/bin/sh\necho '" + output + "'\nexit 0"
    ] : [
      name + ".bat",
      "@echo " + output
    ];
  var content = match[1];
  var match$1 = Connection__URI$AgdaModeVscode.parse(Nodepath.join(Nodeos.tmpdir(), match[0]));
  var tempFile = match$1._1.fsPath;
  try {
    Nodefs.writeFileSync(tempFile, Buffer.from(content));
    if (OS$AgdaModeVscode.onUnix) {
      try {
        await Nodefs.promises.chmod(tempFile, 493);
      }
      catch (raw_obj){
        var obj = Caml_js_exceptions.internalToOCamlException(raw_obj);
        if (obj.RE_EXN_ID === Js_exn.$$Error) {
          var errorMsg = Core__Option.getOr(obj._1.message, "unknown chmod error");
          throw {
                RE_EXN_ID: "Failure",
                _1: "Cannot chmod mock ALS executable at " + tempFile + ": " + errorMsg,
                Error: new Error()
              };
        }
        throw obj;
      }
    }
    return tempFile;
  }
  catch (raw_obj$1){
    var obj$1 = Caml_js_exceptions.internalToOCamlException(raw_obj$1);
    if (obj$1.RE_EXN_ID === Js_exn.$$Error) {
      var errorMsg$1 = Core__Option.getOr(obj$1._1.message, "unknown error");
      var detailedError = "Got error when trying to construct a mock for ALS:\n  - Target file: " + tempFile + "\n  - Platform: " + (
        OS$AgdaModeVscode.onUnix ? "Unix" : "Windows"
      ) + "\n  - Temp directory: " + Nodeos.tmpdir() + "\n  - Error: " + errorMsg$1 + "\n  - Content length: " + String(content.length) + " characters";
      throw {
            RE_EXN_ID: "Failure",
            _1: detailedError,
            Error: new Error()
          };
    }
    throw {
          RE_EXN_ID: "Failure",
          _1: "Got error when trying to construct a mock for ALS: unknown error",
          Error: new Error()
        };
  }
}

async function destroy$1(target) {
  try {
    Nodefs.unlinkSync(target);
    return ;
  }
  catch (raw_obj){
    var obj = Caml_js_exceptions.internalToOCamlException(raw_obj);
    if (obj.RE_EXN_ID === Js_exn.$$Error) {
      console.warn("Warning: Cannot delete mock ALS executable: " + Core__Option.getOr(obj._1.message, "unknown error"));
      return ;
    } else {
      return ;
    }
  }
}

var ALS = {
  mock: mock$1,
  destroy: destroy$1
};

var Candidate = {
  Agda: Agda,
  ALS: ALS
};

var withDeleteFailureFor = (function(failedFsPath) {
    const fsModule = require("../../src/FS.bs.js");
    const originalDeleteRecursive = fsModule.deleteRecursive;
    fsModule.deleteRecursive = function(uri) {
      if (uri && uri.fsPath === failedFsPath) {
        return Promise.resolve({ TAG: 1, _0: "mock delete failure" });
      }
      return originalDeleteRecursive(uri);
    };
    return function() {
      fsModule.deleteRecursive = originalDeleteRecursive;
    };
  });

var TestFS = {
  withDeleteFailureFor: withDeleteFailureFor
};

exports.Exn = Exn;
exports.$$TextEncoder = $$TextEncoder;
exports.$$TextDecoder = $$TextDecoder;
exports.TypedArray = TypedArray;
exports.$$File = $$File;
exports.ExpectedFiles = ExpectedFiles;
exports.runner = runner;
exports.Path = Path;
exports.activationSingleton = activationSingleton;
exports.activatedLogUri = activatedLogUri;
exports.activateExtension = activateExtension;
exports.activateExtensionAndOpenFile = activateExtensionAndOpenFile;
exports.openNoLoad = openNoLoad;
exports.staleAndRefreshEditor = staleAndRefreshEditor;
exports.wait = wait;
exports.createStorageUri = createStorageUri;
exports.withStorage = withStorage;
exports.ExtHostLog = ExtHostLog;
exports.Strings = Strings;
exports.Golden = Golden;
exports.AgdaMode = AgdaMode;
exports.filteredResponse = filteredResponse;
exports.Candidate = Candidate;
exports.TestFS = TestFS;
/* extensionUri Not a pure module */
