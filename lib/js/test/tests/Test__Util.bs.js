...
async function write(fileName, content) {
  ...
  var succeed = await Editor$AgdaModeVscode.$$Text.replace(editor, replaceRange, content, undefined);
  if (succeed) {
    await $$document.save();
    return ;
  }
...
async function $$case(self, cursor, payload) {
  ...
  var succeed = await Editor$AgdaModeVscode.$$Text.insert(self.state.editor, cursor, payload, undefined);
  if (!succeed) {
    throw {
          RE_EXN_ID: "Failure",
...
async function execute$1(self, command, cursor, payload) {
  ...
  if (cursor !== undefined) {
    ...
    if (payload !== undefined) {
      await Editor$AgdaModeVscode.$$Text.insert(self.state.editor, cursor$1, payload, undefined);
    }
