...
async function activateExtension(endpoint) {
  ...
  await Memento$AgdaModeVscode.Module.PickedConnection.set(memento, endpoint);
  var channels$1 = Main$AgdaModeVscode.activateWithoutContext(platformDeps, disposables, extensionUri, globalStorageUri, memento);
  activationSingleton.contents = channels$1;
  return channels$1;
}

async function activateExtensionAndOpenFile(fileName, endpoint) {
  var channels = await activateExtension(endpoint);
  var editor = await Vscode.window.showTextDocument(Vscode.Uri.file(fileName), undefined);
  return [
          editor,
          channels
...
async function versionGTE(command, expectedVersion) {
  var platformDeps = Desktop$AgdaModeVscode.make();
  var _error = await Connection$AgdaModeVscode.fromCommands(platformDeps, [command]);
  if (_error.TAG !== "Ok") {
    return false;
  }
  var match = _error._0;
  switch (match.TAG) {
    ...
    case "ALS" :
        ...
        if (match$1 !== undefined) {
          return Util$AgdaModeVscode.Version.gte(match$1[1], expectedVersion);
        } else {
          return false;
        }
    case "ALSWASM" :
        var match$2 = match._3;
        if (match$2 !== undefined) {
          return Util$AgdaModeVscode.Version.gte(match$2[1], expectedVersion);
        } else {
          return false;
        }
...
async function commandExists(command) {
  var platformDeps = Desktop$AgdaModeVscode.make();
  var errors = await Connection$AgdaModeVscode.fromCommands(platformDeps, [command]);
  if (errors.TAG === "Ok") {
...
async function makeAndLoad(filepath) {
  ...
  var endpoint = await commandExists("agda");
...
async function quit(self) {
  return await Registry$AgdaModeVscode.removeAndDestroy(self.state.document);
}
...
async function mock(version, name) {
  ...
  var tempFile;
  if (match$1.TAG === "FileURI") {
    tempFile = match$1._1.fsPath;
  } else {
    throw {
          RE_EXN_ID: "Failure",
          _1: "Cannot create mock Agda executable with LspURI",
          Error: new Error()
        };
  }
  try {
