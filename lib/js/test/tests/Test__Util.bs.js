...
function unlinesWith(f, xs) {
  return Util$AgdaModeVscode.$$String.unlines(Belt_Array.map(xs, f));
}
...
function refine(cursorAndPayload, param) {
  var state = param[1];
  var self = param[0];
  return $$Promise.flatMap($$Promise.flatMap($$Promise.flatMap(Vscode.window.showTextDocument(Vscode.Uri.file(self.filepath), undefined), (function (editor) {
                        if (cursorAndPayload === undefined) {
                        ...
                        if (payload !== undefined) {
                          return $$Promise.tap(Editor$AgdaModeVscode.$$Text.insert(state.document, cursor, payload), (function (param) {
                                        Editor$AgdaModeVscode.Cursor.set(editor, cursor);
                                      }));
                        } else {
                          Editor$AgdaModeVscode.Cursor.set(editor, cursor);
                    ...
                    return Vscode.commands.executeCommand("agda-mode.refine");
                  })), (function (result) {
                if (result !== undefined) {
                  if (result.TAG === /* Ok */0) {
                    return $$Promise.resolved({
                ...
                var value$1 = "Cannot case refine " + self.filepath;
                return runner(function (param) {
                            Assert$BsMocha.fail(value$1);
                          });
              }));
