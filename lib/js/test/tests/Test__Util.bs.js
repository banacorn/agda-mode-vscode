...
async function nextGoal(self) {
  var match = await Vscode.commands.executeCommand("agda-mode.next-goal");
  if (match !== undefined) {
    if (match.TAG === "Ok") {
      self.state = match._0;
      return ;
  ...
  throw {
        RE_EXN_ID: "Failure",
        _1: "Cannot jump to the next goal in " + self.filepath,
        Error: new Error()
      };
}

async function previousGoal(self) {
  var match = await Vscode.commands.executeCommand("agda-mode.previous-goal");
  if (match !== undefined) {
    if (match.TAG === "Ok") {
      self.state = match._0;
      return ;
  ...
  throw {
        RE_EXN_ID: "Failure",
        _1: "Cannot jump to the previous goal in " + self.filepath,
        Error: new Error()
      };
}
