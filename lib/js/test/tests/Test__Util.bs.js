...
function activateExtension() {
  ...
  var disposables = [];
  var channels$1 = Main$AgdaModeVscode.activateWithoutContext(platformDeps, disposables, extensionUri, globalStorageUri, undefined);
...
async function versionGTE(command, expectedVersion) {
  ...
  if (match.TAG === "Agda") {
    ...
    return Util$AgdaModeVscode.Version.gte(match._2[1], expectedVersion);
  }
}
...
async function makeAndLoad(filepath) {
  ...
  Chan$AgdaModeVscode.emit(state.channels.log, {
        TAG: "AgdaModeOperation",
        _0: "makeAndLoad",
        _1: rawFilepath
      });
  if (!OS$AgdaModeVscode.onUnix) {
    var normalizedPath = Parser$AgdaModeVscode.filepath(rawFilepath);
    Registry$AgdaModeVscode.add(normalizedPath, state);
    Registry$AgdaModeVscode.add(rawFilepath, state);
  }
  Chan$AgdaModeVscode.emit(state.channels.log, {
        TAG: "AgdaModeOperation",
        _0: "makeAndLoad",
  ...
  if (!OS$AgdaModeVscode.onUnix) {
    var normalizedPath$1 = Parser$AgdaModeVscode.filepath(rawFilepath);
    Registry$AgdaModeVscode.add(normalizedPath$1, state);
    Registry$AgdaModeVscode.add(rawFilepath, state);
  }
  return {
...
async function quit(self) {
  Chan$AgdaModeVscode.emit(self.state.channels.log, {
        TAG: "AgdaModeOperation",
        _0: "quit",
        _1: self.filepath
      });
  await Registry$AgdaModeVscode.removeAndDestroy(self.filepath);
  return Chan$AgdaModeVscode.emit(self.state.channels.log, {
              TAG: "AgdaModeOperation",
              _0: "quit completed",
              _1: self.filepath
            });
