...
async function execute(self, command) {
  var match = await Vscode.commands.executeCommand("agda-mode." + command);
  if (match !== undefined) {
    if (match.TAG === "Ok") {
      self.state = match._0;
      return ;
  ...
  throw {
        RE_EXN_ID: "Failure",
        _1: "Cannot execute command " + command + " in " + self.filepath,
        Error: new Error()
      };
}
...
async function refine(self, cursor, payload) {
  ...
  return await execute(self, "refine");
}

async function give(self, cursor, payload) {
  var editor = await Vscode.window.showTextDocument(Vscode.Uri.file(self.filepath), undefined);
  if (cursor !== undefined) {
    var cursor$1 = Caml_option.valFromOption(cursor);
    if (payload !== undefined) {
  ...
  return await execute(self, "give");
}

async function helperFunctionType(self, normalization, cursor, payload) {
  var editor = await Vscode.window.showTextDocument(Vscode.Uri.file(self.filepath), undefined);
  if (cursor !== undefined) {
    var cursor$1 = Caml_option.valFromOption(cursor);
    if (payload !== undefined) {
  ...
  return await execute(self, "helper-function-type[" + Command$AgdaModeVscode.Normalization.encode(normalization) + "]");
}

async function elaborateAndGive(self, normalization, cursor, payload) {
  var editor = await Vscode.window.showTextDocument(Vscode.Uri.file(self.filepath), undefined);
  if (cursor !== undefined) {
    var cursor$1 = Caml_option.valFromOption(cursor);
    if (payload !== undefined) {
  ...
  return await execute(self, "elaborate-and-give[" + Command$AgdaModeVscode.Normalization.encode(normalization) + "]");
}
...
async function solveConstraints(self, normalization, cursor) {
  ...
  return await execute(self, "solve-constraints[" + Command$AgdaModeVscode.Normalization.encode(normalization) + "]");
}

function nextGoal(__x) {
  return execute(__x, "next-goal");
}

function previousGoal(__x) {
  return execute(__x, "previous-goal");
}
...
var AgdaMode = {
  ...
  refine: refine,
  give: give,
  helperFunctionType: helperFunctionType,
  elaborateAndGive: elaborateAndGive,
