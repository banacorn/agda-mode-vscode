...
async function versionGTE(command, expectedVersion) {
  var platformDeps = Desktop$AgdaModeVscode.make();
  var _error = await Connection$AgdaModeVscode.findCommands(platformDeps, [command]);
  if (_error.TAG === "Ok") {
...
async function commandExists(command) {
  var platformDeps = Desktop$AgdaModeVscode.make();
  var errors = await Connection$AgdaModeVscode.findCommands(platformDeps, [command]);
  if (errors.TAG === "Ok") {
    return ;
  }
  throw {
        RE_EXN_ID: "Failure",
        _1: Object.entries(errors._0).map(function (param) {
                return param[0] + ": " + Connection__Command$AgdaModeVscode.$$Error.toString(param[1]);
              }).join("\n"),
        Error: new Error()
      };
}
