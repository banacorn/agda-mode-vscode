...
async function mock(version, name) {
  ...
  var path = match[0];
  var uint8Array = new TextEncoder().encode(match[1]);
  var error = await FS$AgdaModeVscode.writeFile(Vscode.Uri.file(path), uint8Array);
  if (error.TAG !== "Ok") {
    throw {
          RE_EXN_ID: "Failure",
          _1: "Cannot write mock executable: " + error._0,
          Error: new Error()
        };
  }
  return path;
...
async function destroy(target) {
  var error = await FS$AgdaModeVscode.$$delete(Vscode.Uri.file(Connection__URI$AgdaModeVscode.toString(Connection__Target$AgdaModeVscode.toURI(target))));
  if (error.TAG === "Ok") {
    return ;
  }
  throw {
        RE_EXN_ID: "Failure",
        _1: "Cannot delete mock Agda executable: " + error._0,
        Error: new Error()
      };
}
