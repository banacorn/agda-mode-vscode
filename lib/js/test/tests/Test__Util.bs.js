...
async function mock(version, name) {
  ...
  var tempFile = Nodepath.join(Nodeos.tmpdir(), match[0] + "-" + String(Date.now() | 0));
  Nodefs.writeFileSync(tempFile, Buffer.from(match[1]));
  if (OS$AgdaModeVscode.onUnix) {
    try {
      ...
      if (obj.RE_EXN_ID === Js_exn.$$Error) {
        throw {
              RE_EXN_ID: "Failure",
              _1: "Cannot chmod mock executable: " + Core__Option.getOr(obj._1.message, "unknown error"),
              Error: new Error()
            };
      }
      throw obj;
...
async function destroy(target) {
  ...
  try {
    ...
    if (obj.RE_EXN_ID === Js_exn.$$Error) {
      throw {
            RE_EXN_ID: "Failure",
            _1: "Cannot delete mock Agda executable: " + Core__Option.getOr(obj._1.message, "unknown error"),
            Error: new Error()
          };
    }
    throw obj;
