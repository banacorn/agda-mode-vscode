...
describe("Registry__Connection", (function () {
        ...
        it("same-owner concurrent execute calls should be serialized", (async function () {
                await setup();
                var dummyConnection = makeDummyConnection();
                var make = async function () {
                  return {
                ...
                var firstTaskCanFinish = match[0];
                var first = Registry__Connection$AgdaModeVscode.execute("owner1", (async function (param) {
                        executionOrder.push("task1-start");
                ...
                var second = Registry__Connection$AgdaModeVscode.execute("owner1", (async function (param) {
                        secondStarted.contents = true;
                        executionOrder.push("task2-start");
                        return {
                                TAG: "Ok",
                                _0: undefined
                ...
                await Util$AgdaModeVscode.Promise_.$$setTimeout(0);
                Curry._3(Assert.deepStrictEqual, secondStarted.contents, false, undefined);
                match[1]();
                await first;
                await second;
                return Curry._3(Assert.deepStrictEqual, executionOrder, [
        ...
        it("queued execute should not run after registry leaves active state", (async function () {
                await setup();
                var dummyConnection = makeDummyConnection();
                await Registry__Connection$AgdaModeVscode.acquire("owner1", (async function () {
                        return {
                ...
                var second = Registry__Connection$AgdaModeVscode.execute("owner2", (async function (param) {
                        secondTaskRan.contents = true;
                        return {
                                TAG: "Ok",
                                _0: "second-ran"
                              };
                ...
                Curry._3(Assert.deepStrictEqual, secondTaskRan.contents, false, undefined);
                if (secondResult.TAG === "Ok") {
                  Assert.fail("Queued task ran even though registry was no longer active");
                  return ;
                }
                Assert.ok(true);
              }));
        ...
        describe("Real-world Scenario", (function () {
                ...
                it("should share a single connection mechanism between two different Agda files", (async function () {
                        ...
                        var listener = Log$AgdaModeVscode.collect(agdaA.channels.log);
                        var agdaB = await Test__Util$AgdaModeVscode.AgdaMode.makeAndLoad("Lib.agda");
                        var activationEvents = Core__Array.filterMap(listener(undefined), (function (log) {
                                if (log.TAG !== "Connection") {
                                  return ;
                                }
                                var $$event = log._0;
                                if ($$event.TAG === "ActivationFlow") {
                                  return $$event._0;
                                }
                        ...
                        return await Test__Util$AgdaModeVscode.AgdaMode.quit(agdaB);
                      }));
              }));
      }));
