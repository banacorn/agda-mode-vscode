...
function run(normalization) {
  ...
  it("should be responded with the correct responses", (async function () {
          var ctx = await Test__Util$AgdaModeVscode.AgdaMode.makeAndLoad(undefined, "Issue204.agda");
          ...
          console.log(filteredResponses.map(Response$AgdaModeVscode.toString).join("\n"));
  ...
  it("should remove all goals", (async function () {
          var ctx = await Test__Util$AgdaModeVscode.AgdaMode.makeAndLoad(undefined, "Issue204.agda");
          await Test__Util$AgdaModeVscode.AgdaMode.solveConstraints(ctx, normalization, undefined);
          return Curry._3(Assert.deepEqual, ctx.state.goals.length, 0, undefined);
        }));
}
