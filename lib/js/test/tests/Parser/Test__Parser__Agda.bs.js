...
describe("when running Agda.OutputConstraint.parse", (function () {
        ...
        it("should parse OfType with a wrapped type", (function () {
                var expected = {
                  TAG: "OfType",
                  _0: RichText$AgdaModeVscode.string("f"),
                  _1: RichText$AgdaModeVscode.string("Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat")
                };
                var actual = Agda$AgdaModeVscode.OutputConstraint.parse("f : Nat →\n    Nat →\n    Nat →\n    Nat →\n    Nat →\n    Nat →\n    Nat →\n    Nat →\n    Nat →\n    Nat →\n    Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat → Nat");
                Curry._3(Assert.deepStrictEqual, actual, expected, undefined);
              }));
        ...
        it("should parse JustType with a wrapped type", (function () {
                var expected = {
                  TAG: "JustType",
                  _0: RichText$AgdaModeVscode.string("A -> B -> C")
                };
                var actual = Agda$AgdaModeVscode.OutputConstraint.parse("Type A ->\n  B -> C");
                Curry._3(Assert.deepStrictEqual, actual, expected, undefined);
              }));
        it("should parse JustSort on Windows", (function () {
                var expected = {
                  TAG: "JustSort",
                  _0: RichText$AgdaModeVscode.string("ℕ ℕ")
                };
