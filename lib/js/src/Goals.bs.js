...
async function scanAllGoals(self, editor, changes) {
  ...
  var stateEntries = Array.from(map.entries());
  stateEntries.forEach(function (param) {
        var state = param[1];
        var index = param[0];
        if (state.TAG === "IsQuestionMark") {
          var goal = self.goals.get(index);
          if (goal !== undefined && goal.start !== state._0) {
            AVLTree$AgdaModeVscode.remove(self.positions, goal.start);
            return ;
          } else {
            return ;
        ...
        if (goal$1 !== undefined) {
          AVLTree$AgdaModeVscode.remove(self.positions, goal$1.start);
          return ;
        }
        
      });
  var rewrites = Core__Array.filterMap(stateEntries, (function (param) {
          var state = param[1];
          var index = param[0];
          if (state.TAG === "IsQuestionMark") {
            var offset = state._0;
