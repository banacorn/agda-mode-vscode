...
function redecorate(self) {
  Core__Option.forEach(Vscode.window.activeTextEditor, (function (editor) {
          Array.from(self.goals.values()).forEach(function (goal) {
                Core__Option.forEach(goal.decoration, (function (param) {
                        var $$document = editor.document;
                        var backgroundRange = new Vscode.Range($$document.positionAt(goal.start), $$document.positionAt(goal.end));
                        var indexRange = new Vscode.Range($$document.positionAt(goal.end - 2 | 0), $$document.positionAt(goal.end - 2 | 0));
                        Editor$AgdaModeVscode.Decoration.decorate(editor, param[0], [backgroundRange]);
                        Editor$AgdaModeVscode.Decoration.decorate(editor, param[1], [indexRange]);
                      }));
              });
        }));
}
...
async function resetGoalIndices(self, editor, indices) {
  console.log("=== resetGoalIndices called ===");
  console.log("Indices from Agda: [" + indices.map(function (i) {
              return i.toString();
            }).join(", ") + "]");
  console.log("Current goals before clear: " + toString$1(self));
  console.log("Current goalsWithoutIndices before clear: " + Array.from(self.goalsWithoutIndices.entries()).map(function (param) {
              return "(" + param[0].toString() + "," + param[1].toString() + ")";
            }).join(", "));
  clear(self);
  console.log("After clear: " + toString$1(self));
  var positionsArray = Array.from(self.goalsWithoutIndices.entries());
  console.log("Positions array: " + positionsArray.map(function (param) {
              return "(" + param[0].toString() + "," + param[1].toString() + ")";
            }).join(", "));
  positionsArray.forEach(function (param, i) {
        var end = param[1];
        var start = param[0];
        console.log("Processing position " + i.toString() + ": (" + start.toString() + "," + end.toString() + ")");
        var index = indices[i];
        if (index !== undefined) {
          return insertGoal(self, start, end, index);
        }
...
async function resetGoalIndicesNew(self, editor, indices) {
  var existingPositions = new Map(Array.from(self.goals.values()).map(function (goal) {
            return [
                    goal.start,
                    [
                      goal.end,
  ...
  var allPositions = Array.from(existingPositions.entries()).toSorted(function (param, param$1) {
        return Caml.int_compare(param[0], param$1[0]);
      });
  clear(self);
  allPositions.forEach(function (param, i) {
        var index = indices[i];
        if (index !== undefined) {
...
var Module = {
  ...
  resetGoalIndicesNew: resetGoalIndicesNew,
...
exports.resetGoalIndicesNew = resetGoalIndicesNew;
