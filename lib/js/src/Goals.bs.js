...
function read(goal, editor) {
  var innerRange = makeInnerRange(goal, editor);
  var $$document = editor.document;
...
async function modify(self, editor, index, f) {
  var goal = self.goals.get(index);
  if (goal === undefined) {
    return ;
  }
  var innerRange = makeInnerRange(goal, editor);
  var goalContent = read(goal, editor);
  await Editor$AgdaModeVscode.$$Text.replace(editor, innerRange, " " + f(goalContent) + " ", undefined);
}
...
async function removeBoundaryAndDestroy(self, $$document, index) {
  ...
  if (await Editor$AgdaModeVscode.$$Text.replace($$document, outerRange, content, undefined)) {
    removeGoal(self, goal);
    return true;
  } else {
    return false;
...
function setCursorByIndex(self, editor, index) {
  ...
  var range = makeOuterRange(goal, editor);
...
async function scanAllGoals(self, editor, changes) {
  ...
  setBusy(self);
  await Editor$AgdaModeVscode.$$Text.batchReplace(editor, rewrites, undefined);
...
function parseGoalPositionsFromRefine(raw) {
  ...
  var chunks = raw$1.split(goalQuestionMark);
