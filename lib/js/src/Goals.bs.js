...
function reset(self) {
  ...
  self.recentlyCaseSplited = undefined;
  setNotBusy(self);
}
...
function isGoalIntact(goal, $$document) {
  var documentEnd = $$document.getText(undefined).length;
  if (goal.start < 0 || goal.end > documentEnd || goal.start >= goal.end) {
    return false;
  }
  var text = Editor$AgdaModeVscode.$$Text.get($$document, makeOuterRange(goal, $$document));
...
function isIntact(self, $$document, index) {
  var goal = self.goals.get(index);
  if (goal !== undefined) {
    return isGoalIntact(goal, $$document);
  } else {
    return false;
  }
}
...
async function modify(self, $$document, index, f) {
  ...
  if (!isGoalIntact(goal, $$document)) {
    return false;
  }
  var innerRange = makeInnerRange(goal, $$document);
  var goalContent = read(goal, $$document);
  return await Editor$AgdaModeVscode.$$Text.replace($$document, innerRange, " " + f(goalContent) + " ");
}
...
async function scanAllGoals(self, editor, changes) {
  ...
  var rewrites = Core__Array.filterMap(Array.from(map.entries()), (function (param) {
          ...
          if (state.TAG === "IsQuestionMark") {
            var offset = state._0;
            var goal = self.goals.get(index);
            if (goal !== undefined && goal.start !== offset) {
              AVLTree$AgdaModeVscode.remove(self.positions, goal.start);
              AVLTree$AgdaModeVscode.insert(self.positions, offset, index);
              self.goals.set(index, {
                    index: goal.index,
                    start: offset,
                    end: offset + 1 | 0,
                    decoration: goal.decoration
  ...
  if (rewrites.length === 0) {
    return setNotBusy(self);
  }
  if (Core__Option.isSome(self.isBusy)) {
    return ;
  }
  var originalCursorPosition = Editor$AgdaModeVscode.Cursor.get(editor);
  setBusy(self);
  var applied = await Editor$AgdaModeVscode.$$Text.batchReplace($$document, rewrites);
  if (!applied) {
    setNotBusy(self);
  }
...
async function resetGoalIndicesOnLoad(self, editor, indices) {
  clear(self);
  var positionsArray = Array.from(self.goalsWithoutIndices.entries()).toSorted(function (param, param$1) {
