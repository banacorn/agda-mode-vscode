...
function redecorate(self, editor) {
  var newGoals = Array.from(self.goals.values()).map(function (goal) {
        destroyGoal(self, goal);
        return make(editor, goal.start, goal.end, goal.index);
      });
  self.goals = new Map(newGoals.map(function (goal) {
            return [
                    goal.index,
                    goal
                  ];
