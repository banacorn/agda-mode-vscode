...
async function scanAllGoals(self, editor, changes) {
  ...
  var scanGoal = function (delta, goal, deltaStart, deltaEnd, destroyed) {
    ...
    var resizeEnd = match[1];
    var resizeStart = match[0];
    console.log("  delta: " + String(delta) + ", deltaStart: " + String(deltaStart) + ", deltaEnd: " + String(deltaEnd) + ", destroyed: " + PervasivesU.string_of_bool(destroyed) + ", intact: " + PervasivesU.string_of_bool(holeIsIntact) + ", resizing: " + String(resizeStart) + "/" + String(resizeEnd) + ", goal: " + toString(goal) + "  \t\"" + holeText + "\"");
    if (holeText === "?") {
      return [
              0,
              [{
      ...
      return [
              ...
              [{
                  TAG: "UpdatePosition",
                  _0: goal,
                  _1: (delta + deltaStart | 0) + resizeStart | 0,
                  _2: (delta + deltaEnd | 0) + resizeEnd | 0,
                  _3: false,
                  [Symbol.for("name")]: "UpdatePosition"
                }]
            ];
    } else {
      console.log("\nDAMAGED: " + toString(goal) + "\n");
      if (holeText.startsWith("{!") && holeText.endsWith("!")) {
        console.log("FIX!");
        return [
                0,
                [
    ...
    while(true) {
      ...
      if (changes) {
        ...
        if (goal.start >= removalStart && goal.end <= removalEnd) {
          ...
          var match = scanGoal(delta, goal, deltaStart, deltaEnd, true);
          var delta$p = ((delta + change.inserted | 0) - change.removed | 0) + match[0] | 0;
        ...
        var match$1 = scanGoal(delta, goal, deltaStart$1, deltaEnd$1, false);
        var delta$p$1 = ((delta + change.inserted | 0) - change.removed | 0) + match$1[0] | 0;
        return Belt_Array.concatMany([
                    match$1[1],
                    go(delta$p$1, goals$1, changes$1)
                  ]);
      }
      var match$2 = scanGoal(delta, goal, 0, 0, false);
      return Belt_Array.concatMany([
                  match$2[1],
                  go(delta + match$2[0] | 0, goals$1, changes)
                ]);
    };
  };
