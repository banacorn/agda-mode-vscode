...
function add(fileName, state) {
  ...
  if (match !== undefined) {
    if (match.TAG === /* PendingInit */0) {
      return $$Promise.get(Curry._1(Decoration$AgdaModeVscode.Module.SemanticHighlighting.requestTokens, state.decoration), match._1);
    } else {
      return ;
    }
  }
...
function requestSemanticTokens(fileName) {
  ...
  if (match !== undefined) {
    if (match.TAG === /* PendingInit */0) {
      ...
      return Curry._1(Decoration$AgdaModeVscode.Module.SemanticHighlighting.requestTokens, match._0.decoration);
    }
  }
