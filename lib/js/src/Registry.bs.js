...
function add(fileName, state) {
  ...
  if (match !== undefined) {
    if (match.TAG === /* PendingInit */0) {
      return $$Promise.get(Highlighting$AgdaModeVscode.requestSemanticTokens(state.highlighting), match._1);
    } else {
      return ;
    }
  }
...
function requestSemanticTokens(fileName) {
  ...
  if (match !== undefined) {
    if (match.TAG === /* PendingInit */0) {
      ...
      return Highlighting$AgdaModeVscode.requestSemanticTokens(match._0.highlighting);
    }
  }
