...
function getDownloadDescriptor(target, useCache) {
  return async function (memento, globalStorageUri, platform) {
    ...
    switch (target) {
      case "LatestALS" :
          ...
          break;
      case "DevALS" :
          repo = Connection__DevALS$AgdaModeVscode.makeAgdaLanguageServerRepo(globalStorageUri);
          break;
      case "DevWASMALS" :
          repo = Connection__DevWASMALS$AgdaModeVscode.makeAgdaLanguageServerRepo(globalStorageUri);
          break;
      
    }
    var toDownloadDescriptor;
    switch (target) {
      case "LatestALS" :
          toDownloadDescriptor = (function (__x) {
              return Connection__LatestALS$AgdaModeVscode.toDownloadDescriptor(__x, platform);
            });
          break;
      case "DevALS" :
          toDownloadDescriptor = (function (__x) {
              return Connection__DevALS$AgdaModeVscode.toDownloadDescriptor(__x, platform);
            });
          break;
      case "DevWASMALS" :
          toDownloadDescriptor = (function (__x) {
              return Connection__DevWASMALS$AgdaModeVscode.toDownloadDescriptor(__x);
            });
          break;
...
function download(_memento, _globalStorageUri) {
  return Promise.resolve({
              TAG: "Error",
...
var Web = {
  ...
  getDownloadDescriptor: getDownloadDescriptor,
  download: download,
