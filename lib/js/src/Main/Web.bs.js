...
function resolveDownloadChannel(channel, _useCache) {
  return async function (memento, globalStorageUri, _platform) {
    ...
    var repo = Connection__Download__Channel$AgdaModeVscode.Dev.makeRepo(globalStorageUri);
    var error = await Connection__Download$AgdaModeVscode.getReleaseManifestFromGitHub(memento, repo, _useCache);
    if (error.TAG === "Ok") {
      return Core__Result.map(Connection__Download__Channel$AgdaModeVscode.Dev.toDownloadOrder(error._0, "Web"), (function (descriptor) {
                    return {
                            TAG: "FromGitHub",
                            _0: "DevALS",
                            _1: descriptor
