...
function activateWithoutContext(platformDeps, subscriptions, extensionUri, globalStorageUri, memento) {
  ...
  subscribe(onOpenEditor(function (editor) {
            var fileName = editor.document.fileName;
            if (isAgda(fileName)) {
              return Core__Option.forEach(Core__Option.flatMap(Registry$AgdaModeVscode.get(fileName), (function (entry) {
                                return entry.state;
                              })), (function (state) {
                            state.editor = editor;
  ...
  subscribeMany(onTriggerCommand(async function (command, editor) {
            ...
            if (exit === 1) {
              var entry = Registry$AgdaModeVscode.get(fileName);
              if (entry !== undefined) {
            ...
            var state$2 = Core__Option.flatMap(Registry$AgdaModeVscode.get(fileName), (function (entry) {
                    return entry.state;
                  }));
