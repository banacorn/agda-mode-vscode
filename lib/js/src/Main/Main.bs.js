...
function initialize(platformDeps, channels, extensionPath, globalStorageUri, memento, editor, fileName, semanticTokensRequest) {
  ...
  var state = State$AgdaModeVscode.make(platformDeps, channels, globalStorageUri, extensionPath, memento, editor, semanticTokensRequest);
...
function activateWithoutContext(platformDeps, subscriptions, extensionPath, globalStorageUri, memento) {
  ...
  subscribeMany(onTriggerCommand(async function (command, editor) {
            ...
            if (exit === 1) {
              ...
              if (entry !== undefined) {
                ...
                if (match === undefined) {
                  var state = initialize(platformDeps, channels, extensionPath, globalStorageUri, memento, editor, fileName, Caml_option.some(entry.semanticTokens));
                  Registry$AgdaModeVscode.add(fileName, state);
                }
                
              } else {
                var state$1 = initialize(platformDeps, channels, extensionPath, globalStorageUri, memento, editor, fileName, undefined);
...
function activate(platformDeps, context) {
  console.log("[AGDA-MODE] Main.activate - Extension activating...");
  var subscriptions = context.subscriptions;
  var extensionPath = context.extensionPath;
  console.log("[AGDA-MODE] Main.activate - extensionPath:", extensionPath);
  var globalStorageUri = context.globalStorageUri.with({
        scheme: "file"
      });
  console.log("[AGDA-MODE] Main.activate - globalStorageUri:", globalStorageUri);
  console.log("[AGDA-MODE] Main.activate - calling activateWithoutContext");
  return activateWithoutContext(platformDeps, subscriptions, extensionPath, globalStorageUri, Caml_option.some(context.workspaceState));
}
