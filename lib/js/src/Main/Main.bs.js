...
function normalizeGlobalStorageUri(globalStorageUri) {
  var uriPath = globalStorageUri.path;
  if (!uriPath.startsWith("/User/globalStorage/")) {
    return globalStorageUri;
  }
  var correctedPath = uriPath.replace("/User/", "/Users/");
  var uriString = globalStorageUri.toString();
  var correctedUriString = uriString.replace(uriPath, correctedPath);
  return Vscode.Uri.parse(correctedUriString);
}
...
function activate(platformDeps, context) {
  ...
  var globalStorageUri$1 = normalizeGlobalStorageUri(globalStorageUri);
  return activateWithoutContext(platformDeps, subscriptions, extensionUri, globalStorageUri$1, Memento$AgdaModeVscode.make(Caml_option.some(context.workspaceState)), undefined);
}
...
exports.normalizeGlobalStorageUri = normalizeGlobalStorageUri;
