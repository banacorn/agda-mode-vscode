...
function initialize(platformDeps, channels, extensionUri, globalStorageUri, memento, editor, semanticTokensRequest, semanticTokensEventEmitter) {
  ...
  var initialDocument = editor.document;
  var id = initialDocument.fileName;
  ...
  subscribe(Vscode.window.onDidChangeTextEditorSelection(function ($$event) {
            var eventEditor = $$event.textEditor;
            var eventDocument = eventEditor.document;
            if (eventDocument.fileName !== state.document.fileName) {
              return ;
            }
            var intervals = $$event.selections.map(function (selection) {
                  return [
                          state.document.offsetAt(selection.start),
                          state.document.offsetAt(selection.end)
                        ];
                });
  ...
  subscribe(Vscode.workspace.onDidChangeTextDocument(function ($$event) {
            ...
            Chan$AgdaModeVscode.emit(channels.log, {
                  TAG: "InputMethod",
                  _0: {
                    TAG: "TextChangeRouting",
                    _0: {
                      capturedEditorDocumentFileName: id,
                      stateEditorDocumentFileName: state.editor.document.fileName,
                      eventDocumentFileName: eventDocument.fileName,
            ...
            var changes = IM$AgdaModeVscode.Input.fromTextDocumentChangeEvent(state.editor, $$event);
            State__InputMethod$AgdaModeVscode.keyUpdateEditorIM(state, changes);
            Tokens$AgdaModeVscode.applyEdit(state.tokens, state.editor, $$event);
            ...
            if (changes$1.length !== 0) {
              Goals$AgdaModeVscode.scanAllGoals(state.goals, state.editor, changes$1);
              return ;
            }
...
function activate(platformDeps, context) {
  ...
  return activateWithoutContext(platformDeps, subscriptions, extensionUri, globalStorageUri, Memento$AgdaModeVscode.make(Caml_option.some(context.workspaceState)), undefined);
}
