...
function registerDefinitionProvider() {
  return Editor$AgdaModeVscode.Provider.registerDefinitionProvider(function ($$document, position) {
              var state = Core__Option.flatMap(Registry$AgdaModeVscode.get($$document), (function (entry) {
                      return entry.state;
                    }));
              if (state !== undefined) {
                return Tokens$AgdaModeVscode.goToDefinition(state.tokens, $$document)(Parser$AgdaModeVscode.Filepath.make($$document.fileName), position);
              }
...
var ExtensionEvents = {
  ...
  registerInputMethodHintHoverProvider: registerInputMethodHintHoverProvider
};
...
function initialize(platformDeps, channels, extensionUri, globalStorageUri, memento, editor, semanticTokensRequest, semanticTokensEventEmitter) {
  ...
  var id = editor.document.fileName;
  var state = State$AgdaModeVscode.make(id, platformDeps, channels, globalStorageUri, extensionUri, memento, editor, semanticTokensRequest);
...
function activateWithoutContext(platformDeps, subscriptions, extensionUri, globalStorageUri, memento, startupCleanupOpt) {
  var startupCleanup = startupCleanupOpt !== undefined ? startupCleanupOpt : Connection__Download__GitHub$AgdaModeVscode.cleanupInFlightFiles;
  ...
  Core__Promise.$$catch(startupCleanup(globalStorageUri), (function (param) {
  ...
  subscribe(Vscode.window.onDidChangeTextEditorSelection(function ($$event) {
            var $$document = $$event.textEditor.document;
            var state = Core__Option.flatMap(Registry$AgdaModeVscode.get($$document), (function (entry) {
                    return entry.state;
                  }));
            if (state === undefined) {
  ...
  subscribe(Vscode.workspace.onDidChangeTextDocument(function ($$event) {
            ...
            var state = Core__Option.flatMap(Registry$AgdaModeVscode.get(eventDocument), (function (entry) {
                    return entry.state;
                  }));
            if (state === undefined) {
              return ;
            }
            var currentEditor = state.editor;
            if (currentEditor.document.fileName !== state.id) {
              return ;
            }
            var changes = IM$AgdaModeVscode.Input.fromTextDocumentChangeEvent(currentEditor, $$event);
            State__InputMethod$AgdaModeVscode.keyUpdateEditorIM(state, changes);
            Tokens$AgdaModeVscode.applyEdit(state.tokens, currentEditor, $$event);
            ...
            if (changes$1.length !== 0) {
              Goals$AgdaModeVscode.scanAllGoals(state.goals, currentEditor, changes$1);
              return ;
            }
  ...
  subscribe(registerDefinitionProvider());
...
function activate(platformDeps, context) {
  ...
  var memento = Memento$AgdaModeVscode.make(Caml_option.some(context.workspaceState));
  var logUri = context.logUri;
  var channels = activateWithoutContext(platformDeps, subscriptions, extensionUri, globalStorageUri, memento, undefined);
  return {
          channels: channels,
          memento: memento,
          logUri: logUri
...
exports.ExtensionEvents = ExtensionEvents;
