...
function initialize(platformDeps, channels, extensionUri, globalStorageUri, memento, editor, fileName, semanticTokensRequest) {
  ...
  subscribeMany(onTriggerCommand(async function (command, editor) {
            ...
            if (exit === 1) {
              ...
              if (entry !== undefined) {
                ...
                if (match === undefined) {
                  var state = initialize(platformDeps, channels, extensionUri, globalStorageUri, memento, editor, fileName, Caml_option.some(entry.semanticTokens));
                  Registry$AgdaModeVscode.add(fileName, state);
                }
                
              } else {
                var state$1 = initialize(platformDeps, channels, extensionUri, globalStorageUri, memento, editor, fileName, undefined);
