...
function make(vscodeTokensResource) {
  return {
          ...
          holes: new Map(),
          holesOffsets: Resource$AgdaModeVscode.make()
        };
}
...
function generateHighlighting(self, editor) {
  ...
  var match = traverseIntervals(AVLTree$AgdaModeVscode.toArray(self.agdaTokens), self.deltas, (function (param, token, action) {
          ...
          if (token.aspects.some(function (x) {
            ...
            holes.set(offsetStart, offsetEnd);
          }
  ...
  Resource$AgdaModeVscode.set(self.holesOffsets, match[1]);
}
...
function getHolesOffsets(self) {
  return self.holesOffsets;
}
...
var Module = {
  ...
  getHolesOffsets: getHolesOffsets,
  getHoles: getHoles,
...
exports.getHolesOffsets = getHolesOffsets;
