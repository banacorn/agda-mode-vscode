...
function insertWithVSCodeOffsets(self, offsetStart, offsetEnd, token) {
...
function generate(self, editor) {
  var $$document = editor.document;
  var match = traverseIntervals(AVLTree$AgdaModeVscode.toArray(self.agdaTokens), self.deltas, (function (param, token, param$1, action) {
          var holes = param[1];
          var acc = param[0];
...
function toOriginalOffset(self, offset) {
  return traverseIntervals(AVLTree$AgdaModeVscode.toArray(self.agdaTokens), self.deltas, (function (acc, param, param$1, action) {
                if (typeof action !== "object") {
                  return acc;
                }
                var delta = action._0;
                if (offset >= (param$1[0] + delta | 0) && offset < (param$1[1] + delta | 0)) {
