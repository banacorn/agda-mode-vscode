    ...
    if (changes) {
      ...
      if (typeof xs !== "object") {
        ...
        return {
                ...
                _2: deltaAfter,
                _3: applyChangeAux("EOF", deltaAfter, translation, changes$1),
      ...
      switch (changeRemovalStart.TAG) {
        case "Before" :
            ...
            if (changeRemovalEnd.TAG === "Before") {
              ...
              return {
                      ...
                      _3: applyChangeAux(xs, deltaBefore + delta$1 | 0, translation + delta$1 | 0, changes$1),
                      [Symbol.for("name")]: "Replace"
                    };
            }
            ...
            return {
                    ...
                    _3: applyChangeAux(tail, deltaAfter$1 + delta$2 | 0, translation + delta$2 | 0, {
                          hd: change$p,
                          tl: changes$1
                        }),
                    [Symbol.for("name")]: "Replace"
        ...
        case "InInsertion" :
            ...
            return {
                    ...
                    _3: applyChangeAux(tail, deltaAfter$1 + delta$3 | 0, translation + delta$3 | 0, {
                          hd: change$p$1,
                          tl: changes$1
                        }),
                    [Symbol.for("name")]: "Replace"
        ...
        case "After" :
            if (changeRemovalEnd.TAG === "Before") {
              ...
              return {
                      ...
                      _1: end,
                      _2: deltaAfter$1,
                      _3: applyChangeAux(tail, deltaAfter$1, translation, {
                            hd: change,
                            tl: changes$1
                          }),
                      [Symbol.for("name")]: "Replace"
      ...
      return {
              ...
              _1: xs._1,
              _2: deltaAfter$2,
              _3: applyChangeAux(xs._3, deltaBefore, translation, /* [] */0),
              [Symbol.for("name")]: "Replace"
            };
    }
  };
...
function toString$3(self) {
  ...
  var tokens = AVLTree$AgdaModeVscode.toArray(self.agdaTokens).map(function (param) {
          return toString(param[0]) + " " + Editor$AgdaModeVscode.$$Range.toString(param[2]);
        }).join("\n    ");
...
function insert(self, editor, tokens) {
  ...
  tokens.forEach(function (info) {
        var offsetStart = Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, info.start);
        var offsetEnd = Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, info.end_);
        var existing = AVLTree$AgdaModeVscode.find(self.agdaTokens, offsetStart);
        if (existing !== undefined) {
          var old = existing[0];
          self.agdaTokens.remove(offsetStart);
          ...
          self.agdaTokens.insert(offsetStart, [
                $$new,
                existing[1],
                existing[2]
              ]);
        ...
        var start = $$document.positionAt(offsetStart);
...
function lookupSrcLoc(self, offset) {
  return Core__Option.map(Core__Option.flatMap(AVLTree$AgdaModeVscode.lowerBound(self.agdaTokens, offset), (function (param) {
                    var range = param[2];
...
function toDecorationsAndSemanticTokens(tokens, editor) {
  var xs = AVLTree$AgdaModeVscode.toArray(tokens.agdaTokens).map(function (param) {
          var info = param[0];
          var ranges = Highlighting__SemanticToken$AgdaModeVscode.Module.SingleLineRange.splitRange(editor.document, param[2]);
  ...
  return fromAspects(editor, xs);
}

function toDecorations(self, editor) {
  var aspects = AVLTree$AgdaModeVscode.toArray(self.agdaTokens).map(function (param) {
          var range = param[2];
...
function applyEdit(self, editor, $$event) {
  ...
  self.deltas = applyChanges(self.deltas, changes);
  var match = Core__Array.reduce(AVLTree$AgdaModeVscode.toArray(self.agdaTokens), [
        [],
        self.deltas,
        0
      ], (function (param, param$1) {
          var match = param$1[1];
          var offsetEnd = match[1];
          var offsetStart = match[0];
          var token = param$1[0];
          var deltaBefore = param[2];
          var interval = param[1];
          var $$document = editor.document;
          var match$1;
          if (typeof interval !== "object" || offsetEnd < interval._0) {
            match$1 = [
              {
                TAG: "Translate",
                _0: deltaBefore,
          ...
          } else if (offsetStart < interval._1) {
            match$1 = [
              "Remove",
              interval,
              deltaBefore
          ...
          var action = match$1[0];
          var tokens;
          if (typeof action !== "object") {
            tokens = [];
          } else {
            var delta$1 = action._0;
            var offsetStart$1 = offsetStart + delta$1 | 0;
            var offsetEnd$1 = offsetEnd + delta$1 | 0;
            var range = new Vscode.Range($$document.positionAt(offsetStart$1), $$document.positionAt(offsetEnd$1));
            var singleLineRanges = Highlighting__SemanticToken$AgdaModeVscode.Module.SingleLineRange.splitRange($$document, range);
            tokens = singleLineRanges.map(function (range) {
                  return [
                          token.aspects,
                          range
                        ];
