...
function generate(self, editor) {
  var $$document = editor.document;
  var convert = function (_acc, tokens, _i, _intervals, _deltaBefore) {
    while(true) {
      ...
      var match$1 = match[1];
      var offsetEnd = match$1[1];
      var offsetStart = match$1[0];
      var token = match[0];
      var match$2;
      match$2 = typeof intervals !== "object" || offsetEnd < intervals._0 ? [
          {
            TAG: "Translate",
            _0: deltaBefore,
          ...
          offsetStart < intervals._1 ? [
            ...
            ] : [
              {
                TAG: "NextInterval",
                _0: intervals._3,
                [Symbol.for("name")]: "NextInterval"
              },
              intervals._2
            ]
        );
      var deltaAfter = match$2[1];
      var action = match$2[0];
      if (typeof action !== "object") {
        _deltaBefore = deltaAfter;
        _i = i + 1 | 0;
        continue ;
      }
      if (action.TAG === "Translate") {
        var delta = action._0;
        var offsetStart$1 = offsetStart + delta | 0;
        var offsetEnd$1 = offsetEnd + delta | 0;
        ...
        var result = singleLineRanges.map((function(token){
      ...
      _deltaBefore = deltaAfter;
      _intervals = action._0;
      continue ;
    };
  };
  var aspects = convert([], AVLTree$AgdaModeVscode.toArray(self.agdaTokens), 0, self.deltas, 0);
  var match = fromAspects(editor, aspects);
  var semanticTokens = match[1];
  console.log("setting SemanticTokens: " + semanticTokens.length.toString());
  Resource$AgdaModeVscode.set(self.vscodeTokens, semanticTokens);
}
