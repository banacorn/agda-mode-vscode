...
var Token = {
  toStringWithoutOffsets: toStringWithoutOffsets,
  toString: toString,
  parse: parse,
...
function reset(self) {
  console.log("Tokens.reset: Starting token reset");
  self.tempFiles.forEach(function (format) {
        var filepath = format._0;
        var uri = Vscode.Uri.file(filepath);
        FS$AgdaModeVscode.$$delete(uri);
      });
  console.log("Tokens.reset: Deleted temp files");
  self.agdaTokens = AVLTree$AgdaModeVscode.make();
  console.log("Tokens.reset: Reset agdaTokens");
  self.deltas = "EOF";
  console.log("Tokens.reset: Reset deltas");
  if (Resource$AgdaModeVscode.isPending(self.vscodeTokens)) {
    console.log("Tokens.reset: vscodeTokens is pending, not resetting");
  } else {
    console.log("Tokens.reset: Setting vscodeTokens to empty array");
    Resource$AgdaModeVscode.set(self.vscodeTokens, []);
    console.log("Tokens.reset: vscodeTokens set to empty array");
  }
  console.log("Tokens.reset: Token reset completed");
}
