...
function toStringWithoutOffsets(self) {
  ...
  return Util$AgdaModeVscode.Pretty.list(Core__List.fromArray(self.aspects.map(Highlighting__AgdaAspect$AgdaModeVscode.toString))) + (
...
function convertFromAgdaAspectsAndApplyDecorations(editor, xs) {
  ...
  var decorations = Highlighting__Decoration$AgdaModeVscode.apply(match[1].flat(), editor);
  return [
          decorations,
          semanticTokens
        ];
...
function generateHighlighting(self, editor) {
  ...
  var match$1 = convertFromAgdaAspectsAndApplyDecorations(editor, match[0]);
