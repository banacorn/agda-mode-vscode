...
function make(vscodeTokensResource) {
  return {
          ...
          loadConverter: undefined,
          editsSinceLoad: TokenIntervals$AgdaModeVscode.empty,
          ...
          onUpdate: Chan$AgdaModeVscode.make()
        };
}
...
function insertWithVSCodeOffsets(self, token) {
  ...
  if (existing !== undefined) {
    ...
    var newAspects = Caml_obj.equal(existing.aspects, token.aspects) ? existing.aspects : Core__Array.reduce(token.aspects, existing.aspects, (function (acc, aspect) {
              if (acc.some(function (x) {
                      return x === aspect;
                    })) {
                return acc;
              } else {
                return acc.concat([aspect]);
              }
            }));
...
function insertTokens(self, editor, tokens) {
  ...
  var converter = self.loadConverter;
  var offsetConverter = converter !== undefined ? Caml_option.valFromOption(converter) : Agda$AgdaModeVscode.OffsetConverter.make(Editor$AgdaModeVscode.$$Text.getAll($$document));
  var shift = function (offset) {
    return TokenIntervals$AgdaModeVscode.translateOffset(self.editsSinceLoad, offset);
  };
  var currentFilepath = Parser$AgdaModeVscode.Filepath.make($$document.fileName);
  tokens.forEach(function (token) {
        var start = shift(Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, token.start));
        var end = shift(Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, token.end));
        var source = Core__Option.map(token.source, (function (param) {
                var agdaOffset = param[1];
                var filepath = param[0];
                if (Caml_obj.equal(filepath, currentFilepath)) {
                  return [
                          filepath,
                          shift(Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, agdaOffset - 1 | 0))
                        ];
                } else {
                  return [
                          filepath,
...
function beginLoad(self, $$document) {
  self.loadConverter = Caml_option.some(Agda$AgdaModeVscode.OffsetConverter.make(Editor$AgdaModeVscode.$$Text.getAll($$document)));
  self.editsSinceLoad = TokenIntervals$AgdaModeVscode.empty;
  self.holes = new Map();
  self.holePositions = Resource$AgdaModeVscode.make();
}

function endLoad(self) {
  self.loadConverter = undefined;
  self.editsSinceLoad = TokenIntervals$AgdaModeVscode.empty;
}
...
function reset(self) {
  ...
  if (Resource$AgdaModeVscode.isPending(self.vscodeTokens)) {
    ...
    Resource$AgdaModeVscode.set(self.vscodeTokens, []);
  }
  self.decorations.forEach(function (_ranges, decoration) {
        Editor$AgdaModeVscode.Decoration.destroy(decoration);
      });
  self.decorations = new Map();
  self.liveDecorations = 0;
}

function destroyUpdateChannel(self) {
  Chan$AgdaModeVscode.destroy(self.onUpdate);
}
  ...
  return function (fileName, position) {
    ...
    if (Caml_obj.equal(fileName, currentFileName)) {
      ...
      return Core__Option.map(Core__Option.flatMap(AVLTree$AgdaModeVscode.lowerBound(self.agdaTokens, offset), (function (token) {
                    ...
                    var srcOffset = param[2];
                    var filepath = param[1];
                    var srcRange = param[0];
                    var filepathStr = Parser$AgdaModeVscode.Filepath.toString(filepath);
                    return Vscode.workspace.openTextDocument(filepathStr).then(function (sourceDoc) {
                                var position;
                                if (Caml_obj.equal(filepath, currentFilepath)) {
                                  position = sourceDoc.positionAt(srcOffset);
                                } else {
                                  var text = Editor$AgdaModeVscode.$$Text.getAll(sourceDoc);
                                  var offsetConverter = Agda$AgdaModeVscode.OffsetConverter.make(text);
                                  var vscodeOffset = Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, srcOffset - 1 | 0);
                                  position = sourceDoc.positionAt(vscodeOffset);
                                }
                                return [[
                                          srcRange,
                                          filepathStr,
...
function rebaseTokens(self, editingFilepath) {
  var translateSourceOffset = function (vscodeOffset) {
    return TokenIntervals$AgdaModeVscode.translateOffset(self.deltas, vscodeOffset);
  };
  var newAgdaTokens = AVLTree$AgdaModeVscode.make();
  var newHoles = new Map();
  traverseIntervals(toTokenArray(self), self.deltas, (function (param, token, action) {
          if (typeof action !== "object") {
            return ;
          }
          var delta = action._0;
          var newSource = Core__Option.map(token.source, (function (param) {
                  var srcOffset = param[1];
                  var filepath = param[0];
                  if (Caml_obj.equal(filepath, editingFilepath)) {
                    return [
                            filepath,
                            translateSourceOffset(srcOffset)
                          ];
                  } else {
                    return [
                            filepath,
          ...
          var rebased_start = token.start + delta | 0;
          var rebased_end = token.end + delta | 0;
          var rebased_aspects = token.aspects;
          var rebased_isTokenBased = token.isTokenBased;
          var rebased_note = token.note;
          var rebased = {
            start: rebased_start,
            end: rebased_end,
            aspects: rebased_aspects,
            isTokenBased: rebased_isTokenBased,
            note: rebased_note,
            source: newSource
          };
          if (rebased_aspects.some(function (x) {
                  return x === "Hole";
                })) {
            newHoles.set(rebased_start, rebased);
          }
          AVLTree$AgdaModeVscode.insert(newAgdaTokens, rebased_start, rebased);
        }), undefined);
...
function applyEdit(self, editor, $$event) {
  ...
  if (Core__Option.isSome(self.loadConverter)) {
    self.editsSinceLoad = TokenIntervals$AgdaModeVscode.applyChanges(self.editsSinceLoad, changes.toReversed());
  }
...
function deltasLength(self) {
  return TokenIntervals$AgdaModeVscode.length(self.deltas);
}

function liveDecorationTypes(self) {
  return self.liveDecorations;
}
...
var Module = {
  ...
  liveDecorationTypes: liveDecorationTypes
};
...
exports.deltasLength = deltasLength;
exports.liveDecorationTypes = liveDecorationTypes;
