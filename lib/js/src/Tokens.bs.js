...
function toString$2(xs) {
  ...
  var start = xs._0;
  if (start === end) {
    return "━━┫" + String(end) + " ━" + deltaToString(delta) + toString$2(tail);
  } else {
    return "━━┫" + String(start) + "     ┃" + String(end) + " ━━" + deltaToString(delta) + toString$2(tail);
  }
}
...
function isValid(xs) {
  ...
  while(true) {
    ...
    var delta = xs$1._2;
    if ((prevDelta - delta | 0) > (end - start | 0)) {
      return false;
    }
    _xs = xs$1._3;
    _prevDelta = delta;
...
function debugIsValid(xs) {
  ...
  var aux = function (_prevEnd, _prevDelta, _xs) {
    while(true) {
      var xs = _xs;
      var prevDelta = _prevDelta;
      var prevEnd = _prevEnd;
      ...
      var start = xs._0;
      if (start < prevEnd) {
        return "Has overlapping intervals: {\n  start: " + String(start) + "\n  end: " + String(end) + "\n  delta: " + String(delta) + "\n}\n";
      }
      if (start > end) {
        return "Has reversed order intervals: {\n  start: " + String(start) + "\n  end: " + String(end) + "\n  delta: " + String(delta) + "\n}\n";
      }
      if ((prevDelta - delta | 0) > (end - start | 0)) {
        return "Has negative translation larger than the length of the removed text: {\n  start: " + String(start) + "\n  end: " + String(end) + "\n  delta: " + String(delta) + "\n}\n";
      }
      _xs = xs._3;
      _prevDelta = delta;
      _prevEnd = end;
  ...
  var error = aux(0, 0, xs);
  if (error !== undefined) {
    console.log("Intervals: " + toString$2(xs));
    console.log("Intervals are not valid: " + error);
    return ;
  }
...
function applyChangesAux(xs, changes, i, accDelta) {
  var match = changes[i];
  if (match === undefined) {
    return xs;
  }
  ...
  var delta = (accDelta + match.inserted | 0) - match.removed | 0;
  return {
          TAG: "Replace",
          _0: match.offset,
          _1: match.offset + match.removed | 0,
          _2: delta,
          _3: applyChangesAux("EOF", changes, i + 1 | 0, delta),
          [Symbol.for("name")]: "Replace"
        };
}

function applyChanges(xs, changes) {
  return applyChangesAux(xs, changes, 0, 0);
}
