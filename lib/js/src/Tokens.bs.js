...
function toString(self) {
  ...
  return "Token (" + String(self.start) + ", " + String(self.end_) + ") " + Util$AgdaModeVscode.Pretty.list(Belt_List.fromArray(self.aspects)) + (
...
function decodeResponseHighlightingInfoDirect(param) {
  return Json_decode.map((function (param) {
                return [
                        param[0],
                        param[1]
...
function readAndParse(format) {
  return $$Promise.map($$Promise.Js.toResult($$Promise.Js.fromBsPromise(Util.promisify(function (prim0, prim1) {
                ...
                return decodeResponseHighlightingInfoDirect(json);
              }));
}
