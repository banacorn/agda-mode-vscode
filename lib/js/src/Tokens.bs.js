...
function applyChangesAux(xs, changes, deltaBefore) {
  if (!changes) {
    return xs;
  }
  ...
  if (typeof xs !== "object") {
    ...
    return {
            TAG: "Replace",
            _0: change.offset,
            _1: change.offset + change.removed | 0,
            _2: deltaAfter,
            _3: applyChangesAux("EOF", changes$1, deltaAfter),
            [Symbol.for("name")]: "Replace"
          };
  }
  ...
  if (changeRemovalStart.TAG !== "Before") {
    if (typeof changeRemovalEnd !== "object") {
      return Js_exn.raiseError(" Replaced / After");
    } else if (changeRemovalEnd.TAG === "Before") {
      return Js_exn.raiseError("Not a possible case");
    } else {
      return Js_exn.raiseError(" Replaced / Replaced");
    }
  }
  if (typeof changeRemovalEnd !== "object") {
    return Js_exn.raiseError(" Before/After");
  }
  if (changeRemovalEnd.TAG !== "Before") {
    return Js_exn.raiseError(" Before/Replaced");
  }
  ...
  return {
          TAG: "Replace",
          _0: changeRemovalStart._0,
          _1: changeRemovalEnd._0,
          _2: deltaAfter$1 + delta$1 | 0,
          _3: adjustDelta(delta$1, applyChangesAux(xs, changes$1, deltaAfter$1 + delta$1 | 0)),
          [Symbol.for("name")]: "Replace"
        };
}

function applyChanges(xs, changes) {
  return applyChangesAux(xs, Core__List.fromArray(changes), 0);
}
