...
function toString$1(self) {
  return "(" + String(self.offset) + ", " + String(self.removed) + ", " + String(self.inserted) + ")";
}
...
function arbitraryBatch(batchSize) {
  var aux = function (after, size) {
    if (size === 0) {
      return FastCheck.constant([]);
    } else {
      return arbitrary(after).chain(function (change) {
                  return aux(change.offset + change.removed | 0, size - 1 | 0).map(function (changes) {
...
function calculateOriginalOffset(deltaBefore, start, end, deltaAfter, x) {
  if (x <= (deltaBefore + start | 0)) {
    return {
            TAG: "Before",
            _0: x - deltaBefore | 0,
            [Symbol.for("name")]: "Before"
          };
  } else if (x > (end + deltaAfter | 0)) {
    return "After";
  } else {
    return {
            TAG: "Replaced",
...
function applyChangesAux(_xs, _changes, _deltaBefore, _deltaBeforeChange, _fromBM) {
  while(true) {
    var fromBM = _fromBM;
    var deltaBeforeChange = _deltaBeforeChange;
    ...
    if (typeof xs !== "object") {
      ...
      console.log("== EOF non empty ============================================");
      console.log("channge: " + toString$1(change));
      console.log("interval delta before: " + String(deltaBefore));
      var deltaBefore$1 = Core__Option.getOr(deltaBeforeChange, deltaBefore);
      console.log("interval delta before*: " + String(deltaBefore$1));
      var deltaAfter = (deltaBefore$1 + change.inserted | 0) - change.removed | 0;
      console.log("interval delta after: " + String(deltaAfter));
      return {
              TAG: "Replace",
              _0: change.offset,
              _1: fromBM !== undefined ? fromBM : change.offset + change.removed | 0,
              _2: deltaAfter,
              _3: applyChangesAux("EOF", changes.tl, deltaAfter, undefined, undefined),
              [Symbol.for("name")]: "Replace"
            };
    }
    var changes$1 = changes.tl;
    var change$1 = changes.hd;
    var tail = xs._3;
    var deltaAfter$1 = xs._2;
    var end = xs._1;
    var start = xs._0;
    var changeStart = calculateOriginalOffset(deltaBefore, start, end, deltaAfter$1, change$1.offset);
    var changeEnd = calculateOriginalOffset(deltaBefore, start, end, deltaAfter$1, change$1.offset + change$1.removed | 0);
    if (typeof changeStart !== "object") {
      return Js_exn.raiseError("Not a possible case");
    }
    if (changeStart.TAG !== "Before") {
      return Js_exn.raiseError("Not a possible case");
    }
    var changeStart$1 = changeStart._0;
    if (typeof changeEnd !== "object") {
      console.log("== Before/After ============================================");
      printInterval(deltaBefore, start, end, deltaAfter$1);
      console.log("\nchange: " + toString$1(change$1));
      var deltaBeforeChange$1 = Core__Option.getOr(deltaBeforeChange, deltaBefore);
      var removedByInterval = end - start | 0;
      var insertedByInterval = (deltaAfter$1 - deltaBefore | 0) + removedByInterval | 0;
      var change_offset = change$1.offset;
      var change_removed = change$1.removed + removedByInterval | 0;
      var change_inserted = change$1.inserted + insertedByInterval | 0;
      var change$2 = {
        offset: change_offset,
        removed: change_removed,
        inserted: change_inserted
      };
      console.log("     => " + toString$1(change$2));
      _fromBM = undefined;
      _deltaBeforeChange = deltaBeforeChange$1;
    ...
    if (changeEnd.TAG === "Before") {
      console.log("== Before/Before ============================================");
      printInterval(deltaBefore, start, end, deltaAfter$1);
      console.log("change: " + toString$1(change$1));
      var delta = change$1.inserted - change$1.removed | 0;
      return {
              TAG: "Replace",
              _0: changeStart$1,
              _1: changeEnd._0,
              _2: deltaBefore + delta | 0,
              _3: adjustDelta(delta, tail),
              [Symbol.for("name")]: "Replace"
            };
    }
    console.log("== Before/Middle ============================================");
    printInterval(deltaBefore, start, end, deltaAfter$1);
    var insertedByInteval = ((deltaAfter$1 - deltaBefore | 0) + end | 0) - start | 0;
    var removedByInterval$1 = end - start | 0;
    console.log("change: " + toString$1(change$1));
    var change_offset$1 = change$1.offset;
    var change_removed$1 = (change$1.removed + change$1.offset | 0) - changeStart$1 | 0;
    var change_inserted$1 = (change$1.inserted + insertedByInteval | 0) - removedByInterval$1 | 0;
    var change$3 = {
      offset: change_offset$1,
      removed: change_removed$1,
      inserted: change_inserted$1
    };
    console.log("     => " + toString$1(change$3));
    _fromBM = end;
    _deltaBeforeChange = undefined;
    _changes = {
      hd: change$3,
      tl: changes$1
    };
...
function applyChanges(xs, changes) {
  return applyChangesAux(xs, Core__List.fromArray(changes), 0, undefined, undefined);
}
