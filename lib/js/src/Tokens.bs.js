...
function insertWithVSCodeOffsets(self, editor, offsetStart, offsetEnd, token) {
  var $$document = editor.document;
  var existing = AVLTree$AgdaModeVscode.find(self.agdaTokens, offsetStart);
  if (existing !== undefined) {
    var range = existing[2];
    var old = existing[0];
    self.agdaTokens.remove(offsetStart);
    var newAspects = Caml_obj.equal(old.aspects, token.aspects) ? old.aspects : old.aspects.concat(token.aspects);
  ...
  var range$1 = new Vscode.Range($$document.positionAt(offsetStart), $$document.positionAt(offsetEnd));
  self.agdaTokens.insert(offsetStart, [
        token,
        [
          offsetStart,
  ...
  if (token.aspects.some(function (x) {
    ...
    console.log("start: " + String(token.start) + " => " + String(offsetStart));
    self.holes.set(offsetStart, [
          token,
          [
            offsetStart,
...
function insertTokens(self, editor, tokens) {
  ...
  tokens.forEach(function (token) {
        var offsetStart = Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, token.start);
        var offsetEnd = Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, token.end);
...
function generate(self, editor) {
  var $$document = editor.document;
  var convert = function (_param, tokens, _i, _intervals, _deltaBefore) {
    while(true) {
      var param = _param;
  ...
  var match = convert([
        [],
        new Map()
      ], AVLTree$AgdaModeVscode.toArray(self.agdaTokens), 0, self.deltas, 0);
  self.holes = match[1];
...
function getHolesSorted(self) {
  return Array.from(self.holes.values()).toSorted(function (param, param$1) {
              return Caml.int_compare(param[1][0], param$1[1][0]);
            });
}
