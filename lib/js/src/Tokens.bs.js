...
function insert(self, editor, tokens) {
  tokens.forEach(function (info) {
        ...
        var existing = AVLTree$AgdaModeVscode.find(self.agdaTokens, startOffset);
        if (existing !== undefined) {
          var old = existing[0];
          self.agdaTokens.remove(startOffset);
          ...
          self.agdaTokens.insert(startOffset, [
                $$new,
                existing[1],
                existing[2]
              ]);
...
function toDecorationsAndSemanticTokens(tokens, editor) {
  var match = Belt_Array.unzip(Core__Array.filterMap(AVLTree$AgdaModeVscode.toArray(tokens.agdaTokens).map(function (param) {
                  var info = param[0];
