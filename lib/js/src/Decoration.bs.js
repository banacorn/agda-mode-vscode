...
function normalize($$document, point) {
  var match = VSCode.TextDocument.eol($$document);
  var useCRLF = match ? false : true;
  if (!useCRLF) {
    return point;
  }
  var skippedCRLF = point.line;
  return point.translate(0, skippedCRLF);
}

function offsetToPoint($$document, indices, offset) {
  var offset$1 = Editor$AgdaModeVscode.Indices.convert(indices, offset);
...
function offsetToPointSlow($$document, offset) {
  ...
  var indices = Editor$AgdaModeVscode.Indices.make(Editor$AgdaModeVscode.computeUTF16SurrogatePairIndices(text));
  var offset$1 = Editor$AgdaModeVscode.Indices.convert(indices, offset);
  var point = $$document.positionAt(offset$1);
  return normalize($$document, point);
}
...
function decorateHighlightings(editor, highlightings) {
  ...
  var highlightings$1 = Belt_Array.map(highlightings, (function (highlighting) {
          var start = offsetToPoint($$document, indices, highlighting.start);
          var end_ = offsetToPoint($$document, indices, highlighting.end_);
