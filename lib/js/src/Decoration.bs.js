...
function toString(token) {
  ...
  return "(" + String(range.start.line) + ", " + String(range.start.character) + ") (" + String(range.end.line) + ", " + String(range.end.character) + ") " + tokenType + " " + modifiers;
}
...
function update(self, $$event) {
  ...
  var partial_arg = self.semanticTokens;
  return Belt_Array.forEach(changes, (function (param) {
                console.log(Belt_Array.map(partial_arg, toString));
                console.log(param);
...
function reset(self, editor) {
  var $$document = editor.document;
  var text = Editor$AgdaModeVscode.$$Text.getAll($$document);
  var offsetConverter = Agda$AgdaModeVscode.OffsetConverter.make(text);
  var intervalTree = new IntervalTree();
  Belt_Array.forEach(self.highlightings, (function (highlighting) {
          var start = Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, highlighting.start);
          var end_ = Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, highlighting.end_);
          var range = Editor$AgdaModeVscode.$$Range.fromInterval($$document, [
                start,
          ...
          var alreadyExists = intervalTree.intersect_any([
                start,
                end_ - 1 | 0
              ]);
          if (!alreadyExists) {
  ...
  var tokens = Belt_Array.keepMap(Belt_Array.concatMany(Belt_Array.map(intervalTree.items, (function (item) {
                  var match = item.value;
                  var aspects = match[1];
                  var ranges = lines(editor.document, match[0]);
