...
function toDecorations(infosWithRanges, editor) {
  var aspects = Belt_Array.concatMany(Belt_Array.map(infosWithRanges, (function (param) {
              var range = param[1];
...
function applyHighlightings(self, editor, infosWithRanges) {
  var decorations = toDecorations(infosWithRanges, editor);
...
function convert(editor, infosWithRanges) {
  return Belt_Array.keepMap(Belt_Array.concatMany(Belt_Array.map(infosWithRanges, (function (param) {
                        var info = param[0];
                ...
                Belt_Array.forEach(Belt_Array.keepMap(param[0], Highlighting__Agda$AgdaModeVscode.Aspect.toTokenTypeAndModifiers), (function (param) {
...
function apply(self, editor) {
  return $$Promise.map(readTempFiles(self), (function (param) {
                var infosWithRanges = toArrayWithRange(editor, self.infos);
                self.srcLocs = Belt_Array.keepMap(infosWithRanges, (function (param) {
                        var range = param[1];
                ...
                if (Config$AgdaModeVscode.Highlighting.getSemanticHighlighting(undefined)) {
                  var tokens = convert(editor, infosWithRanges);
                  if (tokens.length === 0) {
                    var match = $$Promise.pending(undefined);
