...
function fromInfostoDecorations(tokensWithRanges, editor) {
  var aspects = Belt_Array.concatMany(Belt_Array.map(AVLTree$AgdaModeVscode.toArray(tokensWithRanges), (function (param) {
              var range = param[1];
...
function fromTokens(tokens, editor) {
  var match = Belt_Array.unzip(Belt_Array.keepMap(Belt_Array.concatMany(Belt_Array.map(AVLTree$AgdaModeVscode.toArray(Tokens$AgdaModeVscode.get(tokens)), (function (param) {
                      var info = param[0];
...
function apply(self, tokens, editor) {
  return $$Promise.map(Tokens$AgdaModeVscode.readTempFiles(tokens, editor), (function (param) {
                if (Config$AgdaModeVscode.Highlighting.getSemanticHighlighting(undefined)) {
                  var match = fromTokens(tokens, editor);
                ...
                var decorations = fromInfostoDecorations(Tokens$AgdaModeVscode.get(tokens), editor);
