...
function decorateHighlightings(editor, highlightings) {
  ...
  var highlightings$1 = Belt_Array.map(highlightings.items, (function (item) {
...
function toString$1(token) {
  var tokenType = Curry._1(Highlighting$AgdaModeVscode.Aspect.TokenType.toString, token.type_);
...
function make(param) {
  ...
  return {
          tempFilePaths: [],
          highlightings: new IntervalTree(),
          decorations: [],
          srcLocs: [],
...
function destroy(self) {
  ...
  self.tempFilePaths = [];
  self.highlightings = new IntervalTree();
  return clear(self);
}
...
function addViaPipe(self, highlightings) {
  return Belt_Array.forEach(highlightings, (function (highlighting) {
                var alreadyExists = self.highlightings.intersect_any([
                      highlighting.start,
                      highlighting.end_ - 1 | 0
                    ]);
                if (!alreadyExists) {
                  self.highlightings.insert([
                        highlighting.start,
                        highlighting.end_ - 1 | 0
                      ], highlighting);
                  return ;
...
function classify(change, token) {
  ...
  if (tokenRange.end.isBeforeOrEqual(removedRange.start)) {
    return /* NoOp */0;
  } else if (removedRange.contains(tokenRange)) {
    return /* Remove */1;
...
function update(self, $$event) {
  ...
  return $$Promise.get(self.semanticTokens, (function (semanticTokensRef) {
                console.log("UPDATE", semanticTokensRef.contents.length);
...
function apply(self, editor) {
  console.log("APPLY FROM HIGHLIGHTINGS", self.highlightings.size);
  var $$document = editor.document;
  var text = Editor$AgdaModeVscode.$$Text.getAll($$document);
  var offsetConverter = Agda$AgdaModeVscode.OffsetConverter.make(text);
  var tokens = Belt_Array.keepMap(Belt_Array.concatMany(Belt_Array.map(self.highlightings.items, (function (item) {
                  var highlighting = item.value;
                  var start = Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, highlighting.start);
                  var end_ = Agda$AgdaModeVscode.OffsetConverter.convert(offsetConverter, highlighting.end_);
                  var range = Editor$AgdaModeVscode.$$Range.fromInterval($$document, [
          ...
          Belt_Array.forEach(Belt_Array.keepMap(param[1], Highlighting$AgdaModeVscode.Aspect.toTokenTypeAndModifiers), (function (param) {
  ...
  console.log("DEPOSIT", Belt_Array.map(tokens, toString$1));
