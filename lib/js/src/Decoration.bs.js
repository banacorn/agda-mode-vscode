...
function applyHighlightings(self, editor) {
  var decorations = toDecorations(self.infosWithRanges, editor);
...
function convert(self, editor) {
  return Belt_Array.keepMap(Belt_Array.concatMany(Belt_Array.map(IntervalTree$AgdaModeVscode.elems(self.infosWithRanges), (function (param) {
                        var info = param[0];
...
function applyAndClear(self, editor) {
  return $$Promise.map(readTempFiles(self), (function (param) {
                ...
                if (Config$AgdaModeVscode.Highlighting.getSemanticHighlighting(undefined)) {
                  var tokens = convert(self, editor);
                  if (tokens.length === 0) {
                    var match = $$Promise.pending(undefined);
