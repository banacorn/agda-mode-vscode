...
function generateSemanticTokens(self, editor) {
  var $$document = editor.document;
  var text = Editor$AgdaModeVscode.$$Text.getAll($$document);
  var offsetConverter = Agda$AgdaModeVscode.OffsetConverter.make(text);
  var intervalTree = new IntervalTree();
  console.log("highlightings", self.highlightings.length);
  ...
  return Belt_Array.keepMap(Belt_Array.concatMany(Belt_Array.map(intervalTree.items, (function (item) {
                ...
                Belt_Array.forEach(param[1], (function (aspect) {
                        var match = Highlighting$AgdaModeVscode.Aspect.toTokenTypeAndModifiers(aspect);
                        var tokenModifiers = match[1];
                        var tokenModifiers$1 = tokenModifiers !== undefined ? tokenModifiers : [];
                        tokenTypeAccum.push(match[0]);
                        Caml_splice_call.spliceObjApply(tokenModifiersAccum, "push", [tokenModifiers$1]);
