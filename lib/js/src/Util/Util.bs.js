...
function toString(e) {
  var message = Core__Option.getOr(e.message, "");
  var name = (e.name || 'Error');
  var stack = (e.stack || '');
  if (message.length > 0) {
    if (stack.length > 0 && stack !== message) {
      return name + ": " + message + "\n" + stack;
    } else {
      return name + ": " + message;
    }
  }
  var stringified = (e.toString());
  if (stringified !== "[object Object]") {
    return stringified;
  }
  var jsonString;
  try {
    jsonString = (JSON.stringify(e, null, 2));
  }
  catch (exn){
    jsonString = "Unknown error (cannot stringify)";
  }
