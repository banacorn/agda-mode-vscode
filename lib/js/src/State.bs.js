...
function sendRequest(state, handleResponse, request) {
  return push(state.agdaRequestQueue, (function (request) {
                ...
                return $$Promise.flatMap(Connection$AgdaModeVscode.sendRequest(Config$AgdaModeVscode.Connection.useAgdaLanguageServer(undefined), state.devMode, state.document, request, onResponse), (function (result) {
                              if (result.TAG === /* Ok */0) {
...
function make$2(chan, editor, view, devMode) {
  return {
          devMode: devMode,
          editor: editor,
