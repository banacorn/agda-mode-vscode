...
async function handleDownloadPolicy(state, policy) {
  switch (policy) {
    case "YesKeepUpToDate" :
        ...
        var target = await Connection$AgdaModeVscode.downloadLatestALS(state.memento, state.globalStorageUri, reportProgress);
        if (target !== undefined) {
          console.log(target);
          return ;
        } else {
          return await State__View$AgdaModeVscode.Panel.display(state, {
                      TAG: "Error",
                      _0: "Cannot find the latest Agda Language Server for download",
