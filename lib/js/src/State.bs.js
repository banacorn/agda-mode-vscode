...
function Impl(Editor) {
  ...
  var connect = function (state) {
    ...
    if (match !== undefined) {
      ...
      return $$Promise.mapOk($$Promise.mapError(Connection$AgdaModeVscode.make(Editor.Config.getAgdaPath, Editor.Config.setAgdaPath), (function (e) {
                        return /* Connection */Caml_chrome_debugger.simpleVariant("Connection", [e]);
                      })), (function (conn) {
  ...
  var sendRequest = function (state, request) {
    ...
    var encoded = Request$AgdaModeVscode.encode("2.6.1", "Editor.getFileName(state.editor)", libraryPath, highlightingMethod, request);
    console.log("<<<", encoded);
