  ...
  return State__Type$AgdaModeVscode.RequestQueue.push(state.agdaRequestQueue, (function (request) {
                var onResponse = function (result) {
                  if (result.TAG === /* Ok */0) {
                    ...
                    return State__View$AgdaModeVscode.displayConnectionError(state, result._0);
                  }
                };
                return $$Promise.flatMap(Connection$AgdaModeVscode.sendRequest(state.globalStoragePath, Config$AgdaModeVscode.Connection.useAgdaLanguageServer(undefined), state.document, request, onResponse), (function (result) {
                              if (result.TAG === /* Ok */0) {
                                return State__View$AgdaModeVscode.displayConnectionStatus(state, result._0);
                              } else {
                                return State__View$AgdaModeVscode.displayConnectionError(state, result._0);
                              }
                            }));
...
function make(chan, globalStoragePath, editor, panel) {
  return {
          editor: editor,
          ...
          debugBufferCache: State__Type$AgdaModeVscode.ViewCache.make(undefined),
          goals: [],
