...
function refresh(state) {
  return Decoration$AgdaModeVscode.refresh(state.decorations, state.editor);
}
...
function sendRequest(state, handleResponse, request) {
      ...
      if (match) {
        ...
        if (error.TAG === /* Ok */0) {
          ...
          if (response.TAG === /* NonLast */0) {
            ...
            console.log(">>> " + Response$AgdaModeVscode.toString(response$1));
            runNonLast(Curry._1(handleResponse, response$1));
            return ;
          }
          var response$2 = response._1;
          var priority = response._0;
          console.log(">>* " + String(priority) + " " + Response$AgdaModeVscode.toString(response$2));
  ...
  return $$Promise.tap($$Promise.flatMap($$Promise.mapOk(connect(state), (function (connection) {
                        ...
                        var encoded = Request$AgdaModeVscode.encode(state.document, version, filepath, backend, libraryPath, highlightingMethod, request);
