...
function sendEvent(state, $$event) {
  ...
  return ViewController$AgdaModeVscode.sendEvent($$event);
}

function activate(state) {
  return Belt_Option.forEach(state.viewCache, (function (content) {
                if (content.TAG === /* Event */0) {
                  ViewController$AgdaModeVscode.sendEvent(content._0);
                  return ;
                }
                ViewController$AgdaModeVscode.sendRequest(content._0, content._1);
...
function reveal(param) {
  ViewController$AgdaModeVscode.reveal(undefined);
...
function prompt(state, header, prompt$1, callbackOnPromptSuccess) {
  ...
  ViewController$AgdaModeVscode.focus(undefined);
  var request = {
    _0: header,
    _1: prompt$1,
  ...
  return ViewController$AgdaModeVscode.sendRequest(request, callback);
}
...
function connect(state) {
  ...
  if (connection !== undefined) {
    ...
    return $$Promise.tapOk(Connection$AgdaModeVscode.make(undefined), (function (conn) {
...
function disconnect(state) {
  ...
  if (connection !== undefined) {
    ...
    return Connection$AgdaModeVscode.destroy(Caml_option.valFromOption(connection));
  } else {
    return $$Promise.resolved(undefined);
  }
}
...
function sendRequest(state, handleResponse, request) {
  return push(state.agdaRequestQueue, (function (param) {
                ...
                return $$Promise.tap($$Promise.flatMap(connect(state), (function (x) {
                                  ...
                                  var connection = x._0;
                                  var promise = Connection$AgdaModeVscode.onResponse(connection, handleResult);
                                  Connection$AgdaModeVscode.sendRequest(connection, state.document, param);
                                  return promise;
                                })), (function (param) {
...
function destroy(state, alsoRemoveFromRegistry) {
  ...
  ViewController$AgdaModeVscode.deactivate(undefined);
  return disconnect(state);
}

function make$1(chan, editor) {
  Vscode.commands.executeCommand("setContext", "agdaMode", true);
  return {
          editor: editor,
