...
function displayConnectionStatus(state, status) {
  ...
  } else if (status._0.TAG === /* ViaStdIO */0) {
    return sendEvent(state, {
                TAG: 1,
                _0: "LSP",
                [Symbol.for("name")]: "SetStatus"
...
function sendRequest(state, handleResponse, request) {
  return push(state.agdaRequestQueue, (function (request) {
                ...
                return $$Promise.flatMap(Connection$AgdaModeVscode.sendRequest(Config$AgdaModeVscode.useAgdaLanguageServer(undefined), state.devMode, state.document, request, handleResult), (function (result) {
                              if (result.TAG === /* Ok */0) {
