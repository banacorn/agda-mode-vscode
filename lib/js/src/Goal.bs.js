...
function getInnerRange(self, $$document) {
  ...
  return Common$AgdaModeVscode.Interval.toRange($$document, interval);
}

function getOuterRange(self, $$document) {
  return Common$AgdaModeVscode.Interval.toRange($$document, self.interval);
}
...
function setCursor(self, editor) {
  ...
  var position = Common$AgdaModeVscode.Offset.toPosition(editor.document, match[0] + 3 | 0);
  return Editor$AgdaModeVscode.Cursor.set(editor, position);
}
...
function buildHaskellRange(self, $$document, version, filepath) {
  ...
  var end_ = match[1];
  var start = match[0];
  var startPoint = Common$AgdaModeVscode.Offset.toPosition($$document, start);
  var endPoint = Common$AgdaModeVscode.Offset.toPosition($$document, end_);
