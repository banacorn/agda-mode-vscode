...
function isIntact(goal, $$document) {
  var documentEnd = $$document.getText(undefined).length;
  if (goal.start < 0 || goal.end > documentEnd || goal.start >= goal.end) {
    return false;
  }
  var outerRange = new Vscode.Range($$document.positionAt(goal.start), $$document.positionAt(goal.end));
  var text = Editor$AgdaModeVscode.$$Text.get($$document, outerRange);
