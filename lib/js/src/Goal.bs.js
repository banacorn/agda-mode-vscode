...
function setCursor(self, editor) {
  ...
  var position = Editor$AgdaModeVscode.Position.fromOffset(editor.document, match[0] + 3 | 0);
...
function buildHaskellRange(self, $$document, version, filepath) {
  ...
  if (Util$AgdaModeVscode.Version.gte(version, "2.5.1")) {
    return "(intervalsSrcLoc (Just (mkAbsolute \"" + filepath + "\")) [Interval (Pn () " + startPart + ") (Pn () " + endPart + ")])";
  } else {
