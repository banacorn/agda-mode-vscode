    ...
    case "Yes" :
        ...
        if (platform.TAG !== "Ok") {
          return await State__View$AgdaModeVscode.Panel.displayConnectionError(state, {
                      TAG: "TempPlatformNotSupported",
                      _0: platform._0,
                      [Symbol.for("name")]: "TempPlatformNotSupported"
                    });
        }
        var reportProgress = await Connection__Download__Util$AgdaModeVscode.Progress.report("Agda Language Server");
        var error = await Connection$AgdaModeVscode.downloadLatestALS(state.memento, state.globalStorageUri, platform._0, reportProgress);
        if (error.TAG === "Ok") {
