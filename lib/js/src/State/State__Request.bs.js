...
async function handleDownloadPolicy(state, dispatchCommand, policy) {
  switch (policy) {
...
async function onCannotFindALSorAgdaError(state, dispatchCommand) {
  var policy = Config$AgdaModeVscode.Connection.Download.getDownloadPolicy();
  return await handleDownloadPolicy(state, dispatchCommand, policy);
}

async function connectionErrorHandler(state, dispatchCommand, error) {
  if (typeof error !== "object") {
    if (error === "CannotFindALSorAgda") {
      return await onCannotFindALSorAgdaError(state, dispatchCommand);
    } else {
...
exports.onCannotFindALSorAgdaError = onCannotFindALSorAgdaError;
