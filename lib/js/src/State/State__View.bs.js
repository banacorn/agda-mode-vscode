...
function get(state) {
  return Singleton$AgdaModeVscode.Panel.get(state.extensionPath);
}

function sendEvent(state, $$event) {
  State__Type$AgdaModeVscode.ViewCache.cacheEvent(state.panelCache, $$event);
  return WebviewPanel$AgdaModeVscode.sendEvent(Singleton$AgdaModeVscode.Panel.get(state.extensionPath), $$event);
}

function restore(state) {
  return State__Type$AgdaModeVscode.ViewCache.restore(state.panelCache, Singleton$AgdaModeVscode.Panel.get(state.extensionPath));
}
...
function prompt(state, header, prompt$1, callbackOnPromptSuccess) {
  ...
  WebviewPanel$AgdaModeVscode.focus(Singleton$AgdaModeVscode.Panel.get(state.extensionPath));
  var request = {
    _0: header,
    _1: prompt$1,
  ...
  var callback = function (response) {
    if (response) {
      ...
      State__Type$AgdaModeVscode.ViewCache.restore(state.panelCache, Singleton$AgdaModeVscode.Panel.get(state.extensionPath));
      return $$Promise.resolved(undefined);
    }
  };
  State__Type$AgdaModeVscode.ViewCache.cacheRequest(state.panelCache, request, callback);
  return WebviewPanel$AgdaModeVscode.sendRequest(Singleton$AgdaModeVscode.Panel.get(state.extensionPath), request, callback);
}
