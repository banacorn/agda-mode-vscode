...
function displayConnectionStatus(state, status) {
  ...
  var error = status._2;
  var agdaVersion = status._1;
  var alsVersion = status._0;
  if (error.TAG !== "Ok") {
    return display(state, {
                TAG: "Error",
                _0: "Connection Error",
                [Symbol.for("name")]: "Error"
              }, [Item$AgdaModeVscode.plainText(error._0)]);
  }
  var match = error._0;
  if (match.TAG === "ViaPipe") {
    if (match._3.TAG === "FromGitHub") {
      return displayStatus(state, "Prebuilt Agda v" + agdaVersion + " Language Server v" + alsVersion);
    } else {
      return displayStatus(state, "Agda v" + agdaVersion + " Language Server v" + alsVersion);
    }
  } else {
