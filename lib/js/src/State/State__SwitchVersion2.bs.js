...
async function switchAgdaVersion(state) {
  ...
  if (storedPathBeforeConnection !== undefined) {
    ...
    pathsFromSystem = pathExists ? configPaths : [selectedUri].concat(configPaths);
  } else {
  ...
  if (conn.TAG === "Ok") {
    ...
    var match = await Connection__Endpoint$AgdaModeVscode.getPicked(state.memento, pathsFromSystem);
    if (match.TAG !== "Ok") {
      return ;
    }
