...
async function getItemData(self, downloadInfo) {
  ...
  var endpoint = await Connection__Endpoint$AgdaModeVscode.getPicked(self.memento, Config$AgdaModeVscode.Connection.getAgdaPaths());
  var currentPath;
  currentPath = endpoint.TAG === "Ok" ? Connection__URI$AgdaModeVscode.toString(Connection__Endpoint$AgdaModeVscode.toURI(endpoint._0)) : undefined;
...
async function switchAgdaVersion(state) {
  ...
  if (storedPathBeforeConnection !== undefined) {
    var selectedUri = Connection__URI$AgdaModeVscode.parse(storedPathBeforeConnection);
    var pathExists = configPaths.some(function (configPath) {
          return Connection__URI$AgdaModeVscode.toString(configPath) === storedPathBeforeConnection;
        });
    suppliedPaths = pathExists ? configPaths : [selectedUri].concat(configPaths);
  } else {
    suppliedPaths = configPaths;
  }
  var conn = await Connection$AgdaModeVscode.make(state.platformDeps, state.memento, state.globalStorageUri, suppliedPaths, [
        "als",
        "agda"
      ]);
  if (conn.TAG === "Ok") {
    state.connection = conn._0;
    var match = await Connection__Endpoint$AgdaModeVscode.getPicked(state.memento, suppliedPaths);
    if (match.TAG !== "Ok") {
      return ;
    }
    var match$1 = match._0;
    if (match$1.TAG === "Agda") {
      var formattedVersion = State__SwitchVersion$AgdaModeVscode.VersionDisplay.formatAgdaVersion(match$1._0);
    ...
    var formattedVersion$1 = State__SwitchVersion$AgdaModeVscode.VersionDisplay.formatALSVersion(match$1._0, match$1._1);
...
async function run(state, platformDeps) {
  ...
  if (match === undefined) {
    var endpoint = await Connection__Endpoint$AgdaModeVscode.getPicked(manager.memento, Config$AgdaModeVscode.Connection.getAgdaPaths());
    if (endpoint.TAG === "Ok") {
      var path = Connection__URI$AgdaModeVscode.toString(Connection__Endpoint$AgdaModeVscode.toURI(endpoint._0));
