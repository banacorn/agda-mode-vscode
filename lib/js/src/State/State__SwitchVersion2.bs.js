...
function entriesToItems(entries, extensionUri, pickedPath, downloadItem, param) {
...
function toItems(self, memento, downloadItem, param) {
  var pickedPath = getPickedPath(memento);
  return entriesToItems(self.entries, self.extensionUri, pickedPath, downloadItem, undefined);
}

function updateItems$1(self, qp, memento, downloadItem, param) {
  var items = toItems(self, memento, downloadItem, undefined);
...
async function run(state, platformDeps) {
  ...
  updateItems$1(endpointManager, qp, state.memento, undefined, undefined);
  qp.quickPick.show();
                    ...
                    if (item !== undefined) {
                      ...
                      if (platform.TAG === "Ok") {
                        ...
                        if (error.TAG === "Ok") {
                          ...
                          updateItems$1(endpointManager, qp, state.memento, newDownloadItem, undefined);
  ...
  var backgroundUpdate = async function () {
    try {
      ...
      if (phase2Changed) {
        updateItems$1(endpointManager, qp, state.memento, downloadItem, undefined);
      }
      ...
      if (!phase2Changed && !phase3Changed) {
        return updateItems$1(endpointManager, qp, state.memento, downloadItem, undefined);
      } else {
        return ;
      }
    }
