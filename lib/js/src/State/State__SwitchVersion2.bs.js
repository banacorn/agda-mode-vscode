...
async function switchAgdaVersion(state) {
  ...
  if (conn.TAG === "Ok") {
    ...
    if (conn$1.TAG === "Agda") {
      var formattedVersion = State__SwitchVersion$AgdaModeVscode.VersionDisplay.formatAgdaVersion(conn$1._2);
    ...
    var formattedVersion$1 = State__SwitchVersion$AgdaModeVscode.VersionDisplay.formatALSVersion(conn$1._2, conn$1._3);
...
async function getAvailableDownload(state, platformDeps) {
  ...
  var installedEndpoints = await platformDeps.getInstalledEndpointsAndPersistThem2(state.globalStorageUri);
  ...
  ((async function () {
          ...
          if (selectedItem.label === downloadLatestALS) {
            ...
            if (match !== undefined) {
              ...
              var platform = await platformDeps.determinePlatform();
              if (platform.TAG === "Ok") {
                var error = await platformDeps.downloadLatestALS2(state.memento, state.globalStorageUri)(platform._0);
                if (error.TAG === "Ok") {
          ...
          await Config$AgdaModeVscode.Connection.addAgdaPath2(path$1);
