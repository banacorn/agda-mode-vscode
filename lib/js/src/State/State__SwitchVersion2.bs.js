...
function onHide$1(state, view) {
  Chan$AgdaModeVscode.emit(state.channels.log, {
        TAG: "Others",
        _0: "SwitchVersion2: QuickPick hidden/cancelled"
      });
  destroy(view);
}
...
async function run(state, platformDeps) {
  ...
  var channelUnsubscribe = Chan$AgdaModeVscode.on(state.channels.switchVersion, (function ($$event) {
          if (typeof $$event !== "object") {
            return onHide$1(state, view);
          } else {
            return onSelection$1(state, platformDeps, manager, updateUI, view, $$event._0);
          }
        }));
  var channelDisposable = new Vscode.Disposable(channelUnsubscribe);
  view.subscriptions.push(channelDisposable);
