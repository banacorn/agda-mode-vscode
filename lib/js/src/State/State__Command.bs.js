...
async function dispatchCommand(state, command) {
  ...
  if (state.editedSinceLoad && Command$AgdaModeVscode.requiresUpToDateLoad(command)) {
    await dispatchCommand(state, "Load");
  }
  ...
  if (typeof command !== "object") {
    switch (command) {
      case "Load" :
          ...
          Tokens$AgdaModeVscode.beginLoad(state.tokens, state.document);
          ...
          return Tokens$AgdaModeVscode.endLoad(state.tokens);
      case "Quit" :
      ...
      case "Restart" :
          ...
          Goals$AgdaModeVscode.reset(state.goals);
      ...
      case "SwitchAgdaVersion" :
          return await Connection$AgdaModeVscode.activateSwitchVersion(state, state.platformDeps, undefined);
      case "Escape" :
