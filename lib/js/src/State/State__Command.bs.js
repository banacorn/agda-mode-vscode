    ...
    if (typeof command === "number") {
      switch (command) {
        ...
        case /* SwitchAgdaVersion */15 :
            var oldAgdaVersion = Config$AgdaModeVscode.getAgdaVersion(undefined);
            return State$AgdaModeVscode.View.prompt(state, header, {
                        body: undefined,
                        placeholder: undefined,
                        value: oldAgdaVersion
                      }, (function (expr) {
                          var oldAgdaPath = Config$AgdaModeVscode.getAgdaPath(undefined);
                          var newAgdaVersion = expr.trim();
                          return $$Promise.flatMap($$Promise.flatMap($$Promise.flatMap($$Promise.flatMap($$Promise.flatMap(Config$AgdaModeVscode.setAgdaPath(""), (function (param) {
                                                        return Config$AgdaModeVscode.setAgdaVersion(newAgdaVersion);
                                                      })), (function (param) {
                                            ...
                                            return Connection$AgdaModeVscode.start(false, state.devMode);
                                          })), (function (result) {
                                        if (result.TAG === /* Ok */0) {
                                          var match = result._0;
                                          if (match.TAG === /* Emacs */0) {
                                            return State$AgdaModeVscode.View.display(state, {
                                                        TAG: 1,
                                                        _0: "Switched to '" + match._0 + "'",
                                                        [Symbol.for("name")]: "Success"
                                                      }, [Component$AgdaModeVscode.Item.plainText("Found '" + newAgdaVersion + "' at: " + match._1)]);
                                          } else {
                                            return State$AgdaModeVscode.View.display(state, {
                                                        TAG: 1,
                                                        _0: "Panic, Switched to LSP server '" + match._0 + "'",
                                                        [Symbol.for("name")]: "Success"
                                                      }, [Component$AgdaModeVscode.Item.plainText("Should have switched to an Agda executable, please file an issue")]);
                                          }
                                        }
                                        ...
                                        var body = [Component$AgdaModeVscode.Item.plainText(match$1[1] + "\n\nSwitching back to " + oldAgdaPath)];
                                        return $$Promise.flatMap(Config$AgdaModeVscode.setAgdaPath(oldAgdaPath), (function (param) {
                                                      return State$AgdaModeVscode.View.display(state, header, body);
                                                    }));
                                      }));
                        }));
        case /* Escape */16 :
              ...
              case /* JumpToTarget */2 :
                  ...
                  if (link.TAG === /* SrcLoc */0) {
                    ...
                    if (removeRoot(path) === removeRoot(fileName$1)) {
                      var ranges = Belt_Array.map(match$12._1, Editor$AgdaModeVscode.$$Range.fromAgdaInterval);
