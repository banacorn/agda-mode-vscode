...
async function handle(state, output) {
  var handle$1 = async function (kind) {
    if (typeof kind !== "object") {
      ...
      var replacements = kind._0;
      var applyEditResult;
      var exit = 0;
      var val;
      try {
        val = await Editor$AgdaModeVscode.$$Text.batchReplace(state.document, replacements);
        exit = 1;
      }
      catch (raw_e){
        var e = Caml_js_exceptions.internalToOCamlException(raw_e);
        applyEditResult = e.RE_EXN_ID === Js_exn.$$Error ? ({
              TAG: "Threw",
              _0: Core__Option.getOr(e._1.message, "unknown")
            }) : ({
              TAG: "Threw",
              _0: "unknown"
            });
      ...
      if (exit === 1) {
        applyEditResult = val ? "Succeeded" : "Failed";
      }
      Chan$AgdaModeVscode.emit(state.channels.log, {
            TAG: "InputMethod",
            _0: {
              TAG: "RewriteApplication",
              _0: {
                targetDocumentFileName: state.document.fileName,
