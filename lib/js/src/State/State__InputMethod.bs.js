...
async function handle(state, output) {
  ...
  await Util$AgdaModeVscode.oneByOne(Belt_Array.map(output, handle$1));
}
...
function activate(state) {
  var intervals = Belt_Array.map(Editor$AgdaModeVscode.$$Selection.getMany(state.editor), (function (extra) {
...
async function handle$1(state, output) {
  ...
  await Util$AgdaModeVscode.oneByOne(Belt_Array.map(output, handle$2));
}
