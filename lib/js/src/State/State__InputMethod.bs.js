...
async function handle(state, output) {
  var handle$1 = async function (kind) {
    if (typeof kind !== "object") {
      ...
      var replacements = kind._0.map(function (param) {
            var range = Common$AgdaModeVscode.Interval.toVSCodeRange(state.document, param[0]);
            return [
                    range,
                    param[1]
                  ];
          });
      await Editor$AgdaModeVscode.$$Text.batchReplace(state.document, replacements);
      return kind._1();
    }
  };
  await Util$AgdaModeVscode.oneByOne(output.map(handle$1));
}
...
async function handle$1(state, output) {
  ...
  await Util$AgdaModeVscode.oneByOne(output.map(handle$2));
}
