...
async function getAvailableDevDownload(state, platformDeps) {
  ...
  var _error$1 = await platformDeps.getDownloadDescriptorOfDevALS(state.memento, state.globalStorageUri, _error._0);
  if (_error$1.TAG === "Ok") {
    var downloadDescriptor = _error$1._0;
    Util$AgdaModeVscode.log("Dev ALS download descriptor: ", downloadDescriptor);
    var match = await Connection__Download$AgdaModeVscode.alreadyDownloaded(state.globalStorageUri, [
          "dev-als",
          "als"
        ]);
    var downloaded = match !== undefined;
          ...
          ((async function () {
                  ...
                  if (devDownloadInfo !== undefined) {
                    ...
                    if (downloaded) {
                      var platform = await platformDeps.determinePlatform();
                      if (platform.TAG === "Ok") {
                        var downloadDescriptor = await platformDeps.getDownloadDescriptorOfDevALS(state.memento, state.globalStorageUri, platform._0);
                        if (downloadDescriptor.TAG === "Ok") {
                      ...
                      if (platform$1.TAG === "Ok") {
                        var error = await Connection__DevALS$AgdaModeVscode.download(state.memento, state.globalStorageUri)(platform$1._0);
                        if (error.TAG === "Ok") {
