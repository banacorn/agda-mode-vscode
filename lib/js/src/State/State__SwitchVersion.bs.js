...
async function getDownloadDescriptorWithPlatform(platformOps, target, memento, globalStorageUri) {
  var _error = await platformOps.determinePlatform();
  if (_error.TAG === "Ok") {
    return await platformOps.getDownloadDescriptor(target, true)(memento, globalStorageUri, _error._0);
  } else {
    return {
            TAG: "Error",
...
async function getAvailableLatestDownload(state, platformDeps) {
  var downloadDescriptor = await getDownloadDescriptorWithPlatform(platformDeps, "LatestALS", state.memento, state.globalStorageUri);
...
async function getAvailableDevDownload(state, platformDeps) {
  var _error = await getDownloadDescriptorWithPlatform(platformDeps, "DevALS", state.memento, state.globalStorageUri);
  if (_error.TAG !== "Ok") {
    return ;
  }
...
async function getAvailableDevWASMDownload(state, platformDeps) {
  var _error = await getDownloadDescriptorWithPlatform(platformDeps, "DevWASMALS", state.memento, state.globalStorageUri);
  if (_error.TAG !== "Ok") {
    return ;
  }
...
async function handleDownload(state, platformDeps, target, downloaded, versionString, refreshUIOpt) {
  ...
  if (downloaded) {
    var downloadDescriptor = await getDownloadDescriptorWithPlatform(platformDeps, target, state.memento, state.globalStorageUri);
    if (downloadDescriptor.TAG === "Ok") {
      var fileName;
      switch (target) {
        case "LatestALS" :
      ...
      var downloadedPath = Vscode.Uri.joinPath(state.globalStorageUri, downloadDescriptor._0.saveAsFileName, fileName).fsPath;
    ...
    var error = await getDownloadDescriptorWithPlatform(platformDeps, target, state.memento, state.globalStorageUri);
    var downloadResult;
                  ...
                  if (selectedItem !== undefined) {
                    if (selectedItem.label.startsWith("$(cloud-download)")) {
                      if (selectedItem.label.includes("WASM")) {
                        if (wasmDownloadInfo !== undefined) {
                          return await handleDownload(state, platformDeps, "DevWASMALS", wasmDownloadInfo[0], wasmDownloadInfo[1], undefined);
                        } else {
                      ...
                      } else if (devDownloadInfo !== undefined) {
                        return await handleDownload(state, platformDeps, "DevALS", devDownloadInfo[0], devDownloadInfo[1], undefined);
                      } else {
                        Vscode.window.showErrorMessage("Download not available for this platform");
                        return Chan$AgdaModeVscode.emit(state.channels.log, {
                                    TAG: "SwitchVersionUI",
