...
async function run(state) {
  ...
  var fetchSpecToItem = function (globalStoragePath, installedPaths) {
    return function (fetchSpec) {
      ...
      return {
              ...
              label: "$(squirrel)  Agda v" + agdaVersion + " Language Server v" + alsVersion
            };
    };
  };
  ...
  var platform = await Connection__Download__Platform$AgdaModeVscode.determine();
  var downloadableFetchSpecs;
  if (platform.TAG === "Ok") {
    var fetchSpec = await Connection$AgdaModeVscode.LatestALS.getFetchSpec(state.memento, state.globalStorageUri, platform._0);
    downloadableFetchSpecs = fetchSpec.TAG === "Ok" ? [fetchSpec._0] : [];
  } else {
    downloadableFetchSpecs = [];
  }
  var downloadableItems = downloadableFetchSpecs.map(fetchSpecToItem(state.globalStorageUri, installedPaths));
  var items = [
      installedSeperator,
