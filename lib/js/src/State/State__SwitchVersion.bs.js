          ...
          if (selectedItem.label === downloadLatestALS || selectedItem.label === "$(cloud-download)  Download the dev Agda Language Server") {
            ...
            if (downloadInfoResult !== undefined) {
              ...
              if (platform$2.TAG === "Ok") {
                ...
                var downloadResult;
                if (downloadType === "dev") {
                  downloadResult = await Connection__DevALS$AgdaModeVscode.download(state.globalStorageUri)(platform$3);
                } else {
                  var error = await platformDeps.getDownloadDescriptorOfLatestALS(state.memento, state.globalStorageUri, platform$3);
