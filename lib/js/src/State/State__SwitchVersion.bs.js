...
var Connection__Probe$AgdaModeVscode = require("../Connection/Connection__Probe.bs.js");
...
async function showInputBoxForSwitchingAgdaVersion(state) {
  ...
  var result = await Vscode.window.showInputBox({
        ...
        validateInput: (function (name) {
            ...
            var promise = Source$LanguageServerMule.search({
                    TAG: "FromCommand",
                    _0: name$1,
                  ...
                  var msg$1 = Source$LanguageServerMule.$$Error.toString(result._0);
                  return Caml_option.some(VSCode.StringOr.make({
                                  TAG: "String",
...
async function run(state) {
  ...
  var agdaLanguageServerRepo = Connection__Probe$AgdaModeVscode.makeAgdaLanguageServerRepo(state.globalStorageUri.fsPath);
  var match = await Source__GitHub$LanguageServerMule.getReleaseManifest(agdaLanguageServerRepo);
  var result = match[0];
  var languageServerItemsSeparator = [{
      kind: -1,
      label: "Language Servers"
    }];
  var languageServerItems;
  languageServerItems = result.TAG === "Ok" ? result._0.map(function (release) {
          var version = release.tag_name;
          var body = release.body;
          var description = body !== undefined ? body : "No description";
          var label = "Agda Language Server " + version;
          return {
