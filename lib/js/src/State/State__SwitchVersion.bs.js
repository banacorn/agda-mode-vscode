...
async function handleDevDownload(state, platformDeps, target, downloaded, versionString) {
  ...
  if (downloaded) {
    var downloadDescriptor = await getDownloadDescriptorWithPlatform(platformDeps, target, false, state.memento, state.globalStorageUri);
    if (downloadDescriptor.TAG === "Ok") {
      var fileName;
      switch (target) {
        case "LatestALS" :
    ...
    var error = await getDownloadDescriptorWithPlatform(platformDeps, target, false, state.memento, state.globalStorageUri);
    var downloadResult;
                  ...
                  if (selectedItem !== undefined) {
                    if (selectedItem.label.startsWith("$(cloud-download)")) {
                      if (selectedItem.label.includes("WASM")) {
                        if (wasmDownloadInfo !== undefined) {
                          return await handleDevDownload(state, platformDeps, "DevWASMALS", wasmDownloadInfo[0], wasmDownloadInfo[1]);
                        } else {
                        ...
                        return await handleDevDownload(state, platformDeps, "DevALS", devDownloadInfo[0], devDownloadInfo[1]);
                      } else {
                        Vscode.window.showErrorMessage("Download not available for this platform");
                        return Chan$AgdaModeVscode.emit(state.channels.log, {
                                    TAG: "SwitchVersionUI",
...
var Handler = {
  handleDevDownload: handleDevDownload,
