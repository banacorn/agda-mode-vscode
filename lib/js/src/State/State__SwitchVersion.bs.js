...
async function handleSelection(self, platformDeps, memento, globalStorageUri, selection) {
  ...
  if (typeof rawPath !== "object") {
    ...
    if (url.TAG === "FileURI") {
      var e = await Connection__Endpoint$AgdaModeVscode.fromVSCodeUri(url._0);
      if (e.TAG === "Ok") {
...
async function run(state, platformDeps) {
  ...
  var installedTargets = await platformDeps.getInstalledEndpointsAndPersistThem(state.globalStorageUri);
  ...
  var installedItemsFromSettings = Object.values(installedTargets).map(endpointToItem);
