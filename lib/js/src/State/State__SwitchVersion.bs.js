...
async function getAvailableLatestDownload(state, platformDeps) {
  ...
  var downloadDescriptor = match._0._1;
  var match$1 = await platformDeps.alreadyDownloaded(state.globalStorageUri, "LatestALS");
  ...
  var alsVersion = Core__Option.getOr(Core__Array.last(downloadDescriptor.release.name.split(".")), downloadDescriptor.release.name);
...
async function getAvailableDevDownload(state, platformDeps) {
  ...
  var agdaVersion = getAgdaVersion(_error._0._1.asset);
...
async function handleDownload(state, platformDeps, target, downloaded, versionString, refreshUIOpt) {
  ...
  if (downloaded) {
    ...
    if (match.TAG === "Ok") {
      ...
      var downloadedPath = Vscode.Uri.joinPath(state.globalStorageUri, match$1._1.saveAsFileName, fileName).fsPath;
