...
async function showOtherVersionsSubmenu(state, platformDeps) {
  ...
  if (devDownloadInfo !== undefined) {
    ...
    submenuItems = [createQuickPickItem("$(cloud-download)  Download dev ALS", description, devDownloadInfo[1])];
  } else {
    submenuItems = [createQuickPickItem("$(info)  No dev downloads available", "Dev downloads not supported on this platform", undefined)];
  }
          ...
          ((async function () {
                  ...
                  if (devDownloadInfo !== undefined) {
                    ...
                    if (downloaded) {
                      var platform = await platformDeps.determinePlatform();
                      if (platform.TAG === "Ok") {
                        var downloadDescriptor = await platformDeps.getDownloadDescriptorOfDevALS(state.globalStorageUri, platform._0);
                        if (downloadDescriptor.TAG === "Ok") {
                      ...
                      var platform$1 = await platformDeps.determinePlatform();
                      if (platform$1.TAG === "Ok") {
                        var error = await Connection__DevALS$AgdaModeVscode.download(state.globalStorageUri)(platform$1._0);
                        if (error.TAG === "Ok") {
                          await Config$AgdaModeVscode.Connection.addAgdaPath(state.channels.log, error._0);
                        ...
                        Vscode.window.showErrorMessage("Failed to determine the platform for downloading");
                      }
                    }
                    return Chan$AgdaModeVscode.emit(state.channels.log, {
                                TAG: "SwitchVersionUI",
          ...
          if (selectedItem.label === downloadLatestALS) {
            ...
            if (downloadInfoResult !== undefined) {
              ...
              if (downloaded) {
                var platform = await platformDeps.determinePlatform();
                if (platform.TAG === "Ok") {
                  var downloadDescriptorResult = await platformDeps.getDownloadDescriptorOfLatestALS(state.memento, state.globalStorageUri, platform._0);
              ...
              var platform$1 = await platformDeps.determinePlatform();
              if (platform$1.TAG === "Ok") {
                var error = await platformDeps.getDownloadDescriptorOfLatestALS(state.memento, state.globalStorageUri, platform$1._0);
                var downloadResult;
