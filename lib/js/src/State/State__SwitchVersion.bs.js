...
var downloadDevALS = "$(cloud-download)  Download the dev Agda Language Server";
  ...
  if (typeof itemData !== "object") {
    ...
    switch (itemData.TAG) {
      case "Endpoint" :
          ...
          break;
      case "DownloadAction" :
          var label = itemData._2 === "dev" ? downloadDevALS : downloadLatestALS;
...
async function getAvailableLatestDownload(state, platformDeps) {
  var concreteOrder = await resolveDownloadChannelWithPlatform(platformDeps, "LatestALS", state.memento, state.globalStorageUri);
  if (concreteOrder.TAG !== "Ok") {
    return ;
  }
  var match = await platformDeps.alreadyDownloaded(state.globalStorageUri, "LatestALS");
  var downloaded = match !== undefined;
  var versionString = Connection__Download$AgdaModeVscode.Source.toVersionString(concreteOrder._0);
  return [
          downloaded,
          versionString,
          "latest"
...
async function getAvailableDevDownload(state, platformDeps) {
  var _error = await resolveDownloadChannelWithPlatform(platformDeps, "DevALS", state.memento, state.globalStorageUri);
  if (_error.TAG !== "Ok") {
    return ;
  }
  var match = await platformDeps.alreadyDownloaded(state.globalStorageUri, "DevALS");
  var downloaded = match !== undefined;
  var versionString = Connection__Download$AgdaModeVscode.Source.toVersionString(_error._0);
  return [
          downloaded,
          versionString,
          "dev"
...
async function getAllAvailableDownloads(state, platformDeps) {
  ...
  if (exit === 1) {
    var item = await getAvailableLatestDownload(state, platformDeps);
    latestItems = item !== undefined ? [item] : [[
          false,
...
async function handleDownload(state, platformDeps, target, downloaded, versionString, refreshUIOpt) {
  ...
  if (downloaded) {
    ...
    var match = await resolveDownloadChannelWithPlatform(platformDeps, target, state.memento, state.globalStorageUri);
    if (match.TAG === "Ok") {
      ...
      if (match$1.TAG === "FromGitHub") {
        ...
        switch (target) {
          case "DevALS" :
              isWasm = platformResult.TAG === "Ok" && platformResult._0 === "Web" ? true : false;
              break;
          case "LatestALS" :
          case "Hardcoded" :
              isWasm = false;
        ...
        var fileName = isWasm ? "als.wasm" : "als";
        var downloadedUri = Vscode.Uri.joinPath(state.globalStorageUri, match$1._1.saveAsFileName, fileName);
        var downloadedPath = isWasm ? downloadedUri.toString() : downloadedUri.fsPath;
        ...
        var isWasm$1 = match$1._1.endsWith(".wasm");
        var fileName$1 = isWasm$1 ? "als.wasm" : "als";
        var downloadedUri$1 = Vscode.Uri.joinPath(state.globalStorageUri, match$1._2, fileName$1);
        var downloadedPath$1 = isWasm$1 ? downloadedUri$1.toString() : downloadedUri$1.fsPath;
    ...
    var platformResult$1 = await platformDeps.determinePlatform();
    var downloadResult;
    var exit = 0;
    switch (target) {
      case "DevALS" :
          if (platformResult$1.TAG === "Ok" && platformResult$1._0 === "Web") {
            var error = await Connection__Download$AgdaModeVscode.downloadFromURL(state.globalStorageUri, "https://unpkg.com/agda-wasm@0.0.3-als.2.8.0/als/2.8.0/als.wasm", "dev-als", "Agda Language Server (WASM)");
    ...
    if (exit === 1) {
      var error$1 = await resolveDownloadChannelWithPlatform(platformDeps, target, state.memento, state.globalStorageUri);
      downloadResult = error$1.TAG === "Ok" ? await platformDeps.download(state.globalStorageUri, error$1._0) : ({
            TAG: "Error",
            _0: error$1._0
          });
    }
    if (downloadResult.TAG === "Ok") {
      var downloadedPath$2 = downloadResult._0;
      await Config$AgdaModeVscode.Connection.addAgdaPath(state.channels.log, downloadedPath$2);
      await Memento$AgdaModeVscode.Module.PickedConnection.set(state.memento, downloadedPath$2);
...
function onSelection$1(state, platformDeps, manager, updateUI, view, selectedItems) {
  ...
  ((async function () {
          ...
          if (selectedItem.label === downloadDevALS) {
            var downloadInfoResult$1 = await getAvailableDevDownload(state, platformDeps);
            if (downloadInfoResult$1 !== undefined) {
              return await handleDownload(state, platformDeps, "DevALS", downloadInfoResult$1[0], downloadInfoResult$1[1], Caml_option.some((async function () {
                                var newDownloadItems = await getAllAvailableDownloads(state, platformDeps);
                                refreshFromMemento(manager);
                                return await updateUI(newDownloadItems);
                              })));
