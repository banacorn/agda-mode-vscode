...
function entriesToItemData(entries, pickedPath, downloadItems, showChannelSelectorOpt) {
...
async function getItemData(self, downloadItems, showChannelSelectorOpt) {
  var showChannelSelector = showChannelSelectorOpt !== undefined ? showChannelSelectorOpt : false;
  ...
  return entriesToItemData(self.entries, pickedPath, downloadItems, showChannelSelector);
}
...
async function onActivate(state, platformDeps) {
  ...
  var updateUI = async function (downloadItems) {
    var itemData = await getItemData(manager, downloadItems, availableChannels.contents.length >= 2);
