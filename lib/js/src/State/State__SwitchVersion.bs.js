...
async function downloadLatestALS(memento, globalStorageUri) {
  ...
  var target = await Connection__LatestALS$AgdaModeVscode.alreadyDownloaded(globalStorageUri)();
  if (target !== undefined) {
    return {
            TAG: "Success",
            _0: target,
  ...
  var error = await Connection__LatestALS$AgdaModeVscode.download(memento, globalStorageUri)(platform._0);
  if (error.TAG === "Ok") {
    return {
            TAG: "Success",
            _0: error._0,
...
async function handleDownloadResult(result, rerender) {
  if (result.TAG === "Success") {
    var target = result._0;
    var version;
    version = target.TAG === "Agda" ? "Agda v" + target._0 : formatALSVersion(target._0, target._1);
    if (result._1) {
...
function createAgdaItem(version, path, isSelected, extensionPath) {
  return {
...
function createErrorItem(error) {
  return {
          description: "",
          detail: Connection__Target$AgdaModeVscode.$$Error.toString(error),
...
async function switchAgdaVersion(state) {
  ...
  var conn = await Connection$AgdaModeVscode.make(platformDeps, state.memento, state.globalStorageUri, Config$AgdaModeVscode.Connection.getAgdaPaths(), [
        "als",
        "agda"
      ]);
  if (conn.TAG === "Ok") {
...
async function handleSelection(self, memento, globalStorageUri, selection) {
  ...
  if (typeof rawPath !== "object") {
    ...
    var result = await downloadLatestALS(memento, globalStorageUri);
    ...
    var selectionChanged = rawPath$1 !== Connection__URI$AgdaModeVscode.toString(Connection__Target$AgdaModeVscode.toURI(original._0));
    if (!selectionChanged) {
      return ;
    }
    var url = Connection__URI$AgdaModeVscode.parse(rawPath$1);
    if (url.TAG === "Filepath") {
...
async function run(state) {
  ...
  var targetToItem = function (target) {
    if (target.TAG !== "Ok") {
      return createErrorItem(target._0);
    }
    var match = target._0;
    if (match.TAG === "Agda") {
      return createAgdaItem(match._0, match._1, isSelected(target), state.extensionPath);
    } else {
      return createALSItem(match._0, match._1, match._2, isSelected(target));
    }
  };
  ...
  var platformDeps = Platform$AgdaModeVscode.makeDesktop();
  var installedTargets = await platformDeps.getInstalledTargetsAndPersistThem(state.globalStorageUri);
