...
async function getAvailableLatestDownload(state, platformDeps) {
  ...
  var fetchSpec = await platformDeps.getDownloadDescriptorOfLatestALS(state.memento, state.globalStorageUri, platform._0);
  if (fetchSpec.TAG !== "Ok") {
    return ;
  }
  var fetchSpec$1 = fetchSpec._0;
  var filepath = Vscode.Uri.joinPath(state.globalStorageUri, fetchSpec$1.saveAsFileName, "als");
...
async function getAvailableDevDownload(state, platformDeps) {
  ...
  var _error$1 = await Connection__DevALS$AgdaModeVscode.getDownloadDescriptor(state.globalStorageUri, _error._0);
  if (_error$1.TAG !== "Ok") {
    return ;
  }
  var fetchSpec = _error$1._0;
  ...
  var alsVersion = fetchSpec.release.name;
  var versionString = formatALSVersion(alsVersion, agdaVersion);
  return [
          downloaded,
          versionString,
            ...
            if (downloadInfoResult !== undefined) {
              ...
              if (downloaded) {
                ...
                if (platform.TAG === "Ok") {
                  ...
                  var fetchSpecResult = downloadType === "dev" ? await Connection__DevALS$AgdaModeVscode.getDownloadDescriptor(state.globalStorageUri, platform$1) : await platformDeps.getDownloadDescriptorOfLatestALS(state.memento, state.globalStorageUri, platform$1);
                  if (fetchSpecResult.TAG === "Ok") {
                    var downloadedPath = Vscode.Uri.joinPath(state.globalStorageUri, fetchSpecResult._0.saveAsFileName, "als").fsPath;
