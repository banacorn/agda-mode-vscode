...
async function handleDownload(state, platformDeps, target, downloaded, versionString, refreshUIOpt) {
  ...
  if (downloaded) {
    var downloadDescriptor = await getDownloadDescriptorWithPlatform(platformDeps, target, useCache, state.memento, state.globalStorageUri);
    if (downloadDescriptor.TAG === "Ok") {
      var fileName;
      switch (target) {
        case "LatestALS" :
    ...
    var error = await getDownloadDescriptorWithPlatform(platformDeps, target, useCache, state.memento, state.globalStorageUri);
    var downloadResult;
