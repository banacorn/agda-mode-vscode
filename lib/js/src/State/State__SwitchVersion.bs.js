...
function entriesToItemData(entries, pickedPath, downloadItems, folderPath) {
  ...
  if (downloadItems.length > 0) {
    var downloadSection = downloadItems.map(function (param) {
          return {
                  TAG: "DownloadAction",
                  _0: param[0],
                  _1: param[1],
...
async function getAvailableLatestDownload(state, platformDeps) {
  ...
  var downloadDescriptor = await platformDeps.getDownloadDescriptorOfLatestALS(state.memento, state.globalStorageUri, platform._0);
  ...
  var match = await platformDeps.alreadyDownloaded(filepath)();
...
async function getAvailableDevDownload(state, platformDeps) {
  var _error = await platformDeps.determinePlatform();
  if (_error.TAG !== "Ok") {
    return ;
  }
  var _error$1 = await platformDeps.getDownloadDescriptorOfDevALS(state.globalStorageUri, _error._0);
  if (_error$1.TAG === "Ok") {
    var downloadDescriptor = _error$1._0;
    Util$AgdaModeVscode.log("Dev ALS download descriptor: ", downloadDescriptor);
    var match = await Connection__DevALS$AgdaModeVscode.alreadyDownloaded(state.globalStorageUri)();
    var downloaded = match !== undefined;
    var getAgdaVersion = function (asset) {
  ...
  Util$AgdaModeVscode.log("PlatformOps.getDownloadDescriptorOfDevALS error: ", _error$1._0);
}
...
async function getAllAvailableDownloads(state, platformDeps) {
  ...
  var devPromise = getAvailableDevDownload(state, platformDeps);
...
function onSelection$1(state, platformDeps, manager, updateUI, view, selectedItems) {
  ...
  ((async function () {
          ...
          if (selectedItem.label === downloadLatestALS || selectedItem.label === "$(cloud-download)  Download the dev Agda Language Server") {
            var isDevDownload = selectedItem.label === "$(cloud-download)  Download the dev Agda Language Server";
            var downloadInfoResult = isDevDownload ? await getAvailableDevDownload(state, platformDeps) : await getAvailableLatestDownload(state, platformDeps);
            if (downloadInfoResult !== undefined) {
              var downloadType = downloadInfoResult[2];
              ...
              if (downloaded) {
                ...
                if (platform.TAG === "Ok") {
                  ...
                  var downloadDescriptorResult = downloadType === "dev" ? await platformDeps.getDownloadDescriptorOfDevALS(state.globalStorageUri, platform$1) : await platformDeps.getDownloadDescriptorOfLatestALS(state.memento, state.globalStorageUri, platform$1);
                  if (downloadDescriptorResult.TAG === "Ok") {
                    var downloadedPath = Vscode.Uri.joinPath(state.globalStorageUri, downloadDescriptorResult._0.saveAsFileName, "als").fsPath;
              ...
              var platform$2 = await platformDeps.determinePlatform();
              if (platform$2.TAG === "Ok") {
                var platform$3 = platform$2._0;
                var downloadResult = downloadType === "dev" ? await Connection__DevALS$AgdaModeVscode.download(state.globalStorageUri)(platform$3) : await platformDeps.downloadLatestALS(state.channels.log, state.memento, state.globalStorageUri)(platform$3);
