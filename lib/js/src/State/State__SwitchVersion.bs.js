...
function formatDevALSVersion(agdaVersion) {
...
var VersionDisplay = {
  ...
  formatDevALSVersion: formatDevALSVersion,
...
async function getAvailableDevDownload(state, platformDeps) {
  ...
  var agdaVersion = getAgdaVersion(_error._0.asset);
  var versionString = formatDevALSVersion(agdaVersion);
  return [
          downloaded,
          versionString,
          "dev"
...
async function getAvailableDevWASMDownload(state, platformDeps) {
  var _error = await getDownloadDescriptorWithPlatform(platformDeps, "DevWASMALS", false, state.memento, state.globalStorageUri);
  if (_error.TAG !== "Ok") {
    return ;
  }
...
async function getAllAvailableDownloads(state, platformDeps) {
  ...
  var latestResult = match[0];
  var devResult = match[1];
  var latestItem = latestResult !== undefined ? latestResult : [
      false,
      "Not available for this platform",
      "latest"
    ];
  var devItem = Config$AgdaModeVscode.DevMode.get() ? (
      devResult !== undefined ? devResult : [
          false,
          "Not available for this platform",
          "dev"
        ]
...
async function showOtherVersionsSubmenu(state, platformDeps) {
  ...
  var wasmDownloadInfo = await getAvailableDevWASMDownload(state, platformDeps);
  var devItems;
  ...
  var wasmItems;
  if (wasmDownloadInfo !== undefined) {
    var description$1 = wasmDownloadInfo[0] ? downloadedAndInstalled : "";
    wasmItems = [createQuickPickItem("$(cloud-download)  Download WASM build", description$1, wasmDownloadInfo[1])];
  } else {
    wasmItems = [];
  }
  ...
  onSelection(submenuView, (function (selectedItems) {
          ...
          ((async function () {
                  ...
                  if (selectedItem.label.includes("WASM")) {
                    if (wasmDownloadInfo !== undefined) {
                      var versionString = wasmDownloadInfo[1];
                      var downloaded = wasmDownloadInfo[0];
                      ...
                      if (downloaded) {
                        var downloadDescriptor = await getDownloadDescriptorWithPlatform(platformDeps, "DevWASMALS", false, state.memento, state.globalStorageUri);
                        if (downloadDescriptor.TAG === "Ok") {
                          var downloadedPath = Vscode.Uri.joinPath(state.globalStorageUri, downloadDescriptor._0.saveAsFileName, "als.wasm").fsPath;
                        ...
                        var error = await getDownloadDescriptorWithPlatform(platformDeps, "DevWASMALS", false, state.memento, state.globalStorageUri);
                        var downloadResult;
                  ...
                  if (devDownloadInfo !== undefined) {
                    var versionString$1 = devDownloadInfo[1];
                    var downloaded$1 = devDownloadInfo[0];
                    Chan$AgdaModeVscode.emit(state.channels.log, {
                          TAG: "SwitchVersionUI",
                          _0: {
                            TAG: "SelectedDownloadAction",
                    ...
                    if (downloaded$1) {
                      var downloadDescriptor$1 = await getDownloadDescriptorWithPlatform(platformDeps, "DevALS", false, state.memento, state.globalStorageUri);
                      if (downloadDescriptor$1.TAG === "Ok") {
                        var downloadedPath$1 = Vscode.Uri.joinPath(state.globalStorageUri, downloadDescriptor$1._0.saveAsFileName, "als").fsPath;
                        await Config$AgdaModeVscode.Connection.addAgdaPath(state.channels.log, downloadedPath$1);
                      }
                      Vscode.window.showInformationMessage(versionString$1 + " is already downloaded");
                    } else {
                      var error$1 = await getDownloadDescriptorWithPlatform(platformDeps, "DevALS", false, state.memento, state.globalStorageUri);
                      var downloadResult$1;
                      downloadResult$1 = error$1.TAG === "Ok" ? await platformDeps.download(state.globalStorageUri, error$1._0) : ({
                            TAG: "Error",
                            _0: error$1._0
                          });
                      if (downloadResult$1.TAG === "Ok") {
                        await Config$AgdaModeVscode.Connection.addAgdaPath(state.channels.log, downloadResult$1._0);
                        Vscode.window.showInformationMessage(versionString$1 + " successfully downloaded");
                      } else {
                        Vscode.window.showErrorMessage(Connection__Download$AgdaModeVscode.$$Error.toString(downloadResult$1._0));
                      }
                    }
                    return Chan$AgdaModeVscode.emit(state.channels.log, {
                                TAG: "SwitchVersionUI",
