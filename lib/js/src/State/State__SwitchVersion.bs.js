...
async function getAvailableDevDownload(state, platformDeps) {
  ...
  var fetchSpec = await Connection__DevALS$AgdaModeVscode.getFetchSpec(state.memento, state.globalStorageUri, platform._0);
  if (fetchSpec.TAG !== "Ok") {
    return ;
  }
  var fetchSpec$1 = fetchSpec._0;
  ...
  var agdaVersion = getAgdaVersion(fetchSpec$1.asset);
  var alsVersion = fetchSpec$1.release.name;
  var versionString = formatALSVersion(alsVersion, agdaVersion);
  return [
          downloaded,
          versionString,
            ...
            if (downloadInfoResult !== undefined) {
              ...
              if (downloaded) {
                ...
                if (platform.TAG === "Ok") {
                  ...
                  var fetchSpecResult = downloadType === "dev" ? await Connection__DevALS$AgdaModeVscode.getFetchSpec(state.memento, state.globalStorageUri, platform$1) : await platformDeps.getFetchSpec(state.memento, state.globalStorageUri, platform$1);
              ...
              if (platform$2.TAG === "Ok") {
                ...
                var downloadResult = downloadType === "dev" ? await Connection__DevALS$AgdaModeVscode.download(state.channels.log, state.memento, state.globalStorageUri)(platform$3) : await platformDeps.downloadLatestALS(state.channels.log, state.memento, state.globalStorageUri)(platform$3);
