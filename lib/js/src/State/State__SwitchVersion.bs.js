...
async function getAvailableLatestDownload(state, platformDeps) {
  ...
  var downloadDescriptor = await platformDeps.getDownloadDescriptor("LatestALS", true)(state.memento, state.globalStorageUri, platform._0);
...
async function getAvailableDevDownload(state, platformDeps) {
  ...
  var _error$1 = await platformDeps.getDownloadDescriptor("DevALS", false)(state.memento, state.globalStorageUri, _error._0);
  if (_error$1.TAG !== "Ok") {
    return ;
  }
          ...
          ((async function () {
                  ...
                  if (devDownloadInfo !== undefined) {
                    ...
                    if (downloaded) {
                      var platform = await platformDeps.determinePlatform();
                      if (platform.TAG === "Ok") {
                        var downloadDescriptor = await platformDeps.getDownloadDescriptor("DevALS", false)(state.memento, state.globalStorageUri, platform._0);
                        if (downloadDescriptor.TAG === "Ok") {
          ...
          if (selectedItem.label === downloadLatestALS) {
            ...
            if (downloadInfoResult !== undefined) {
              ...
              if (downloaded) {
                var platform = await platformDeps.determinePlatform();
                if (platform.TAG === "Ok") {
                  var downloadDescriptorResult = await platformDeps.getDownloadDescriptor("LatestALS", true)(state.memento, state.globalStorageUri, platform._0);
              ...
              var platform$1 = await platformDeps.determinePlatform();
              if (platform$1.TAG === "Ok") {
                var error = await platformDeps.getDownloadDescriptor("LatestALS", true)(state.memento, state.globalStorageUri, platform$1._0);
                var downloadResult;
