...
var Connection__Process__Exec$AgdaModeVscode = require("../Connection/Shared/Connection__Process__Exec.bs.js");
var Connection__Endpoint__Error$AgdaModeVscode = require("../Connection/Endpoint/Connection__Endpoint__Error.bs.js");
...
async function probeFilepath(uri) {
  ...
  var assetPath = await Connection$AgdaModeVscode.checkForPrebuiltDataDirectory(path);
  var lspOptions;
...
async function probeVersions(self) {
  ...
  var probePromises = pathsToProbe.map(async function (path) {
        ...
        var endpoing = await probeFilepath(uri);
        if (endpoing.TAG === "Ok") {
          await Memento$AgdaModeVscode.Module.Endpoints.setVersion(self.memento, path, endpoing._0);
          return path;
        }
        await Memento$AgdaModeVscode.Module.Endpoints.setError(self.memento, path, Connection__Endpoint__Error$AgdaModeVscode.toString(endpoing._0));
        return path;
      });
...
async function switchAgdaVersion(state) {
  ...
  if (conn.TAG === "Ok") {
    ...
    var formattedVersion$1 = formatALSVersion(conn$1._2, conn$1._3);
...
function onSelection$1(state, platformDeps, manager, updateUI, view, selectedItems) {
  ...
  ((async function () {
          ...
          await Config$AgdaModeVscode.Connection.addAgdaPath(path$1);
