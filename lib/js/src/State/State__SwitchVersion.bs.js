...
async function getAvailableLatestDownload(state, platformDeps) {
  var concreteOrder = await resolveDownloadOrderWithPlatform(platformDeps, "LatestALS", state.memento, state.globalStorageUri);
  ...
  var versionString = Connection__Download$AgdaModeVscode.DownloadOrderConcrete.toVersionString(concreteOrder._0);
  return [
          downloaded,
          versionString,
          "latest"
...
async function handleDownload(state, platformDeps, target, downloaded, versionString, refreshUIOpt) {
  ...
  if (downloaded) {
    ...
    if (exit === 1) {
      var error$1 = await resolveDownloadOrderWithPlatform(platformDeps, target, state.memento, state.globalStorageUri);
