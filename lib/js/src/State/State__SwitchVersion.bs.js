...
async function switchAgdaVersion(state) {
  ...
  var conn = await Connection$AgdaModeVscode.makeWithFallback(state.platformDeps, state.memento, state.globalStorageUri, pathsFromSystem, [
        "als",
        "agda"
      ], state.channels.log);
  if (conn.TAG === "Ok") {
  ...
  ((async function () {
          ...
          if (selectedItem.label === downloadLatestALS) {
            ...
            if (match !== undefined) {
              ...
              if (downloaded) {
                var match$1 = await getAvailableDownload(state, platformDeps);
                if (match$1 !== undefined) {
                  var platform = await platformDeps.determinePlatform();
                  if (platform.TAG === "Ok") {
                    var fetchSpec = await platformDeps.getFetchSpec(state.memento, state.globalStorageUri, platform._0);
                    if (fetchSpec.TAG === "Ok") {
                      var downloadedPath = Vscode.Uri.joinPath(state.globalStorageUri, fetchSpec._0.saveAsFileName, "als").fsPath;
              ...
              if (platform$1.TAG === "Ok") {
                var error = await platformDeps.downloadLatestALS(state.channels.log, state.memento, state.globalStorageUri)(platform$1._0);
                if (error.TAG === "Ok") {
          ...
          var raw = uri._0;
          await Memento$AgdaModeVscode.Module.PickedConnection.set(state.memento, raw);
          await Config$AgdaModeVscode.Connection.addAgdaPath(state.channels.log, raw);
          await switchAgdaVersion(state);
          return Chan$AgdaModeVscode.emit(state.channels.log, {
                      TAG: "SwitchVersionUI",
                      _0: "SelectionCompleted"
