...
async function download(memento, globalStoragePath, target) {
  ...
  var e = await Connection__Resolver__GitHub$AgdaModeVscode.download(target, memento, globalStoragePath, onDownload);
  if (e.TAG === "Ok") {
...
async function handleSelection(self, memento, latestALS, globalStoragePath, selection) {
  ...
  switch (match) {
    ...
    case "$(sync)  Check for updates" :
        ...
        await Connection__Resolver__GitHub$AgdaModeVscode.ReleaseManifest.fetchFromGitHubAndCache(repo);
        return await self.rerender();
    default:
      ...
      if (original !== undefined) {
        ...
        if (match$1 !== undefined) {
          if (match$1.TAG === "Filepath") {
            var e = await Connection__Target$AgdaModeVscode.fromRawPath(match$1._0);
            if (e.TAG === "Ok") {
...
async function run(state) {
  ...
  if (latestALSAlreadyDownloaded) {
    var ageInSecs = Connection__Resolver__GitHub$AgdaModeVscode.ReleaseManifest.cacheAgeInSecs(state.memento);
