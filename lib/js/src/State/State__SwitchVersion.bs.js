...
function entriesToItemData(entries, pickedPath, downloadItems, downloadHeaderOpt, showChannelSelectorOpt) {
  ...
  return sectionsWithChannels.concat([
              {
                TAG: "Separator",
                _0: "Misc"
              },
...
async function removeDownloadedPathsFromConfig(state) {
  var previousPaths = Config$AgdaModeVscode.Connection.getAgdaPaths();
  var keptPaths = previousPaths.filter(function (path) {
        return !isPathUnderDownloadDirectory(state.globalStorageUri, path);
      });
  if (keptPaths.length !== previousPaths.length) {
    return await Config$AgdaModeVscode.Connection.setAgdaPaths(state.channels.log, keptPaths);
  }
  ...
  ((async function () {
          ...
          if (selectedItem !== undefined) {
            ...
            if (selectedItem.label === deleteDownloads) {
              ...
              var path = Memento$AgdaModeVscode.Module.PickedConnection.get(state.memento);
              if (path !== undefined && isPathUnderDownloadDirectory(state.globalStorageUri, path)) {
            ...
            destroy(view);
            var path$1 = Memento$AgdaModeVscode.Module.PickedConnection.get(manager.memento);
            var changed = path$1 !== undefined ? selectedPath !== path$1 : true;
            if (!changed) {
              return Chan$AgdaModeVscode.emit(state.channels.log, {
                          TAG: "SwitchVersionUI",
                          _0: "SelectionCompleted"
...
var Handler = {
  ...
  removeDownloadedPathsFromConfig: removeDownloadedPathsFromConfig,
