...
async function handleDownload(state, platformDeps, target, downloaded, versionString, refreshUIOpt) {
  ...
  if (downloaded) {
    ...
    if (match.TAG === "Ok") {
      var isWasm;
      isWasm = target === "LatestALS" || !(platformResult.TAG === "Ok" && platformResult._0 === "Web") ? false : true;
      var fileName = isWasm ? "als.wasm" : "als";
      var downloadedUri = Vscode.Uri.joinPath(state.globalStorageUri, match._0._1.saveAsFileName, fileName);
    ...
    var downloadResult;
    var exit = 0;
    if (target === "LatestALS" || !(platformResult$1.TAG === "Ok" && platformResult$1._0 === "Web")) {
      exit = 1;
    } else {
