...
async function handleSelection(self, selection) {
  ...
  if (original !== undefined) {
    ...
    if (match$1 !== undefined) {
      ...
      console.log("URL is not supported: " + match$1._0.toString());
      return ;
    }
...
async function run(state) {
  ...
  var isPicked = function (target) {
    if (picked !== undefined && target.TAG === "Ok") {
      return Connection__Target$AgdaModeVscode.toURI(picked) === Connection__Target$AgdaModeVscode.toURI(target._0);
    } else {
      return false;
    }
  };
