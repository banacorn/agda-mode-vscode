...
async function probeVersions(self) {
  ...
  var probePromises = pathsToProbe.map(async function (path) {
        ...
        var error = await Connection$AgdaModeVscode.probeFilepath(uri);
        if (error.TAG === "Ok") {
          var agdaVersion = error._0;
        ...
        await Memento$AgdaModeVscode.Module.Endpoints.setError(self.memento, path, Connection__Endpoint__Error$AgdaModeVscode.toString(error._0));
        return path;
      });
