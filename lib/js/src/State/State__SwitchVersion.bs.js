...
async function getAvailableLatestDownload(state, platformDeps) {
  ...
  var fetchSpec = await platformDeps.getFetchSpec(state.memento, state.globalStorageUri, platform._0);
  if (fetchSpec.TAG !== "Ok") {
    return ;
  }
...
async function getAvailableDevDownload(state, platformDeps) {
  ...
  var _error$1 = await Connection__DevALS$AgdaModeVscode.getFetchSpec(state.globalStorageUri, _error._0);
  if (_error$1.TAG !== "Ok") {
    return ;
  }
            ...
            if (downloadInfoResult !== undefined) {
              ...
              if (downloaded) {
                ...
                if (platform.TAG === "Ok") {
                  ...
                  var fetchSpecResult = downloadType === "dev" ? await Connection__DevALS$AgdaModeVscode.getFetchSpec(state.globalStorageUri, platform$1) : await platformDeps.getFetchSpec(state.memento, state.globalStorageUri, platform$1);
