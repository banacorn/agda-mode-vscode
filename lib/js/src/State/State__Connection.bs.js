...
async function sendRequest(state, handleResponse, request) {
  var sendRequestAndHandleResponses = async function (connection, state, request, handler) {
    ...
    if (connection.TAG === "Agda") {
      ...
      state.agdaVersion = connection._3;
    }
  };
  ...
  var error = await Connection$AgdaModeVscode.make(state.platformDeps, state.memento, state.globalStorageUri, Config$AgdaModeVscode.Connection.getAgdaPaths2(), [
        "als",
        "agda"
      ], state.channels.log);
  if (error.TAG !== "Ok") {
