...
async function sendRequest(state, handleResponse, request) {
  var sendRequestAndHandleResponses = async function (connection, state, request, handler) {
    var composedHandler = Core__Array.reduceRight(state.middlewares, handler, (function (h, mw) {
            return mw(h);
          }));
    var onResponse = async function (response) {
      await Registry__Connection$AgdaModeVscode.withOwnerContext(state.id, (function () {
              return composedHandler(response);
            }));
    ...
    switch (connection.TAG) {
      case "Agda" :
          ...
          return ;
      case "ALS" :
          state.agdaVersion = connection._2.agdaVersion;
          return ;
      case "ALSWASM" :
          state.agdaVersion = connection._3.agdaVersion;
          return ;
  ...
  var make = function () {
    ...
    return Connection$AgdaModeVscode.makeWithFallback(state.platformDeps, state.memento, state.globalStorageUri, Config$AgdaModeVscode.Connection.getAgdaPaths(), state.channels.log);
  };
