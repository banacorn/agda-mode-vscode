    ...
    if (typeof response !== "object") {
      ...
      switch (response.TAG) {
        ...
        case "InteractionPoints" :
            ...
            return await Goals$AgdaModeVscode.instantiateGoalsFromLoad(state.goals2, state.editor, response._0, holePositions);
        case "GiveAction" :
            ...
            if (goal !== undefined) {
              if (typeof give !== "object") {
                ...
                var match = Goals$AgdaModeVscode.indentationWidth(goal, state.document);
            ...
            if (typeof give !== "object") {
              if (give !== "GiveParen") {
                return await State__Goal$AgdaModeVscode.removeBoundaryAndDestroy(state, goal$1);
              }
              await State__Goal$AgdaModeVscode.modify(state, goal$1, (function (content) {
                      return "(" + (content + ")");
                    }));
              return await State__Goal$AgdaModeVscode.removeBoundaryAndDestroy(state, goal$1);
            } else {
              var match$1 = State__Goal$AgdaModeVscode.indentationWidth(state.document, goal$1);
              var indented$1 = indent(Parser$AgdaModeVscode.unescapeEOL(give._0), 2 + match$1[0] | 0);
              await State__Goal$AgdaModeVscode.modify(state, goal$1, (function (param) {
                      return indented$1;
                    }));
              var holeOffsetsFromRefine = State__Goal$AgdaModeVscode.parseHolesFromRefineResult(indented$1);
              console.log("holeOffsetsFromRefine: ", holeOffsetsFromRefine);
              return await State__Goal$AgdaModeVscode.removeBoundaryAndDestroy(state, goal$1);
            }
        case "MakeCase" :
            ...
            if (response._0 === "Function") {
              await State__Goal$AgdaModeVscode.replaceWithLines(state, goal$2, lines);
            } else {
              await State__Goal$AgdaModeVscode.replaceWithLambda(state, goal$2, lines);
            }
