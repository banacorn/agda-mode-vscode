...
function removeTrailingNewline(string) {
  if (string.endsWith("\\n")) {
    return string.slice(0, string.length - 2 | 0);
  } else {
    return string;
  }
}
...
function handle$1(state, dispatchCommand, response) {
  if (typeof response === "number") {
    ...
    switch (response.TAG | 0) {
      ...
      case /* RunningInfo */10 :
          if (response._0 !== 1) {
            return State__View$AgdaModeVscode.displayInAppendMode(state, {
                        TAG: 0,
                        _0: "Debug",
                        [Symbol.for("name")]: "Plain"
                      }, [Item$AgdaModeVscode.plainText(removeTrailingNewline(response._1))]);
          } else {
            return State__View$AgdaModeVscode.displayInAppendMode(state, {
                        TAG: 0,
                        _0: "Type-checking",
