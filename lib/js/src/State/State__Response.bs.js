    ...
    if (typeof response !== "object") {
      switch (response) {
        ...
        case "CompleteHighlightingAndMakePromptReappear" :
            ...
            return Tokens$AgdaModeVscode.generate(state.tokens, state.editor);
        default:
          return ;
      }
    } else {
      switch (response.TAG) {
        ...
        case "GiveAction" :
            ...
            if (typeof give !== "object") {
              ...
              var holeOffsets = State__Goal$AgdaModeVscode.parseHolesFromRefineResult(indented);
              console.log("holeOffsets: ", holeOffsets);
              await State__Goal$AgdaModeVscode.modify(state, goal, (function (param) {
                      return indented;
                    }));
              var offset = goal.interval[0];
              var holeTokens = holeOffsets.map(function (start) {
                    var start$1 = start + offset | 0;
                    var end = start$1 + 6 | 0;
                    return {
                            start: start$1,
                            end: end,
              ...
              console.log("holeTokens: ", holeTokens);
              holeTokens.forEach(function (token) {
                    Tokens$AgdaModeVscode.toOriginalOffset(state.tokens, token.start);
                    var end = Tokens$AgdaModeVscode.toOriginalOffset(state.tokens, token.end);
                    console.log("end: ", end);
                    Tokens$AgdaModeVscode.insertWithVSCodeOffsets(state.tokens, token.start, token.end, token);
                  });
              return await State__Goal$AgdaModeVscode.removeBoundaryAndDestroy(state, goal);
            }
        case "MakeCase" :
