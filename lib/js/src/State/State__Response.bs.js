...
async function handle(state, x) {
  switch (x.TAG) {
    ...
    case "AllGoalsWarnings" :
        ...
        var header = x._0;
        if (body$1 === "nil") {
          if (Tokens$AgdaModeVscode.getHoles(state.tokens).size > 0) {
            return await State__View$AgdaModeVscode.Panel.display(state, {
                        TAG: "Warning",
                        _0: "Stale holes detected"
                      }, [Item$AgdaModeVscode.warning(RichText$AgdaModeVscode.string("Agda returned no interaction points, but holes are still present in the file. This is a known Agda bug (agda/agda#2475, agda/agda#6871) that can occur with cached .agdai files built under --allow-unsolved-metas. To recover: delete the _build directory and reload, or make a small edit inside a hole and reload."), undefined)]);
          } else {
            return await State__View$AgdaModeVscode.Panel.display(state, {
                        TAG: "Success",
                        _0: "*All Done*"
    ...
    if (typeof response !== "object") {
      ...
      switch (response.TAG) {
        ...
        case "InteractionPoints" :
            ...
            var positionsArray = Array.from(holePositions.entries());
            Goals$AgdaModeVscode.addGoalPositions(state.goals, positionsArray);
            if (state.isInRefineOrGiveOperation) {
              return await Goals$AgdaModeVscode.resetGoalIndicesOnRefineOrGive(state.goals, state.editor, indices);
            } else {
              return await Goals$AgdaModeVscode.resetGoalIndicesOnLoad(state.goals, state.editor, indices);
            }
        case "GiveAction" :
            ...
            if (!Goals$AgdaModeVscode.isIntact(state.goals, state.document, index)) {
              return await State__View$AgdaModeVscode.Panel.display(state, {
                          TAG: "Error",
                          _0: "Error: Give failed"
                        }, [Item$AgdaModeVscode.plainText("Goal #" + String(index) + " is out of sync with the file, so nothing was changed. Reload with \"Load\" and try again.")]);
            }
            if (typeof give !== "object") {
              ...
              var indented = indent(Parser$AgdaModeVscode.unescapeEOL(give._0), 2 + match$2[0] | 0);
        ...
        case "MakeCase" :
            ...
            if (!Goal$AgdaModeVscode.isIntact(goal$1, state.document)) {
              return await State__View$AgdaModeVscode.Panel.display(state, {
                          TAG: "Error",
                          _0: "Cannot split the goal"
                        }, [Item$AgdaModeVscode.plainText("Goal #" + String(goal$1.index) + " is out of sync with the file, so nothing was changed. Reload with \"Load\" and try again.")]);
            }
            var result;
