...
function caseSplitAux($$document, goal) {
  ...
  var lastLineBreakOffset = Caml.caml_int_max(0, Caml.caml_int_max(textBeforeGoal.lastIndexOf("\r"), textBeforeGoal.lastIndexOf("\n"))) + 1 | 0;
  var inWhereClause = lastWhereTokenOffset > lastOpenCurlyBracketOffset;
  var offset = Caml.caml_int_max(lastLineBreakOffset > lastSemicolonOffset ? lastLineBreakOffset : lastSemicolonOffset, lastWhereTokenOffset > lastOpenCurlyBracketOffset ? lastWhereTokenOffset : lastOpenCurlyBracketOffset);
  ...
  return $$Promise.flatMap(Goal$AgdaModeVscode.setContent(goal, state.document, Curry._1(f, content)), (function (x) {
                if (x) {
                  ...
                  return State$AgdaModeVscode.View.display(state, {
                            ...
                            }, [Component$AgdaModeVscode.Item.plainText("Failed to modify the content of goal #" + String(goal.index))]);
                }
              }));
}
...
function placeCursorAtFirstNewGoal(state, rewriteText, rewriteRange) {
  var splittedLines = rewriteText.split("\n");
  return Belt_Option.forEach(Belt_Array.get(splittedLines, 0), (function (line) {
                ...
                return Editor$AgdaModeVscode.Cursor.set(state.editor, position);
              }));
}
  ...
  return $$Promise.flatMap(Editor$AgdaModeVscode.$$Text.replace(state.document, rewriteRange, rewriteText), (function (x) {
                if (x) {
                  ...
                  return State$AgdaModeVscode.View.display(state, {
                              ...
                              _0: "Goal-related Error",
                              [Symbol.for("name")]: "Error"
                            }, [Component$AgdaModeVscode.Item.plainText("Unable to replace the lines of goal #" + String(goal.index))]);
                }
              }));
}
  ...
  return $$Promise.flatMap(Editor$AgdaModeVscode.$$Text.replace(state.document, outerRange, content), (function (x) {
                if (x) {
                  ...
                  return State$AgdaModeVscode.View.display(state, {
                              ...
                              _0: "Goal-related Error",
                              [Symbol.for("name")]: "Error"
                            }, [Component$AgdaModeVscode.Item.plainText("Unable to remove the boundary of goal #" + String(goal.index))]);
                }
              }));
}
