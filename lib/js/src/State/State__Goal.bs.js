...
function pointingAt(cursor, state) {
  ...
  var cursorOffset = state.document.offsetAt(cursor$1);
...
function caseSplitAux($$document, goal) {
  ...
  var range = Common$AgdaModeVscode.Interval.toRange($$document, interval);
...
function updateRanges(state) {
  ...
  var diffs = generateDiffs(state.editor.document, indices);
...
function replaceWithLines(state, goal, lines) {
  ...
  var start = Common$AgdaModeVscode.Offset.toPosition(state.document, goal.interval[0]);
  var startLineNo = start.line;
  var startLineRange = $$document.lineAt(startLineNo).range;
  var start$1 = startLineRange.start;
  var end_ = Common$AgdaModeVscode.Offset.toPosition(state.document, goal.interval[1]);
...
function removeBoundaryAndDestroy(state, goal) {
  ...
  var outerRange = Common$AgdaModeVscode.Interval.toRange($$document, goal.interval);
