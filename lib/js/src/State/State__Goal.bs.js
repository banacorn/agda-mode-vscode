...
async function decorateHoles(editor, holeOffsets, indices) {
  return Belt_Array.zip(Array.from(holeOffsets.entries()), indices).map(function (param) {
              var index = param[1];
...
async function instantiateGoals(state, editor, holeOffsets, indices) {
  return await decorateHoles(editor, holeOffsets, indices);
}
...
async function instantiate(state, indices) {
  ...
  var holeOffsets = await Resource$AgdaModeVscode.get(Tokens$AgdaModeVscode.getHolesOffsets(state.tokens));
  var goals = await instantiateGoals(state, state.editor, holeOffsets, indices);
...
exports.instantiateGoals = instantiateGoals;
