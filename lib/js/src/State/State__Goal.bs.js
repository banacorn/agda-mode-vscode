...
async function rewriteQuestionMarks(state, editor) {
  ...
  var questionMarkTokens = Core__Array.filterMap(Array.from(Tokens$AgdaModeVscode.getHoles(state.tokens).values()), (function (token) {
          var range = new Vscode.Range($$document.positionAt(token.start), $$document.positionAt(token.end));
...
async function decorateHoles(state, editor, indices) {
  return Belt_Array.zip(Tokens$AgdaModeVscode.getHolesSorted(state.tokens), indices).map(function (param) {
              var index = param[1];
              var token = param[0];
              console.log("interval: " + Tokens$AgdaModeVscode.Token.toString(token));
