...
function start(useLSP, viaTCP) {
  ...
  if (conn !== undefined) {
    ...
    return $$Promise.mapOk(Connection__LSP$AgdaModeVscode.make(viaTCP), (function (conn) {
...
function sendRequest(useLSP, viaTCP, $$document, request, handler) {
  ...
  return $$Promise.mapOk(Connection__LSP$AgdaModeVscode.sendRequest(conn$1, encodeRequest($$document, match$2[0]), handler), (function (param) {
                return toStatus({
                            TAG: 1,
                            _0: conn$1,
                            [Symbol.for("name")]: "LSP"
