...
function decodeNotification(json) {
  var notification;
  try {
    notification = Json_decode.string(json);
  }
  catch (raw_msg){
    var msg = Caml_js_exceptions.internalToOCamlException(raw_msg);
    if (msg.RE_EXN_ID === Json_decode.DecodeError) {
...
function sendRequest$2(request, notificationHandler) {
  ...
  var subscription = new Vscode.Disposable(Chan$AgdaModeVscode.on(dataChan, (function (json) {
              var notification = decodeNotification(json);
              if (notification.TAG === /* Ok */0) {
                return Curry._1(notificationHandler, notification._0);
              } else {
                return Curry._1(resolve, {
                            TAG: 1,
                            _0: notification._0,
  ...
  $$Promise.get($$Promise.tap($$Promise.flatMapOk(sendRequestPrim(match._0, {
                    _0: request,
