...
function make(param) {
  var getPath = function (param) {
    var agdaVersion = Belt_Option.mapWithDefault(Config$AgdaModeVscode.getAgdaVersion(undefined), "agda", (function (prim) {
            return prim.trim();
          }));
    var storedPath = Belt_Option.mapWithDefault(Config$AgdaModeVscode.getAgdaPath(undefined), "", (function (prim) {
            return prim.trim();
          }));
...
function sendRequest(connection, $$document, request) {
  ...
  Curry._1(connection.process.send, encoded);
  
}

function onResponse(connection, callback, callbackAfterNonLasts) {
  ...
  var listener = function (x) {
    if (x.TAG === /* Ok */0) {
      ...
      $$Promise.flatMap($$Promise.map($$Promise.flatMap($$Promise.tap(onceDone(undefined), stopListener), callbackAfterNonLasts), (function (param) {
