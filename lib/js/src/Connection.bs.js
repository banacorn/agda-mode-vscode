...
function runNonLast(self, callback, response) {
  console.log("[NonLast] " + Response$AgdaModeVscode.toString(response));
  self.tally = self.tally + 1 | 0;
  return $$Promise.get(Curry._1(callback, {
                  TAG: 0,
                  _0: response,
                  [Symbol.for("name")]: "Ok"
                }), (function (param) {
...
function addLast(self, priority, response) {
  console.log("[Add Last] " + String(priority) + " " + Response$AgdaModeVscode.toString(response));
...
function runLast(self, callback) {
...
function onResponse(connection, callback) {
  ...
  var listener = function (x) {
    if (x.TAG === /* Ok */0) {
      var match = x._0;
      if (!match) {
        return $$Promise.get($$Promise.tap(onceDone(scheduler), stopListener), (function (param) {
...
function toString$1(x) {
  if (typeof x === "number") {
    return "========";
  } else if (x.TAG === /* ReactionNonLast */0) {
    return x._0;
  } else {
    return "[Last " + String(x._0) + "] " + x._1;
  }
}
...
function sendRequest$2(request, notificationHandler) {
  ...
  var subscription = new Vscode.Disposable(Chan$AgdaModeVscode.on(dataChan, (function (json) {
              ...
              } else if (responese.TAG === /* ReactionNonLast */0) {
                return Curry._1(notificationHandler, responese._0);
              } else {
                return Curry._1(notificationHandler, responese._1);
              }
            })));
