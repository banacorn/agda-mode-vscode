...
function parseWithContinuation(string) {
  var parseSExpression = function (state, string) {
    ...
    var pendingBackslashes = state.pendingBackslashes;
    var word = state.word;
    ...
    var processChar = function ($$char) {
      ...
      var expr = stack.pop();
      if (expr !== undefined) {
        return pushToTheTop(expr.contents);
      }
    ...
    for(var i = 0; i < totalLength; ++i){
      ...
      if ($$char === "\\" && in_str.contents) {
        pendingBackslashes.contents = pendingBackslashes.contents + 1 | 0;
      } else if (pendingBackslashes.contents > 0) {
        var count = pendingBackslashes.contents;
        var pairs = count / 2 | 0;
        var odd = count % 2 === 1;
        word.contents = word.contents + "\\".repeat(pairs);
        pendingBackslashes.contents = 0;
        if (odd) {
          if ($$char === "n") {
