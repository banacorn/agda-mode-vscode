...
function Impl(Editor) {
  ...
  var make = function (state) {
    ...
    var printStatus = function (task, runner, queues) {
      ...
      return Belt_Array.forEach(queues, (function (queue) {
                    if (queue.tag) {
                      console.log("View " + Util$AgdaModeVscode.Pretty.array(Belt_Array.map(queue[0], Task.toString)));
                      return ;
                    }
                    console.log("Agda " + Util$AgdaModeVscode.Pretty.array(Belt_Array.map(queue[0], Task.toString)));
