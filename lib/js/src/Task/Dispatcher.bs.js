...
function Impl(Editor) {
  ...
  var dispatchCommand = function (self, command) {
    ...
    var tasks = Belt_List.toArray(Curry._1(CommandHandler.handle, command));
    return Runner2$AgdaModeVscode.pushMany(self.otherTaskRunner, tasks);
  };
  var sendAgdaRequest = function (runTasks, state, req) {
    ...
    var handler = function (error) {
      if (error.tag) {
        var tasks = Belt_List.toArray(Curry._1(ErrorHandler.handle, /* Connection */Caml_chrome_debugger.variant("Connection", 0, [error[0]])));
      ...
      var error$1 = match[0];
      if (error$1.tag) {
        var tasks$1 = Belt_List.toArray(Curry._1(ErrorHandler.handle, /* Parser */Caml_chrome_debugger.variant("Parser", 1, [error$1[0]])));
        return $$Promise.get(Curry._1(runTasks, tasks$1), resolve);
      }
      var tasks$2 = Belt_List.toArray(Curry._1(ResponseHandler.handle, error$1[0]));
    ...
    return $$Promise.tap($$Promise.flatMap(Curry._2(Task.State.sendRequestToAgda, state, req), (function (connection) {
                      if (connection.tag) {
                        var tasks = Belt_List.toArray(Curry._1(ErrorHandler.handle, connection[0]));
    ...
    Runner2$AgdaModeVscode.setup(self.otherTaskRunner, (function (task) {
            ...
            if (reqs) {
              ...
              return $$Promise.flatMap(sendAgdaRequest(runTasks, state, req), (function (param) {
                            var reqs = self.agdaRequestStatus;
                            var reqs$1 = reqs ? reqs[0] : [];
                            self.agdaRequestStatus = /* Available */0;
                            return Runner2$AgdaModeVscode.pushManyToFront(self.agdaRequestRunner, reqs$1);
                          }));
            }
          }));
    return self;
