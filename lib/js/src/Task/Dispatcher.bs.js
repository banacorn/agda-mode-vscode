...
function make(extentionPath, editor, removeFromRegistry, chan) {
  ...
  subscribe(Vscode.window.onDidChangeTextEditorSelection(function ($$event) {
            ...
            var offsets = Belt_Array.map(Belt_Array.map($$event.selections, (function (prim) {
                        return prim.anchor;
                      })), (function (param) {
                    return partial_arg.offsetAt(param);
                  }));
            console.log("SELECT", offsets);
            var partial_arg$1 = dispatcher_critical;
            return $$Promise.get(Handle__InputMethod$AgdaModeVscode.select(state, offsets), (function (param) {
                          return TaskQueue$AgdaModeVscode.addToTheFront(partial_arg$1, param);
                        }));
          }));
  subscribe(Vscode.workspace.onDidChangeTextDocument(function ($$event) {
            var input = IM$AgdaModeVscode.Input.fromTextDocumentChangeEvent(editor, $$event);
            var partial_arg = dispatcher_critical;
            return $$Promise.get($$Promise.flatMap(IM$AgdaModeVscode.run(state.editorIM, Caml_option.some(editor), input), (function (param) {
                              return Handle__InputMethod$AgdaModeVscode.EditorIM.handle(state, param);
                            })), (function (param) {
                          return TaskQueue$AgdaModeVscode.addToTheFront(partial_arg, param);
                        }));
          }));
