...
function sendAgdaRequest(addToAgdaQueue, addToDeferredQueue, state, request) {
  ...
  return $$Promise.tap($$Promise.flatMap($$Promise.mapOk(State$AgdaModeVscode.connect(state), (function (connection) {
                    ...
                    if (connection.TAG) {
                      var tasks = Handle__Error$AgdaModeVscode.handle(connection._0);
                      Curry._1(addToAgdaQueue, tasks);
                      return promise;
                    }
                    handle.contents = Curry._1(connection._0.emitter.on, handler);
                    return promise;
                  })), (function (param) {
    ...
    switch (task.TAG | 0) {
      ...
      case /* ViewRequest */3 :
          ...
          if (TaskQueue$AgdaModeVscode.View.isOccupied(queue)) {
            ...
            $$Promise.map($$Promise.flatMap(State$AgdaModeVscode.sendRequestToView(state, task._0), (function (response) {
