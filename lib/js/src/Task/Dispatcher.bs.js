...
function Impl(Editor) {
  ...
  var acquire = function (self, resource) {
    var queue = Belt_List.fromArray(Runner$AgdaModeVscode.empty(self.runner));
  ...
  var onResponse = function (self, tasks) {
  ...
  var toString = function (self, task) {
    console.log("\nTask: " + (Curry._1(Task.toString, task) + ("\nRunner: " + (Util$AgdaModeVscode.Pretty.array(Belt_Array.map(self.runner.queue, Task.toString)) + "\n==============================="))));
      ...
      switch (task.tag | 0) {
        case /* SendRequest */0 :
            if (blockedBy(self, /* Agda */0)) {
              ...
              acquire(self, /* Agda */0);
              $$Promise.get(sendAgdaRequest((function (param) {
                          return Runner$AgdaModeVscode.pushAndRun(self.runner, param);
