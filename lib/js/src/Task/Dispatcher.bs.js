...
function Impl(Editor) {
  ...
  var dispatchCommand = function (self, command) {
    ...
    return Runner2$AgdaModeVscode.pushToBackAndRun(self.generalTaskRunner, tasks);
  };
  ...
  var make = function (state) {
    var self = {
      ...
      generalTaskRunner: Runner2$AgdaModeVscode.make(undefined)
    };
    Runner2$AgdaModeVscode.setup(self.generalTaskRunner, (function (task) {
            ...
            switch (task.tag | 0) {
              ...
              case /* ViewReq */1 :
                  var callback = task[1];
                  var req$1 = task[0];
                  var reqs$1 = self.viewRequestStatus;
                  if (reqs$1) {
                    reqs$1[0].push(req$1);
                    return $$Promise.resolved(undefined);
                  } else {
                    self.viewRequestStatus = /* Occupied */Caml_chrome_debugger.simpleVariant("Occupied", [[]]);
                    return $$Promise.flatMap($$Promise.flatMap(Curry._2(Task.State.sendRequestToView, state, req$1), (function (response) {
                                      var tasks = Curry._1(callback, response);
                                      return Runner2$AgdaModeVscode.pushToBackAndRun(self.generalTaskRunner, tasks);
                                    })), (function (param) {
                                  var reqs = self.viewRequestStatus;
                                  var reqs$1 = reqs ? Belt_List.fromArray(reqs[0]) : /* [] */0;
                                  self.viewRequestStatus = /* Available */0;
                                  return Runner2$AgdaModeVscode.pushToFrontAndRun(self.viewRequestRunner, reqs$1);
                                }));
                  }
              default:
                return $$Promise.resolved(undefined);
