...
function make(extentionPath, editor, removeFromRegistry, chan) {
  ...
  subscribe(Vscode.window.onDidChangeTextEditorSelection(function ($$event) {
            ...
            return $$Promise.get($$Promise.map(EditorIM$AgdaModeVscode.run(state.editorIM, Caml_option.some(editor), input), Handle__InputMethod$AgdaModeVscode.handleEditorIMOutput), (function (actions) {
                          return Belt_Array.forEach(actions, (function (action) {
                                        dispatchCommand(dispatcher, {
                                              TAG: 13,
                                              _0: action,
