...
function make(extentionPath, editor, removeFromRegistry, chan) {
  ...
  subscribe(Vscode.window.onDidChangeTextEditorSelection(function ($$event) {
            var points = EditorIM$AgdaModeVscode.handleTextEditorSelectionChangeEvent($$event);
            return $$Promise.get(EditorIM$AgdaModeVscode.changeSelection(state.editorIM, editor, points), (function (shouldDeactivate) {
                          if (shouldDeactivate) {
                            dispatchCommand(dispatcher, {
                                  TAG: 13,
                                  _0: /* Deactivate */1,
                            ...
                            console.log("EditorIM should deactivate");
                            return ;
                          }
  ...
  subscribe(Vscode.workspace.onDidChangeTextDocument(function ($$event) {
            var changes = EditorIM$AgdaModeVscode.handleTextDocumentChangeEvent(editor, $$event);
            return EditorIM$AgdaModeVscode.changeDocument(state.editorIM, editor, changes);
          }));
  subscribe(new Vscode.Disposable(EditorIM$AgdaModeVscode.onCommand(state.editorIM, (function (command) {
                  dispatchCommand(dispatcher, {
                        TAG: 13,
                        _0: command,
                        [Symbol.for("name")]: "InputMethod"
