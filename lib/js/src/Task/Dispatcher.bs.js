...
function sendAgdaRequest(addToAgdaQueue, addToDeferredQueue, state, request) {
  ...
  var displayConnectionError = function (error) {
    ...
    return Task$AgdaModeVscode.display({
                TAG: 3,
  ...
  var handler = function (x) {
    if (x.TAG) {
      return Curry._1(addToAgdaQueue, {
                  hd: displayConnectionError(x._0),
                  tl: /* [] */0
                });
    }
    var match = x._0;
  ...
  return $$Promise.tap($$Promise.flatMap($$Promise.mapOk(State$AgdaModeVscode.connect(state), (function (connection) {
                    ...
                    if (x.TAG) {
                      var tasks_0 = displayConnectionError(x._0);
                      var tasks = {
                        hd: tasks_0,
                        tl: /* [] */0
                      };
            ...
            keepRunning = $$Promise.map($$Promise.flatMap(sendAgdaRequest((function (tasks) {
                    ...
                    var deferredTasks$2 = Config$AgdaModeVscode.getSemanticHighlighting(undefined) ? deferredTasks$1 : ({
                          hd: {
                            TAG: 4,
                            _0: (function (state) {
                                return $$Promise.map(Handle__Decoration$AgdaModeVscode.apply(state), (function (param) {
                                              return /* [] */0;
                                            }));
                              }),
...
function make(extentionPath, editor, removeFromRegistry, chan) {
  ...
  subscribe(Vscode.window.onDidChangeTextEditorSelection(function ($$event) {
            ...
            return $$Promise.get(Handle__InputMethod$AgdaModeVscode.select(state, intervals), (function (param) {
  ...
  subscribe(Vscode.workspace.onDidChangeTextDocument(function ($$event) {
            ...
            var partial_arg = dispatcher_critical;
            return $$Promise.get(Handle__InputMethod$AgdaModeVscode.keyUpdateEditorIM(state, changes), (function (param) {
                          return TaskQueue$AgdaModeVscode.addToTheFront(partial_arg, param);
                        }));
          }));
