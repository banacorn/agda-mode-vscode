...
function sendAgdaRequest(addToAgdaQueue, addToDeferredQueue, state, request) {
  ...
  var handler = function (error) {
    if (error.tag) {
      return Curry._1(addToAgdaQueue, Handle__Error$AgdaModeVscode.handle(/* Connection */Caml_chrome_debugger.variant("Connection", 0, [error[0]])));
    }
    var match = error[0];
    if (match) {
      var error$1 = match[0];
      if (error$1.tag) {
        return Curry._1(addToAgdaQueue, Handle__Error$AgdaModeVscode.handle(/* Parser */Caml_chrome_debugger.variant("Parser", 1, [error$1[0]])));
      }
      var response = error$1[0];
      if (!response.tag) {
        return Curry._1(addToAgdaQueue, Handle__Response$AgdaModeVscode.handle(response[0]));
      }
  ...
  return $$Promise.tap($$Promise.flatMap($$Promise.mapOk(State$AgdaModeVscode.connect(state), (function (connection) {
                    ...
                    if (connection.tag) {
                      var tasks = Handle__Error$AgdaModeVscode.handle(connection[0]);
  ...
  if (typeof task === "number") {
    ...
    switch (task.tag | 0) {
      case /* DispatchCommand */0 :
          var tasks = Handle__Command$AgdaModeVscode.handle(task[0]);
      ...
      case /* AgdaRequest */1 :
          if (TaskQueue$AgdaModeVscode.Agda.isOccupied(queue)) {
            ...
            keepRunning = $$Promise.map($$Promise.flatMap(sendAgdaRequest((function (tasks) {
                    ...
                    var deferredTasks$2 = Config$AgdaModeVscode.getSemanticHighlighting(undefined) ? deferredTasks$1 : /* :: */Caml_chrome_debugger.simpleVariant("::", [
                          /* Decoration */Caml_chrome_debugger.variant("Decoration", 4, [/* Apply */1]),
                          deferredTasks$1
                        ]);
                    console.log("APPLY!!!!");
          ...
          break;
      case /* ViewEvent */2 :
          keepRunning = $$Promise.map(State$AgdaModeVscode.sendEventToView(state, task[0]), (function (param) {
                  return true;
                }));
          break;
      case /* ViewRequest */3 :
          ...
          if (TaskQueue$AgdaModeVscode.View.isOccupied(queue)) {
            ...
            $$Promise.map($$Promise.flatMap(State$AgdaModeVscode.sendRequestToView(state, task[0]), (function (response) {
                        if (response !== undefined) {
                          TaskQueue$AgdaModeVscode.View.addToTheBack(queue, Curry._1(callback, response));
          ...
          break;
      case /* Decoration */4 :
          var tasks$1 = Handle__Decoration$AgdaModeVscode.handle(task[0]);
          TaskQueue$AgdaModeVscode.addToTheFront(queue, tasks$1);
          keepRunning = $$Promise.resolved(true);
          break;
      case /* Error */5 :
          var tasks$2 = Handle__Error$AgdaModeVscode.handle(task[0]);
          TaskQueue$AgdaModeVscode.addToTheFront(queue, tasks$2);
          keepRunning = $$Promise.resolved(true);
          break;
      case /* Goal */6 :
          var tasks$3 = Handle__Goal$AgdaModeVscode.handle(task[0]);
...
function make(extentionPath, editor, removeFromRegistry, eventEmitter) {
  var state = State$AgdaModeVscode.make(extentionPath, eventEmitter, editor);
  ...
  state.subscriptions.push(new Vscode.Disposable(Curry._1(state.editorIM.eventEmitter.on, (function (action) {
  ...
  var hoverProvider = function (fileName, point) {
    ...
    return Caml_option.some($$Promise.resolved(/* tuple */[
                    [""],
                    range
                  ]));
  };
  ...
  var disposables$1 = Editor$AgdaModeVscode.Provider.registerTestingProvider(documentSemanticTokensProvider, /* tuple */[
        tokenTypes,
        tokenModifiers
      ]);
