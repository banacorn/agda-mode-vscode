...
function Impl(Editor) {
  ...
  var indentationWidth = function (goal, editor) {
    var start = Curry._2(Editor.pointAtOffset, editor, goal.range[0]);
    var startLineNo = Curry._1(Editor.Point.line, start);
    var startLineRange = Curry._2(Editor.rangeForLine, editor, startLineNo);
    var startLineText = Curry._2(Editor.getTextInRange, editor, startLineRange);
    var n = 0;
    for(var i = 0 ,i_finish = startLineText.length; i < i_finish; ++i){
      var match = startLineText.charAt(i);
      var exit = 0;
      switch (match) {
        case "\t" :
        case "\n" :
    ...
    if (typeof indices === "number") {
      ...
      switch (indices.tag | 0) {
        ...
        case /* ReplaceWithLines */3 :
            ...
            return /* :: */Caml_chrome_debugger.simpleVariant("::", [
                      /* WithState */Caml_chrome_debugger.variant("WithState", 5, [(function (state) {
                              var indentWidth = indentationWidth(goal$2, state.editor);
                              var indentation = " ".repeat(indentWidth);
                              ...
                              var endLineNo = Curry._1(Editor.Point.line, end_);
                              var endLineRange = Curry._2(Editor.rangeForLine, state.editor, endLineNo);
        ...
        case /* ReplaceWithLambda */4 :
            ...
            return /* :: */Caml_chrome_debugger.simpleVariant("::", [
                      /* WithState */Caml_chrome_debugger.variant("WithState", 5, [(function (state) {
                              console.log(lines$1);
                              var goalStart = Curry._2(Editor.pointAtOffset, state.editor, goal$3.range[0]);
                              var goalEnd = Curry._2(Editor.pointAtOffset, state.editor, goal$3.range[1]);
                              var lineBeforeGoal = Curry._2(Editor.getTextInRange, state.editor, Curry._2(Editor.$$Range.make, Curry._2(Editor.Point.make, Curry._1(Editor.Point.line, goalStart), 0), goalStart));
                              var indentWidth = indentationWidth(goal$3, state.editor);
                              var scanRow = Curry._1(Editor.Point.line, goalStart);
                              var n = lineBeforeGoal.lastIndexOf(";");
                              var scanColStart = n !== -1 ? n + 1 | 0 : indentWidth;
                              var scanColEnd = Curry._1(Editor.Point.column, goalStart);
                              var bracketCount = 0;
                              var i = scanColEnd - 1 | 0;
                              while(i >= scanColStart && bracketCount >= 0) {
                                var i$prime = i;
                                if (i$prime !== 0) {
                                  var match = lineBeforeGoal.charAt(i$prime - 1 | 0);
                                  switch (match) {
                                    case "{" :
                              ...
                              return $$Promise.map(isLambdaWhere ? Curry._3(Editor.setText, state.editor, rewriteRange, lines$1.join("\n" + " ".repeat(indentWidth))) : Curry._3(Editor.setText, state.editor, rewriteRange, " " + lines$1.join(" ; ")), (function (param) {
                                            if (param) {
