        ...
        case /* RestoreCursor */4 :
            return /* :: */Caml_chrome_debugger.simpleVariant("::", [
                      /* WithState */Caml_chrome_debugger.variant("WithState", 7, [(function (state) {
                              ...
                              var cursorOffset = Curry._2(Editor.offsetAtPoint, state.editor, cursor);
                              var pointedGoals = Belt_Array.keep(state.goals, (function (goal) {
                                      if (goal.range[0] <= cursorOffset && cursorOffset <= (goal.range[0] + 2 | 0)) {
                                        return true;
                                      } else if ((goal.range[1] - 2 | 0) <= cursorOffset) {
                                        return cursorOffset <= goal.range[1];
                                      } else {
                                        return false;
                                      }
                                    }));
                              var pointedGoal = Belt_Array.get(pointedGoals, 0);
                              if (pointedGoal !== undefined) {
                                return Curry._2(Task.Goal.setCursor, pointedGoal, state.editor);
                              } else {
                                Curry._2(Editor.setCursorPosition, state.editor, Caml_option.valFromOption(position));
