// Generated by ReScript, PLEASE EDIT WITH CARE
'use strict';

var VSCode = require("bs-vscode/lib/js/src/VSCode.bs.js");
var Vscode = require("vscode");
var $$Promise = require("reason-promise/lib/js/src/js/promise.bs.js");
var Belt_List = require("bs-platform/lib/js/belt_List.js");
var Belt_Array = require("bs-platform/lib/js/belt_Array.js");
var IM$AgdaModeVscode = require("../InputMethod/IM.bs.js");
var Goal$AgdaModeVscode = require("../Goal.bs.js");
var Task$AgdaModeVscode = require("./Task.bs.js");
var Editor$AgdaModeVscode = require("../Editor.bs.js");
var Parser$AgdaModeVscode = require("../Parser.bs.js");
var Command$AgdaModeVscode = require("../Command.bs.js");
var Handle__Goal$AgdaModeVscode = require("./Handle__Goal.bs.js");
var View__Controller$AgdaModeVscode = require("../View__Controller.bs.js");
var Handle__Decoration$AgdaModeVscode = require("./Handle__Decoration.bs.js");
var Handle__InputMethod$AgdaModeVscode = require("./Handle__InputMethod.bs.js");

function handle(command) {
  var header = {
    TAG: 0,
    _0: Command$AgdaModeVscode.toString(command),
    [Symbol.for("name")]: "Plain"
  };
  if (typeof command === "number") {
    switch (command) {
      case /* Load */0 :
          return {
                  hd: Task$AgdaModeVscode.display({
                        TAG: 0,
                        _0: "Loading ...",
                        [Symbol.for("name")]: "Plain"
                      }, /* Nothing */0),
                  tl: {
                    hd: {
                      TAG: 4,
                      _0: (function (state) {
                          var $$document = state.editor.document;
                          var options = VSCode.TextDocumentShowOptions.make(undefined, false, undefined, undefined, undefined);
                          return $$Promise.map($$Promise.flatMap($$document.save(), (function (param) {
                                            return Vscode.window.showTextDocument($$document, options);
                                          })), (function (param) {
                                        return /* [] */0;
                                      }));
                        }),
                      [Symbol.for("name")]: "WithStateP"
                    },
                    tl: {
                      hd: {
                        TAG: 1,
                        _0: /* Load */0,
                        [Symbol.for("name")]: "AgdaRequest"
                      },
                      tl: /* [] */0
                    }
                  }
                };
      case /* Quit */1 :
          return /* [] */0;
      case /* Restart */2 :
          return {
                  hd: {
                    TAG: 0,
                    _0: /* Load */0,
                    [Symbol.for("name")]: "DispatchCommand"
                  },
                  tl: /* [] */0
                };
      case /* Refresh */3 :
          return {
                  hd: {
                    TAG: 3,
                    _0: (function (state) {
                        Handle__Goal$AgdaModeVscode.updateRanges(state);
                        return Handle__Decoration$AgdaModeVscode.refresh(state);
                      }),
                    [Symbol.for("name")]: "WithState"
                  },
                  tl: /* [] */0
                };
      case /* Compile */4 :
          return {
                  hd: {
                    TAG: 1,
                    _0: /* Compile */1,
                    [Symbol.for("name")]: "AgdaRequest"
                  },
                  tl: /* [] */0
                };
      case /* ToggleDisplayOfImplicitArguments */5 :
          return {
                  hd: {
                    TAG: 1,
                    _0: /* ToggleDisplayOfImplicitArguments */2,
                    [Symbol.for("name")]: "AgdaRequest"
                  },
                  tl: /* [] */0
                };
      case /* ShowConstraints */6 :
          return {
                  hd: {
                    TAG: 1,
                    _0: /* ShowConstraints */3,
                    [Symbol.for("name")]: "AgdaRequest"
                  },
                  tl: /* [] */0
                };
      case /* ShowGoals */7 :
          return {
                  hd: {
                    TAG: 1,
                    _0: /* ShowGoals */4,
                    [Symbol.for("name")]: "AgdaRequest"
                  },
                  tl: /* [] */0
                };
      case /* NextGoal */8 :
          return Handle__Goal$AgdaModeVscode.next;
      case /* PreviousGoal */9 :
          return Handle__Goal$AgdaModeVscode.previous;
      case /* Give */10 :
          return Handle__Goal$AgdaModeVscode.$$case((function (goal, param) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 3,
                                    _0: goal,
                                    [Symbol.for("name")]: "Give"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), (function (goal) {
                        return Task$AgdaModeVscode.prompt(header, {
                                    body: undefined,
                                    placeholder: "expression to give:",
                                    value: undefined
                                  }, (function (expr) {
                                      return Belt_List.concat(Handle__Goal$AgdaModeVscode.modify(goal, (function (param) {
                                                        return expr;
                                                      })), {
                                                  hd: {
                                                    TAG: 1,
                                                    _0: {
                                                      TAG: 3,
                                                      _0: goal,
                                                      [Symbol.for("name")]: "Give"
                                                    },
                                                    [Symbol.for("name")]: "AgdaRequest"
                                                  },
                                                  tl: /* [] */0
                                                });
                                    }));
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* Refine */11 :
          return Handle__Goal$AgdaModeVscode.caseSimple((function (goal) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 4,
                                    _0: goal,
                                    [Symbol.for("name")]: "Refine"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* Auto */12 :
          return Handle__Goal$AgdaModeVscode.caseSimple((function (goal) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 6,
                                    _0: goal,
                                    [Symbol.for("name")]: "Auto"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* Case */13 :
          return Handle__Goal$AgdaModeVscode.$$case((function (goal, param) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 7,
                                    _0: goal,
                                    [Symbol.for("name")]: "Case"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), (function (goal) {
                        return Task$AgdaModeVscode.prompt(header, {
                                    body: "Please specify which variable you wish to split",
                                    placeholder: "variable to case split:",
                                    value: undefined
                                  }, (function (expr) {
                                      return Belt_List.concat(Handle__Goal$AgdaModeVscode.modify(goal, (function (param) {
                                                        return expr;
                                                      })), {
                                                  hd: {
                                                    TAG: 1,
                                                    _0: {
                                                      TAG: 7,
                                                      _0: goal,
                                                      [Symbol.for("name")]: "Case"
                                                    },
                                                    [Symbol.for("name")]: "AgdaRequest"
                                                  },
                                                  tl: /* [] */0
                                                });
                                    }));
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* WhyInScope */14 :
          var placeholder = "name:";
          return Handle__Goal$AgdaModeVscode.$$case((function (goal, expr) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 20,
                                    _0: expr,
                                    _1: goal,
                                    [Symbol.for("name")]: "WhyInScope"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), (function (goal) {
                        return Task$AgdaModeVscode.prompt(header, {
                                    body: undefined,
                                    placeholder: placeholder,
                                    value: undefined
                                  }, (function (expr) {
                                      return {
                                              hd: {
                                                TAG: 1,
                                                _0: {
                                                  TAG: 20,
                                                  _0: expr,
                                                  _1: goal,
                                                  [Symbol.for("name")]: "WhyInScope"
                                                },
                                                [Symbol.for("name")]: "AgdaRequest"
                                              },
                                              tl: /* [] */0
                                            };
                                    }));
                      }), Task$AgdaModeVscode.prompt(header, {
                          body: undefined,
                          placeholder: placeholder,
                          value: undefined
                        }, (function (expr) {
                            return {
                                    hd: {
                                      TAG: 1,
                                      _0: {
                                        TAG: 21,
                                        _0: expr,
                                        [Symbol.for("name")]: "WhyInScopeGlobal"
                                      },
                                      [Symbol.for("name")]: "AgdaRequest"
                                    },
                                    tl: /* [] */0
                                  };
                          })));
      case /* Escape */15 :
          return {
                  hd: {
                    TAG: 4,
                    _0: (function (state) {
                        if (IM$AgdaModeVscode.isActivated(state.editorIM) || IM$AgdaModeVscode.isActivated(state.promptIM)) {
                          return Handle__InputMethod$AgdaModeVscode.deactivate(state);
                        } else {
                          return $$Promise.resolved({
                                      hd: Task$AgdaModeVscode.viewEvent(/* PromptInterrupt */0),
                                      tl: /* [] */0
                                    });
                        }
                      }),
                    [Symbol.for("name")]: "WithStateP"
                  },
                  tl: /* [] */0
                };
      
    }
  } else {
    switch (command.TAG | 0) {
      case /* SolveConstraints */0 :
          var normalization = command._0;
          return Handle__Goal$AgdaModeVscode.caseSimple((function (goal) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 0,
                                    _0: normalization,
                                    _1: goal,
                                    [Symbol.for("name")]: "SolveConstraints"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), {
                      hd: {
                        TAG: 1,
                        _0: {
                          TAG: 1,
                          _0: normalization,
                          [Symbol.for("name")]: "SolveConstraintsGlobal"
                        },
                        [Symbol.for("name")]: "AgdaRequest"
                      },
                      tl: /* [] */0
                    });
      case /* SearchAbout */1 :
          var normalization$1 = command._0;
          return Task$AgdaModeVscode.prompt(header, {
                      body: undefined,
                      placeholder: "name:",
                      value: undefined
                    }, (function (expr) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 2,
                                    _0: normalization$1,
                                    _1: expr,
                                    [Symbol.for("name")]: "SearchAbout"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }));
      case /* ElaborateAndGive */2 :
          var normalization$2 = command._0;
          return Handle__Goal$AgdaModeVscode.$$case((function (goal, expr) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 5,
                                    _0: normalization$2,
                                    _1: expr,
                                    _2: goal,
                                    [Symbol.for("name")]: "ElaborateAndGive"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), (function (goal) {
                        return Task$AgdaModeVscode.prompt(header, {
                                    body: undefined,
                                    placeholder: "expression to elaborate and give:",
                                    value: undefined
                                  }, (function (expr) {
                                      return {
                                              hd: {
                                                TAG: 1,
                                                _0: {
                                                  TAG: 5,
                                                  _0: normalization$2,
                                                  _1: expr,
                                                  _2: goal,
                                                  [Symbol.for("name")]: "ElaborateAndGive"
                                                },
                                                [Symbol.for("name")]: "AgdaRequest"
                                              },
                                              tl: /* [] */0
                                            };
                                    }));
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* HelperFunctionType */3 :
          var normalization$3 = command._0;
          return Handle__Goal$AgdaModeVscode.$$case((function (goal, expr) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 8,
                                    _0: normalization$3,
                                    _1: expr,
                                    _2: goal,
                                    [Symbol.for("name")]: "HelperFunctionType"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), (function (goal) {
                        return Task$AgdaModeVscode.prompt(header, {
                                    body: undefined,
                                    placeholder: "expression:",
                                    value: undefined
                                  }, (function (expr) {
                                      return {
                                              hd: {
                                                TAG: 1,
                                                _0: {
                                                  TAG: 8,
                                                  _0: normalization$3,
                                                  _1: expr,
                                                  _2: goal,
                                                  [Symbol.for("name")]: "HelperFunctionType"
                                                },
                                                [Symbol.for("name")]: "AgdaRequest"
                                              },
                                              tl: /* [] */0
                                            };
                                    }));
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* InferType */4 :
          var normalization$4 = command._0;
          var placeholder$1 = "expression to infer:";
          return Handle__Goal$AgdaModeVscode.$$case((function (goal, expr) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 9,
                                    _0: normalization$4,
                                    _1: expr,
                                    _2: goal,
                                    [Symbol.for("name")]: "InferType"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), (function (goal) {
                        return Task$AgdaModeVscode.prompt(header, {
                                    body: undefined,
                                    placeholder: placeholder$1,
                                    value: undefined
                                  }, (function (expr) {
                                      return {
                                              hd: {
                                                TAG: 1,
                                                _0: {
                                                  TAG: 9,
                                                  _0: normalization$4,
                                                  _1: expr,
                                                  _2: goal,
                                                  [Symbol.for("name")]: "InferType"
                                                },
                                                [Symbol.for("name")]: "AgdaRequest"
                                              },
                                              tl: /* [] */0
                                            };
                                    }));
                      }), Task$AgdaModeVscode.prompt(header, {
                          body: undefined,
                          placeholder: placeholder$1,
                          value: undefined
                        }, (function (expr) {
                            return {
                                    hd: {
                                      TAG: 1,
                                      _0: {
                                        TAG: 10,
                                        _0: normalization$4,
                                        _1: expr,
                                        [Symbol.for("name")]: "InferTypeGlobal"
                                      },
                                      [Symbol.for("name")]: "AgdaRequest"
                                    },
                                    tl: /* [] */0
                                  };
                          })));
      case /* Context */5 :
          var normalization$5 = command._0;
          return Handle__Goal$AgdaModeVscode.caseSimple((function (goal) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 11,
                                    _0: normalization$5,
                                    _1: goal,
                                    [Symbol.for("name")]: "Context"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* GoalType */6 :
          var normalization$6 = command._0;
          return Handle__Goal$AgdaModeVscode.caseSimple((function (goal) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 12,
                                    _0: normalization$6,
                                    _1: goal,
                                    [Symbol.for("name")]: "GoalType"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* GoalTypeAndContext */7 :
          var normalization$7 = command._0;
          return Handle__Goal$AgdaModeVscode.caseSimple((function (goal) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 13,
                                    _0: normalization$7,
                                    _1: goal,
                                    [Symbol.for("name")]: "GoalTypeAndContext"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* EventFromView */8 :
          var $$event = command._0;
          if (typeof $$event === "number") {
            if ($$event === /* Initialized */0) {
              return /* [] */0;
            } else {
              return {
                      hd: /* Destroy */0,
                      tl: /* [] */0
                    };
            }
          }
          switch ($$event.TAG | 0) {
            case /* InputMethod */0 :
                var $$char = $$event._0;
                if (!$$char.TAG) {
                  return {
                          hd: {
                            TAG: 0,
                            _0: {
                              TAG: 13,
                              _0: {
                                _0: $$char._0,
                                [Symbol.for("name")]: "InsertChar"
                              },
                              [Symbol.for("name")]: "InputMethod"
                            },
                            [Symbol.for("name")]: "DispatchCommand"
                          },
                          tl: /* [] */0
                        };
                }
                var symbol = $$char._0;
                return {
                        hd: {
                          TAG: 4,
                          _0: (function (state) {
                              return Handle__InputMethod$AgdaModeVscode.chooseSymbol(state, symbol);
                            }),
                          [Symbol.for("name")]: "WithStateP"
                        },
                        tl: /* [] */0
                      };
            case /* PromptIMUpdate */1 :
                var interval = $$event._0;
                if (typeof interval === "number") {
                  switch (interval) {
                    case /* BrowseUp */0 :
                        return {
                                hd: {
                                  TAG: 0,
                                  _0: {
                                    TAG: 13,
                                    _0: /* BrowseUp */1,
                                    [Symbol.for("name")]: "InputMethod"
                                  },
                                  [Symbol.for("name")]: "DispatchCommand"
                                },
                                tl: /* [] */0
                              };
                    case /* BrowseDown */1 :
                        return {
                                hd: {
                                  TAG: 0,
                                  _0: {
                                    TAG: 13,
                                    _0: /* BrowseDown */3,
                                    [Symbol.for("name")]: "InputMethod"
                                  },
                                  [Symbol.for("name")]: "DispatchCommand"
                                },
                                tl: /* [] */0
                              };
                    case /* BrowseLeft */2 :
                        return {
                                hd: {
                                  TAG: 0,
                                  _0: {
                                    TAG: 13,
                                    _0: /* BrowseLeft */4,
                                    [Symbol.for("name")]: "InputMethod"
                                  },
                                  [Symbol.for("name")]: "DispatchCommand"
                                },
                                tl: /* [] */0
                              };
                    case /* BrowseRight */3 :
                        return {
                                hd: {
                                  TAG: 0,
                                  _0: {
                                    TAG: 13,
                                    _0: /* BrowseRight */2,
                                    [Symbol.for("name")]: "InputMethod"
                                  },
                                  [Symbol.for("name")]: "DispatchCommand"
                                },
                                tl: /* [] */0
                              };
                    case /* Escape */4 :
                        return {
                                hd: {
                                  TAG: 4,
                                  _0: (function (state) {
                                      if (IM$AgdaModeVscode.isActivated(state.editorIM) || IM$AgdaModeVscode.isActivated(state.promptIM)) {
                                        return Handle__InputMethod$AgdaModeVscode.deactivate(state);
                                      } else {
                                        return $$Promise.resolved({
                                                    hd: Task$AgdaModeVscode.viewEvent(/* PromptInterrupt */0),
                                                    tl: /* [] */0
                                                  });
                                      }
                                    }),
                                  [Symbol.for("name")]: "WithStateP"
                                },
                                tl: /* [] */0
                              };
                    
                  }
                } else {
                  if (interval.TAG) {
                    var input = interval._0;
                    return {
                            hd: {
                              TAG: 4,
                              _0: (function (state) {
                                  return Handle__InputMethod$AgdaModeVscode.keyUpdatePromptIM(state, input);
                                }),
                              [Symbol.for("name")]: "WithStateP"
                            },
                            tl: /* [] */0
                          };
                  }
                  var interval$1 = interval._0;
                  return {
                          hd: {
                            TAG: 4,
                            _0: (function (state) {
                                return Handle__InputMethod$AgdaModeVscode.select(state, [interval$1]);
                              }),
                            [Symbol.for("name")]: "WithStateP"
                          },
                          tl: /* [] */0
                        };
                }
            case /* JumpToTarget */2 :
                var link = $$event._0;
                return {
                        hd: {
                          TAG: 3,
                          _0: (function (state) {
                              var $$document = state.editor.document;
                              Editor$AgdaModeVscode.focus($$document);
                              var path = Parser$AgdaModeVscode.filepath($$document.fileName);
                              if (link.TAG) {
                                var index = link._0;
                                var goal = state.goals.find(function (goal) {
                                      return goal.index === index;
                                    });
                                if (goal !== undefined) {
                                  return Goal$AgdaModeVscode.setCursor(goal, state.editor);
                                } else {
                                  return ;
                                }
                              }
                              var match = link._0;
                              if (!match) {
                                return ;
                              }
                              var filePath = match._0;
                              if (filePath === undefined) {
                                return ;
                              }
                              if (path !== filePath) {
                                return ;
                              }
                              var ranges = Belt_Array.map(match._1, View__Controller$AgdaModeVscode.fromInterval);
                              return Editor$AgdaModeVscode.$$Selection.setMany(state.editor, ranges);
                            }),
                          [Symbol.for("name")]: "WithState"
                        },
                        tl: /* [] */0
                      };
            case /* MouseOver */3 :
                return {
                        hd: {
                          TAG: 5,
                          _0: "MouseOver",
                          [Symbol.for("name")]: "Debug"
                        },
                        tl: /* [] */0
                      };
            case /* MouseOut */4 :
                return {
                        hd: {
                          TAG: 5,
                          _0: "MouseOut",
                          [Symbol.for("name")]: "Debug"
                        },
                        tl: /* [] */0
                      };
            
          }
      case /* GoalTypeContextAndInferredType */9 :
          var normalization$8 = command._0;
          return Handle__Goal$AgdaModeVscode.$$case((function (goal, expr) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 14,
                                    _0: normalization$8,
                                    _1: expr,
                                    _2: goal,
                                    [Symbol.for("name")]: "GoalTypeContextAndInferredType"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), (function (goal) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 13,
                                    _0: normalization$8,
                                    _1: goal,
                                    [Symbol.for("name")]: "GoalTypeAndContext"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* GoalTypeContextAndCheckedType */10 :
          var normalization$9 = command._0;
          return Handle__Goal$AgdaModeVscode.$$case((function (goal, expr) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 15,
                                    _0: normalization$9,
                                    _1: expr,
                                    _2: goal,
                                    [Symbol.for("name")]: "GoalTypeContextAndCheckedType"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), (function (goal) {
                        return Task$AgdaModeVscode.prompt(header, {
                                    body: undefined,
                                    placeholder: "expression to type:",
                                    value: undefined
                                  }, (function (expr) {
                                      return {
                                              hd: {
                                                TAG: 1,
                                                _0: {
                                                  TAG: 15,
                                                  _0: normalization$9,
                                                  _1: expr,
                                                  _2: goal,
                                                  [Symbol.for("name")]: "GoalTypeContextAndCheckedType"
                                                },
                                                [Symbol.for("name")]: "AgdaRequest"
                                              },
                                              tl: /* [] */0
                                            };
                                    }));
                      }), {
                      hd: Task$AgdaModeVscode.displayOutOfGoalError,
                      tl: /* [] */0
                    });
      case /* ModuleContents */11 :
          var normalization$10 = command._0;
          var placeholder$2 = "module name:";
          return Handle__Goal$AgdaModeVscode.$$case((function (goal, expr) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 16,
                                    _0: normalization$10,
                                    _1: expr,
                                    _2: goal,
                                    [Symbol.for("name")]: "ModuleContents"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), (function (goal) {
                        return Task$AgdaModeVscode.prompt(header, {
                                    body: undefined,
                                    placeholder: placeholder$2,
                                    value: undefined
                                  }, (function (expr) {
                                      return {
                                              hd: {
                                                TAG: 1,
                                                _0: {
                                                  TAG: 16,
                                                  _0: normalization$10,
                                                  _1: expr,
                                                  _2: goal,
                                                  [Symbol.for("name")]: "ModuleContents"
                                                },
                                                [Symbol.for("name")]: "AgdaRequest"
                                              },
                                              tl: /* [] */0
                                            };
                                    }));
                      }), Task$AgdaModeVscode.prompt(header, {
                          body: undefined,
                          placeholder: placeholder$2,
                          value: undefined
                        }, (function (expr) {
                            return {
                                    hd: {
                                      TAG: 1,
                                      _0: {
                                        TAG: 17,
                                        _0: normalization$10,
                                        _1: expr,
                                        [Symbol.for("name")]: "ModuleContentsGlobal"
                                      },
                                      [Symbol.for("name")]: "AgdaRequest"
                                    },
                                    tl: /* [] */0
                                  };
                          })));
      case /* ComputeNormalForm */12 :
          var computeMode = command._0;
          var placeholder$3 = "expression to normalize:";
          return Handle__Goal$AgdaModeVscode.$$case((function (goal, expr) {
                        return {
                                hd: {
                                  TAG: 1,
                                  _0: {
                                    TAG: 18,
                                    _0: computeMode,
                                    _1: expr,
                                    _2: goal,
                                    [Symbol.for("name")]: "ComputeNormalForm"
                                  },
                                  [Symbol.for("name")]: "AgdaRequest"
                                },
                                tl: /* [] */0
                              };
                      }), (function (goal) {
                        return Task$AgdaModeVscode.prompt(header, {
                                    body: undefined,
                                    placeholder: placeholder$3,
                                    value: undefined
                                  }, (function (expr) {
                                      return {
                                              hd: {
                                                TAG: 1,
                                                _0: {
                                                  TAG: 18,
                                                  _0: computeMode,
                                                  _1: expr,
                                                  _2: goal,
                                                  [Symbol.for("name")]: "ComputeNormalForm"
                                                },
                                                [Symbol.for("name")]: "AgdaRequest"
                                              },
                                              tl: /* [] */0
                                            };
                                    }));
                      }), Task$AgdaModeVscode.prompt(header, {
                          body: undefined,
                          placeholder: placeholder$3,
                          value: undefined
                        }, (function (expr) {
                            return {
                                    hd: {
                                      TAG: 1,
                                      _0: {
                                        TAG: 19,
                                        _0: computeMode,
                                        _1: expr,
                                        [Symbol.for("name")]: "ComputeNormalFormGlobal"
                                      },
                                      [Symbol.for("name")]: "AgdaRequest"
                                    },
                                    tl: /* [] */0
                                  };
                          })));
      case /* InputMethod */13 :
          var $$char$1 = command._0;
          if (typeof $$char$1 === "number") {
            switch ($$char$1) {
              case /* Activate */0 :
                  return {
                          hd: {
                            TAG: 4,
                            _0: Handle__InputMethod$AgdaModeVscode.activateEditorIM,
                            [Symbol.for("name")]: "WithStateP"
                          },
                          tl: /* [] */0
                        };
              case /* BrowseUp */1 :
                  return {
                          hd: {
                            TAG: 4,
                            _0: Handle__InputMethod$AgdaModeVscode.moveUp,
                            [Symbol.for("name")]: "WithStateP"
                          },
                          tl: /* [] */0
                        };
              case /* BrowseRight */2 :
                  return {
                          hd: {
                            TAG: 4,
                            _0: Handle__InputMethod$AgdaModeVscode.moveRight,
                            [Symbol.for("name")]: "WithStateP"
                          },
                          tl: /* [] */0
                        };
              case /* BrowseDown */3 :
                  return {
                          hd: {
                            TAG: 4,
                            _0: Handle__InputMethod$AgdaModeVscode.moveDown,
                            [Symbol.for("name")]: "WithStateP"
                          },
                          tl: /* [] */0
                        };
              case /* BrowseLeft */4 :
                  return {
                          hd: {
                            TAG: 4,
                            _0: Handle__InputMethod$AgdaModeVscode.moveLeft,
                            [Symbol.for("name")]: "WithStateP"
                          },
                          tl: /* [] */0
                        };
              
            }
          } else {
            var $$char$2 = $$char$1._0;
            return {
                    hd: {
                      TAG: 4,
                      _0: (function (state) {
                          return Handle__InputMethod$AgdaModeVscode.insertChar(state, $$char$2);
                        }),
                      [Symbol.for("name")]: "WithStateP"
                    },
                    tl: /* [] */0
                  };
          }
      
    }
  }
}

exports.handle = handle;
/* VSCode Not a pure module */
