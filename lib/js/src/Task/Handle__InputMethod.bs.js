...
function activate(state) {
  ...
  return IM$AgdaModeVscode.activate(state.editorIM, Caml_option.some(state.editor), intervals);
}
...
function activate$1(state, input) {
  ...
  return IM$AgdaModeVscode.activate(state.promptIM, undefined, [[
                cursorOffset,
                cursorOffset
              ]]);
}
...
function activateEditorIM(state) {
  ...
  switch (match) {
    ...
    case /* Prompt */1 :
        ...
        return $$Promise.map(handle(state, activate(state)), (function (tasks2) {
                      return Belt_List.concat(tasks1, tasks2);
                    }));
    case /* None */2 :
        return handle(state, activate(state));
...
function activatePromptIM(state, input) {
  ...
  switch (match) {
    case /* Editor */0 :
        if (input.endsWith("\\")) {
          return $$Promise.map(handle(state, IM$AgdaModeVscode.deactivate(state.editorIM)), (function (tasks1) {
                        var tasks2 = handle$1(activate$1(state, input));
                        return Belt_List.concat(tasks1, tasks2);
                      }));
        } else {
          return $$Promise.resolved({
    ...
    case /* None */2 :
        if (input.endsWith("\\")) {
          return $$Promise.resolved(handle$1(activate$1(state, input)));
        } else {
          return $$Promise.resolved({
                      hd: {
                        TAG: 2,
