    ...
    switch (request.tag | 0) {
      ...
      case /* Auto */1 :
          ...
          if (Util$AgdaModeVscode.Version.gte(version, "2.6.0.1")) {
            return commonPart(/* NonInteractive */1) + ("( Cmd_autoOne " + (String(index$1) + (" " + (String(range$1) + (" \"" + (String(content$1) + "\" )"))))));
          } else {
            return commonPart(/* NonInteractive */1) + ("( Cmd_auto " + (String(index$1) + (" " + (String(range$1) + (" \"" + (String(content$1) + "\" )"))))));
          }
      case /* InferType */2 :
          ...
          return commonPart(/* NonInteractive */1) + ("( Cmd_infer " + (String(normalization) + (" " + (String(index$2) + (" noRange \"" + (String(content$2) + "\" )"))))));
      case /* InferTypeGlobal */3 :
          ...
          return commonPart(/* None */0) + ("( Cmd_infer_toplevel " + (String(normalization$1) + (" \"" + (String(content$3) + "\" )"))));
      case /* GoalType */4 :
          var index$3 = String(request[1].index);
          var normalization$2 = Command$AgdaModeVscode.Normalization.toString(request[0]);
          return commonPart(/* NonInteractive */1) + ("( Cmd_goal_type " + (String(normalization$2) + (" " + (String(index$3) + " noRange \"\" )"))));
