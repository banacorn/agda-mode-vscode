  ...
  var encode = function (editor, version, filepath, libraryPath, highlightingMethod, request) {
    ...
    switch (request.tag | 0) {
      ...
      case /* InferType */1 :
          ...
          return commonPart(/* NonInteractive */1) + ("( Cmd_infer " + (String(normalization$prime) + (" " + (String(index$1) + (" noRange \"" + (String(content$1) + "\" )"))))));
      case /* GoalType */2 :
          var index$2 = String(request[1].index);
          var normalization$prime$1 = Command$AgdaModeVscode.Normalization.toString(request[0]);
          return commonPart(/* NonInteractive */1) + ("( Cmd_goal_type " + (String(normalization$prime$1) + (" " + (String(index$2) + " noRange \"\" )"))));
