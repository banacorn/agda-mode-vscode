...
function addAgdaPath(path) {
  var path$1 = Parser$AgdaModeVscode.filepath(path);
  ...
  var newPaths = paths.concat([path$1]);
  if (inTestingMode.contents) {
...
function getDownloadPrompt() {
  return Core__Option.mapOr(Vscode.workspace.getConfiguration("agdaMode", undefined).get("connection.downloadPrompt"), "AlwaysAsk", (function (s) {
                switch (s) {
                  case "never" :
                      return "NeverAsk";
                  default:
...
function setDownloadPrompt(prompt) {
  var tmp;
  tmp = prompt === "AlwaysAsk" ? "always" : "never";
  return Vscode.workspace.getConfiguration("agdaMode", undefined).update("connection.downloadPrompt", tmp, 1, undefined);
}

var Download = {
  getDownloadPrompt: getDownloadPrompt,
  setDownloadPrompt: setDownloadPrompt
};
