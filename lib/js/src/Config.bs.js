...
function deduplicatePaths(paths) {
  var uniqueCandidates = Connection__Candidate$AgdaModeVscode.deduplicate(paths.map(Connection__Candidate$AgdaModeVscode.make));
  var remaining = {
    contents: uniqueCandidates
  };
  return paths.filter(function (path) {
              var current = Connection__Candidate$AgdaModeVscode.make(path);
              var next = remaining.contents[0];
              if (next !== undefined && Connection__Candidate$AgdaModeVscode.equal(current, next)) {
                remaining.contents = remaining.contents.slice(1);
                return true;
              } else {
                return false;
              }
...
function parseAgdaPaths(raw) {
  var rawPaths;
  rawPaths = !Array.isArray(raw) && (raw === null || typeof raw !== "object") && typeof raw !== "number" && typeof raw !== "string" && typeof raw !== "boolean" ? [] : (
  ...
  return deduplicatePaths(rawPaths);
}
...
function getAgdaPaths() {
  if (inTestingMode.contents) {
    ...
    return parseAgdaPaths(Core__Option.getOr(Vscode.workspace.getConfiguration("agdaMode", undefined).get("connection.paths"), null));
  }
}

function addAgdaPath(logChannel, path) {
  var paths = getAgdaPaths();
  var candidate = Connection__Candidate$AgdaModeVscode.make(path);
  var alreadyExists = paths.some(function (existing) {
        return Connection__Candidate$AgdaModeVscode.equal(Connection__Candidate$AgdaModeVscode.make(existing), candidate);
      });
  if (alreadyExists) {
    return Promise.resolve();
  }
  ...
  if (inTestingMode.contents) {
    ...
    return Vscode.workspace.getConfiguration("agdaMode", undefined).update("connection.paths", newPaths, 1, undefined);
  }
}

function setAgdaPaths(logChannel, paths) {
  var dedupedPaths = deduplicatePaths(paths);
  if (inTestingMode.contents) {
    Chan$AgdaModeVscode.emit(logChannel, {
          TAG: "Config",
          _0: {
            TAG: "Changed",
            _0: agdaPathsInTestingMode.contents,
            _1: dedupedPaths
          }
        });
    agdaPathsInTestingMode.contents = dedupedPaths;
    return Promise.resolve();
  } else {
    Chan$AgdaModeVscode.emit(logChannel, {
          TAG: "Config",
    ...
    return Vscode.workspace.getConfiguration("agdaMode", undefined).update("connection.paths", dedupedPaths, 1, undefined);
  }
}
...
var Connection = {
  ...
  deduplicatePaths: deduplicatePaths,
...
var backendInTestingMode = {
  contents: "GHCNoMain"
};

function getBackend() {
  if (inTestingMode.contents) {
    return backendInTestingMode.contents;
  } else {
    return Config__Backend$AgdaModeVscode.decode(Vscode.workspace.getConfiguration("agdaMode", undefined).get("backend"));
  }
}
...
exports.backendInTestingMode = backendInTestingMode;
