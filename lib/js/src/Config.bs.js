...
function parseAgdaPaths(raw) {
  ...
  if (exit === 1) {
    rawPaths = !Array.isArray(strings) && (strings === null || typeof strings !== "object") && typeof strings !== "number" && typeof strings !== "string" && typeof strings !== "boolean" ? [] : (
        Array.isArray(strings) ? Core__Array.filterMap(strings, (function (s) {
  ...
  return rawPaths.filter(function (s) {
...
function getAgdaPaths() {
  ...
  var paths = parseAgdaPaths(Core__Option.getOr(Vscode.workspace.getConfiguration("agdaMode", undefined).get("connection.paths"), "[]"));
