...
function setAgdaPaths(paths) {
  ...
  var paths$1 = paths.map(Connection__URI$AgdaModeVscode.getOriginalPath);
...
function getAgdaPaths2() {
  if (inTestingMode.contents) {
    return agdaPathsInTestingMode2.contents;
  } else {
...
function addAgdaPath(path) {
  ...
  var newPathsAsStrings = newPaths.map(Connection__URI$AgdaModeVscode.getOriginalPath);
  return Vscode.workspace.getConfiguration("agdaMode", undefined).update("connection.paths", newPathsAsStrings, 1, undefined);
}

function addAgdaPath2(path) {
  var paths = getAgdaPaths2();
  var alreadyExists = paths.includes(path);
  if (alreadyExists) {
    return Promise.resolve();
  }
  var newPaths = paths.concat([path]);
  if (inTestingMode.contents) {
    agdaPathsInTestingMode2.contents = newPaths;
    return Promise.resolve();
  } else {
...
var Connection = {
  ...
  agdaPathsInTestingMode2: agdaPathsInTestingMode2,
  ...
  getAgdaPaths2: getAgdaPaths2,
  addAgdaPath: addAgdaPath,
  addAgdaPath2: addAgdaPath2,
