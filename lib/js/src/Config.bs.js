...
function setAgdaPaths(paths) {
  if (inTestingMode.contents) {
    ...
    return Vscode.workspace.getConfiguration("agdaMode", undefined).update("connection.agdaPath", paths, 1, undefined);
  }
}
...
function getAgdaPaths() {
  if (inTestingMode.contents) {
    ...
    return Core__Option.mapOr(Vscode.workspace.getConfiguration("agdaMode", undefined).get("connection.agdaPath"), [], (function (s) {
                    return s.trim().split(",");
                  })).filter(function (s) {
                return s.trim() !== "";
              });
