...
async function addAgdaPath(path) {
  var addAgdaPathPrim = function (xs, x) {
    var i = Core__Array.findIndexOpt(xs, (function (y) {
  ...
  if (inTestingMode.contents) {
    agdaPathsInTestingMode.contents = addAgdaPathPrim(agdaPathsInTestingMode.contents, path);
    return ;
  }
  var originalPaths = Core__Option.getOr(Vscode.workspace.getConfiguration("agdaMode", undefined).get("connection.paths"), []);
  return await Vscode.workspace.getConfiguration("agdaMode", undefined).update("connection.paths", addAgdaPathPrim(originalPaths, path), 1, undefined);
}
...
function getAgdaPaths() {
  ...
  return Core__Option.getOr(rawPaths, []).filter(function (s) {
              return s.trim() !== "";
            });
}
