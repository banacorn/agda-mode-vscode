...
function parseDirectHighlightings(tokens) {
  return Belt_Array.keepMap(Belt_Array.map(tokens.slice(2), parse$2), (function (x) {
                return x;
              }));
}

function parseIndirectHighlightings(tokens) {
  return Belt_Array.keepMap(Belt_Array.map(tokens.slice(1), parse$2), (function (x) {
                return x;
              }));
}
...
function parseWithPriority(_priority, tokens) {
  ...
  switch (match[0]) {
    ...
    case "agda2-exit-done" :
        return /* Ok */Caml_chrome_debugger.variant("Ok", 0, [/* DoneExiting */4]);
