...
function initialize(channels, extensionPath, globalStorageUri, memento, editor, fileName, semanticTokensRequest) {
  ...
  subscribe(Vscode.workspace.onDidChangeTextDocument(function ($$event) {
            ...
            if (changes$1.length !== 0) {
              Goals$AgdaModeVscode.scanAllGoals(state.goals2, editor, changes$1);
              return ;
            }
            
          }));
  subscribe(Editor$AgdaModeVscode.Provider.registerDefinitionProvider(Tokens$AgdaModeVscode.goToDefinition(state.tokens, state.document)));
  return state;
}
