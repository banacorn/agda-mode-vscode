...
function shouldDeactivateView(param) {
  var allClosed = Registry$AgdaModeVscode.size(undefined) === 0;
  var viewActivated = ViewController$AgdaModeVscode.isActivated(undefined);
  if (allClosed) {
...
function onOpenEditor(_extensionPath, editor) {
  var filePath = editor.document.fileName;
  if (isAgda(filePath)) {
    return Belt_Option.forEach(Registry$AgdaModeVscode.get(filePath), (function (state) {
                  state.editor = editor;
...
function onCloseEditor(doc) {
  ...
  if (shouldDeactivateView(undefined)) {
...
exports.shouldDeactivateView = shouldDeactivateView;
