...
function initiateConnection(devMode) {
  ...
  if (!Config$AgdaModeVscode.useAgdaLanguageServer(undefined)) {
    return $$Promise.mapOk(Connection__Emacs$AgdaModeVscode.make(undefined), (function (conn) {
                  return {
                          TAG: 0,
                          _0: conn,
                          _1: Connection__Emacs$AgdaModeVscode.getVersion(conn),
                          [Symbol.for("name")]: "Emacs"
                        };
                }));
  }
  if (Registry$AgdaModeVscode.isEmpty(undefined)) {
    return $$Promise.flatMapError($$Promise.mapOk(Connection__LSP$AgdaModeVscode.start(devMode), (function (param) {
                      var method = param[1];
                      var version = param[0];
                  ...
                  console.log("[LSP] Connection failed, switching to the Agda executable instead: " + Connection__LSP$AgdaModeVscode.$$Error.toString(error)[0] + "\n" + Connection__LSP$AgdaModeVscode.$$Error.toString(error)[1]);
                  return $$Promise.mapOk(Connection__Emacs$AgdaModeVscode.make(undefined), (function (conn) {
                                return {
                                        TAG: 0,
                                        _0: conn,
  ...
  var match = Connection__LSP$AgdaModeVscode.getVersion(undefined);
  if (match !== undefined) {
