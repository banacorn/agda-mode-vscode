...
function initialize(channels, extensionPath, globalStorageUri, memento, editor, fileName) {
  ...
  var state = State$AgdaModeVscode.make(channels, globalStorageUri, extensionPath, memento, editor);
...
function activateWithoutContext(subscriptions, extensionPath, globalStorageUri, memento) {
  ...
  subscribe(Vscode.workspace.onDidChangeConfiguration(function ($$event) {
            ...
            var state = Registry$AgdaModeVscode.getAll()[0];
            if (state !== undefined) {
  ...
  subscribeMany(onTriggerCommand(async function (command, editor) {
            ...
            if (exit === 1) {
              ...
              if (match === undefined) {
                var state = initialize(channels, extensionPath, globalStorageUri, memento, editor, fileName);
