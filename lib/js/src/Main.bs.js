...
function initialize(param) {
    ...
    if (Config$AgdaModeVscode.useAgdaLanguageServer(undefined)) {
      ...
      return $$Promise.flatMap(LSP$AgdaModeVscode.find(undefined), (function (result) {
                    if (result.TAG === /* Ok */0) {
                      ...
                      return LSP$AgdaModeVscode.start(false);
                    }
  ...
  } else if (Config$AgdaModeVscode.useAgdaLanguageServer(undefined)) {
    return $$Promise.resolved(LSP$AgdaModeVscode.isConnected(undefined));
  } else {
    return $$Promise.resolved(false);
  }
}

function makeAndAddToRegistry(debugChan, extensionPath, editor, fileName, useAgdaLanguageServer) {
  ...
  var state = State$AgdaModeVscode.make(debugChan, editor, view, useAgdaLanguageServer);
...
function activateWithoutContext(subscriptions, extensionPath) {
  ...
  var x$1 = Vscode.workspace.onDidCloseTextDocument(function ($$document) {
        ...
        if (isAgda(filePath)) {
          Registry$AgdaModeVscode.removeAndDestroy(filePath);
          return ;
        }
  ...
  var xs = onTriggerCommand(function (command, editor) {
        ...
        var fileName = editor.document.fileName;
        return $$Promise.flatMap($$Promise.flatMap(typeof command === "number" && (command === 2 || command === 1) ? Registry$AgdaModeVscode.removeAndDestroy(fileName) : $$Promise.resolved(undefined), (function (param) {
                          ...
                          if (match$1 !== undefined) {
                            ...
                            return $$Promise.map(initialize(undefined), (function (param) {
                                          return makeAndAddToRegistry(debugChan, extensionPath, editor, fileName, param);
                                        }));
                          }
                        })), (function (param) {
  ...
  var x$2 = Registry$AgdaModeVscode.onDeactivate(function (param) {
        Vscode.commands.executeCommand("setContext", "agdaMode", false);
        return ViewController$AgdaModeVscode.Handle.destroy(undefined);
      });
