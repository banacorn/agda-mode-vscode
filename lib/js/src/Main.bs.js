...
function initiateConnection(param) {
  ...
  if (Registry$AgdaModeVscode.isEmpty(undefined)) {
    return $$Promise.flatMapError($$Promise.mapOk($$Promise.flatMapOk(Connection$AgdaModeVscode.LSP.find(undefined), (function (path) {
                          console.log("[LSP] Found server at: " + path);
                          return Connection$AgdaModeVscode.LSP.start(false);
                        })), (function (version) {
                      console.log("[LSP] Initiated! Agda version: " + version);
                      return {
                              TAG: 1,
                              _0: version,
                              [Symbol.for("name")]: "LSP"
                  ...
                  console.log("[LSP] Connection failed, switching to the Agda executable instead: " + Connection$AgdaModeVscode.$$Error.toString(error)[0] + "\n" + Connection$AgdaModeVscode.$$Error.toString(error)[1]);
                  return $$Promise.mapOk(Connection$AgdaModeVscode.Emacs.make(undefined), (function (conn) {
                                return {
                                        TAG: 0,
                                        _0: conn,
  ...
  var version = Connection$AgdaModeVscode.LSP.getVersion(undefined);
  ...
  var xs = onTriggerCommand(function (command, editor) {
        ...
        return $$Promise.flatMap($$Promise.flatMap(typeof command === "number" && (command === 2 || command === 1) ? $$Promise.flatMap(Registry$AgdaModeVscode.removeAndDestroy(fileName), finalize) : $$Promise.resolved(undefined), (function (param) {
                          ...
                          if (match$1 !== undefined) {
                            ...
                            return $$Promise.map($$Promise.map(initiateConnection(undefined), (function (result) {
                                              if (result.TAG === /* Ok */0) {
                                                return result._0;
                                              } else {
                                                return {
