...
function make(extensionPath) {
  ...
  var panel$1 = ViewController$AgdaModeVscode.make(extensionPath);
  handle.contents = Caml_option.some(panel$1);
  $$Promise.get(ViewController$AgdaModeVscode.onceDestroyed(panel$1), (function (param) {
...
function destroy(param) {
  Belt_Option.forEach(handle.contents, ViewController$AgdaModeVscode.destroy);
  handle.contents = undefined;
...
function initialize(debugChan, extensionPath, globalStoragePath, editor, fileName) {
  ...
  var state = State$AgdaModeVscode.make(debugChan, globalStoragePath, editor, view);
...
exports.PanelSingleton = PanelSingleton;
