...
function initialize(debugChan, extensionPath, globalStoragePath, editor, fileName) {
  ...
  subscribe(Editor$AgdaModeVscode.Provider.registerDocumentSemanticTokensProvider((function (fileName, pushToken) {
              ...
              var tokens = Decoration$AgdaModeVscode.convertToSemanticTokens(state.decoration, state.editor);
              Belt_Array.forEach(tokens, (function (param) {
                      return Curry._3(pushToken, param[0], param[1], param[2]);
                    }));
              return Caml_option.some($$Promise.resolved(undefined));
            }), [
            tokenTypes,
            tokenModifiers
          ]));
