...
function initiateConnection(param) {
  if (Registry$AgdaModeVscode.isEmpty(undefined)) {
    ...
    return $$Promise.resolved(LSP$AgdaModeVscode.getVersion(undefined));
  } else {
    return $$Promise.resolved(undefined);
  }
}
...
function initialize2(debugChan, extensionPath, editor, fileName, lspConnectionResult) {
  ...
  var useAgdaLanguageServer = lspConnectionResult !== undefined ? (console.log("[LSP] Server version: " + lspConnectionResult), true) : false;
