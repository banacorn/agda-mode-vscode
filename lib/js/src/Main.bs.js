...
function load(param) {
  return Belt_Option.forEach(Vscode.window.activeTextEditor, (function (editor) {
                ...
                if (match !== undefined) {
                  ...
                  return View$AgdaModeVscode.activate(match);
                } else {
                  var state = State$AgdaModeVscode.make(editor);
                  set(editor.document.fileName, state);
                  return View$AgdaModeVscode.activate(state);
                }
              }));
}
...
function activate(context) {
  ...
  context.subscriptions.push(Vscode.commands.registerCommand("extension.load", load));
  return /* () */0;
}
