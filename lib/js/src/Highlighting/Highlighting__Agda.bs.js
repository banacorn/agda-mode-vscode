...
function parseEmacs(buffer) {
  ...
  var infos = Belt_Array.keepMap(tokens.slice(1), parse$1);
  return {
          _0: removeTokenBasedHighlighting,
          _1: infos,
          [Symbol.for("name")]: "AddNewInfos"
        };
}
...
function make(param) {
  return {
          tempFilePaths: [],
          infos: new BinarySearchTree.BinarySearchTree()
        };
}
...
function insert(self, editor, infos) {
  return Belt_Array.forEach(infos, (function (info) {
                var existing = AVLTree$AgdaModeVscode.find(self.infos, info.start);
                if (existing !== undefined) {
                  var old = existing[0];
                  self.infos.remove(info.start);
                  var new_start = old.start;
                  var new_end_ = old.end_;
                  var new_aspects = Belt_Array.concat(old.aspects, info.aspects);
...
function readTempFiles(self, editor) {
  return $$Promise.map($$Promise.map($$Promise.allArray(Belt_Array.map(self.tempFilePaths, readAndParse)), (function (xs) {
                    return Belt_Array.concatMany(Belt_Array.map(xs, toInfos));
                  })), (function (infos) {
...
function destroy(self) {
  ...
  self.infos = new BinarySearchTree.BinarySearchTree();
