...
function checkCursorPositions(self, $$document, points) {
  ...
  if (self.instances.length === 0) {
    Chan$AgdaModeVscode.emit(self.eventEmitter, /* Deactivate */1);
    return Chan$AgdaModeVscode.emit(self.eventEmitterTest, /* Deactivate */2);
  }
...
function updateView(self) {
  return Belt_Option.forEach(Belt_Array.get(self.instances, 0), (function (instance) {
                return Chan$AgdaModeVscode.emit(self.eventEmitter, {
                            TAG: 1,
...
function applyRewrites(self, editor, rewrites) {
  ...
  return $$Promise.get(Util$AgdaModeVscode.oneByOne(Belt_Array.map(rewrites, (function (param, param$1) {
                ...
                Chan$AgdaModeVscode.emit(self.eventEmitterTest, /* Change */0);
                self.busy = false;
...
function activate(self, editor, ranges) {
  self.activated = true;
  Chan$AgdaModeVscode.emit(self.eventEmitterTest, /* Activate */1);
...
function deactivate(self) {
  ...
  Chan$AgdaModeVscode.emit(self.eventEmitterTest, /* Deactivate */2);
...
function make$1(eventEmitterTest) {
  return {
          ...
          busy: false,
          handles: [],
          eventEmitter: Chan$AgdaModeVscode.make(undefined),
