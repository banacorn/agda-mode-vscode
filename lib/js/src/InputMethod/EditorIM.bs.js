...
function toString(self) {
  return "(" + String(self.range[0]) + ", " + (String(self.range[1]) + ")");
}
...
function applyRewrites(self, editor, rewrites) {
  ...
  return $$Promise.get(Util$AgdaModeVscode.oneByOne(Belt_Array.map(rewrites, (function (param, param$1) {
                ...
                self.busy = false;
                Belt_Array.forEach(self.cursorPositionsToBeValidated, (function (param) {
                        return Curry._1(param[1], validateCursorPositions(self, $$document, param[0]));
                      }));
                self.cursorPositionsToBeValidated = [];
...
function changeSelection(self, editor, points) {
  if (!self.activated) {
    return $$Promise.resolved(false);
  }
  if (!self.busy) {
    return $$Promise.resolved(validateCursorPositions(self, editor.document, points));
  }
  var match = $$Promise.pending(undefined);
  self.cursorPositionsToBeValidated.push([
        points,
        match[1]
      ]);
  return match[0];
...
function onCommand(self, callback) {
  return Chan$AgdaModeVscode.on(self.chan, callback);
}
