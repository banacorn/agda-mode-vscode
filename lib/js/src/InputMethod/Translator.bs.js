...
function containsLineBreak(symbol) {
  if (symbol.includes("\n")) {
    return true;
  } else {
...
function fromObject(obj) {
  var symbols = (obj[">>"] || []);
  var childKeys = Object.keys(obj).filter(function (key) {
        return key !== ">>";
      });
  var symbols$1 = childKeys.length === 0 ? symbols : symbols.filter(function (symbol) {
          return !containsLineBreak(symbol);
        });
  var subTrie = Object.fromEntries(childKeys.map(function (key) {
            return [
                    key,
                    fromObject((obj[key]))
                  ];
...
function lookup(symbol) {
  return Core__Option.flatMap(Core__Option.flatMap(Core__Option.map(symbol.codePointAt(0), (function (prim) {
                        return String(prim);
                      })), (function (extra) {
                    return rawTable[extra];
                  })), (function (sequences) {
                var sequences$1 = sequences.filter(function (sequence) {
                      var trie = isInKeymap(sequence);
                      if (trie !== undefined) {
                        return trie.symbols.includes(symbol);
                      } else {
                        return false;
                      }
                    });
                if (sequences$1.length === 0) {
                  return ;
                } else {
                  return sequences$1;
                }
...
exports.containsLineBreak = containsLineBreak;
