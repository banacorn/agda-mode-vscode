...
function unlock(self, value) {
  var match = self.contents;
  if (match !== undefined) {
    console.log(">>>>> UNLOCK");
    self.contents = undefined;
    return Curry._1(match[1], Caml_option.some(value));
  }
...
function applyRewrites(self, editor, rewrites) {
  ...
  $$Promise.get(match[0], (function (param) {
          ...
          unlock(self.semaphore, replacements);
...
function run(self, editor, input) {
  switch (input.TAG | 0) {
    ...
    case /* Select */1 :
        var offsets = input._0;
        if (self.activated) {
          return $$Promise.map(acquire(self.semaphore), (function (replacements) {
                        ...
                        var deltaTable = Belt_Option.mapWithDefault(replacements, [], (function (xs) {
                                return Belt_Array.map(xs, (function (param) {
                                              var match = param[0];
                                              var start = match[0];
                                              var delta = (param[1].length + start | 0) - match[1] | 0;
                                              accum.contents = accum.contents + delta | 0;
                                              return [
                                                      start,
                                                      accum.contents
                        ...
                        var lookupDelta = function (offset) {
                          return Belt_Array.reduce(deltaTable, [
                                        0,
                                        0
                        ...
                        var offsets$1 = Belt_Array.map(offsets, (function (offset) {
                                return offset + lookupDelta(offset) | 0;
                              }));
                        console.log("after", offsets$1, Belt_Array.map(self.instances, (function (instance) {
                                    return instance.interval;
                                  })));
                        validateCursorPositions(self, offsets$1);
                        if (self.instances.length === 0) {
                          deactivate(self);
                          return [/* Deactivate */1];
                        } else {
