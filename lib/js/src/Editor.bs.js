...
function stringAtOffset(editor, codePointOffset) {
  var point = editor.document.positionAt(codePointOffset + 1 | 0);
  ...
  var textWithoutLookahead = textWithLookahead.substring(0, codePointOffset);
  var charWidthWithLookahead = characterWidth(textWithLookahead);
  var charWidthWithoutLookahead = characterWidth(textWithoutLookahead);
  if (charWidthWithLookahead === charWidthWithoutLookahead) {
    return /* tuple */[
            codePointOffset + 1 | 0,
            charWidthWithoutLookahead,
            textWithLookahead
          ];
  } else {
    return /* tuple */[
            codePointOffset,
            charWidthWithoutLookahead,
            textWithoutLookahead
          ];
  }
}

function pointAtOffset(editor, offset) {
  var match = stringAtOffset(editor, offset);
  console.log("target offset " + String(offset));
  console.log("offset " + String(offset));
  console.log("current offset " + (String(match[1]) + (" " + match[2])));
  console.log("=======");
  return editor.document.positionAt(match[0]);
}
...
exports.stringAtOffset = stringAtOffset;
