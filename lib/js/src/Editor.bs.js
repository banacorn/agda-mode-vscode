...
function pointAtOffset(editor, offset) {
  ...
  while(true) {
    ...
    if (offset === current || count.contents > 40) {
      return editor.document.positionAt(offset$2);
    }
    var match$1 = codeUnitEndingOffset(editor, (offset$2 + offset | 0) - current | 0);
    var nextOffset = match$1[0];
    if (current === match$1[1]) {
      return editor.document.positionAt(nextOffset);
    }
    _offset = nextOffset;
    continue ;
  };
...
function offsetAtPoint(editor, point) {
  var range = new Vscode.Range(new Vscode.Position(0, 0), point);
