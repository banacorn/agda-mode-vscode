...
function get$1(memento, uri) {
  var cache = getWithDefault(memento, key, {});
  return cache[Connection__URI$AgdaModeVscode.toString(uri)];
}

async function setVersion(memento, uri, version) {
  var cache = getWithDefault(memento, key, {});
  ...
  cache[Connection__URI$AgdaModeVscode.toString(uri)] = entry;
  return await set(memento, key, cache);
}

async function setError(memento, uri, error) {
  var cache = getWithDefault(memento, key, {});
  var entry_timestamp = new Date();
