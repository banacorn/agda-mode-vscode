...
var legacyKey = "resolvedMetadata";
...
async function writeEntries(memento, cache) {
  await set(memento, key, cache);
  return await set(memento, legacyKey, {});
}

function entries(memento) {
  var current = readEntries(memento, key);
  if (Object.entries(current).length > 0) {
    return current;
  } else {
    return readEntries(memento, legacyKey);
  }
}

function get$1(memento, resolved) {
  var cache = entries(memento);
