...
async function $$fetch(repo) {
  ...
  var result = await fetchFromGitHub(repo);
  if (result.TAG === "Ok") {
    await writeToCache(repo.memento, result._0);
  }
  return [
          result,
          false
...
async function downloadLanguageServer(repo, onDownload, target) {
  ...
  var e = await Connection__Resolver__GitHub__Download$AgdaModeVscode.asFile(httpOptions, inFlightDownloadPath, onDownload);
  var result;
  if (e.TAG === "Ok") {
...
async function download(target, memento, globalStoragePath, onDownload) {
  var repo = {
    username: "agda",
  ...
  var error = await downloadLanguageServer(repo, onDownload, target);
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: error._0,
