...
function readTimestamp(memento) {
  return Core__Option.map(Memento$AgdaModeVscode.get(memento, "alsReleaseCacheTimestamp"), (function (prim) {
                return new Date(prim);
              }));
}

function writeTimestamp(memento, timestamp) {
  return Memento$AgdaModeVscode.set(memento, "alsReleaseCacheTimestamp", timestamp.toString());
}

function writeReleaseCache(memento, releases) {
  return Memento$AgdaModeVscode.set(memento, "alsReleaseCache", releases);
}
...
async function fetchFromCache(memento) {
  var string = Memento$AgdaModeVscode.get(memento, "alsReleaseCache");
  if (string === undefined) {
    return {
            TAG: "Ok",
            _0: []
