...
async function downloadFromURL(globalStorageUri, url, saveAsFileName, displayName) {
  ...
  var destDirUri = Vscode.Uri.file(destDir);
  ...
  var execPathUri = Vscode.Uri.file(execPath);
  ...
  try {
    ...
    var tempFileUri = Vscode.Uri.file(tempFilePath);
    var httpOptions = {
      host: host,
      path: path,
      headers: {
    ...
    if (error.TAG === "Ok") {
      ...
      if (isPKHeader) {
        ...
        var zipFileUri = Vscode.Uri.file(zipFilePath);
        var destDirUri$1 = Vscode.Uri.file(destDir);
        await Connection__Download__Unzip$AgdaModeVscode.run(zipFileUri, destDirUri$1);
