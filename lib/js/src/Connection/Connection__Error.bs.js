  ...
  return (
          ...
          attempts.commands.length === 0 ? "" : attempts.commands.map(function (attempt) {
                    ...
                    return "Tried to run the `" + attempt.command + "` command but failed:\n" + (
                            e !== undefined ? "Cannot find `" + attempt.command + "` because: " + Connection__Process__Exec$AgdaModeVscode.$$Error.toString(e) + "." : "Cannot find `" + attempt.command + "` in PATH"
                          );
                  }).join("\n")
        );
}
