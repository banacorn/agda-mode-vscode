...
var Connection__Method__GitHub__Unzip$AgdaModeVscode = require("./Connection__Method__GitHub__Unzip.bs.js");
var Connection__Method__GitHub__Download$AgdaModeVscode = require("./Connection__Method__GitHub__Download.bs.js");
...
function toString(x) {
  ...
  switch (x.TAG) {
    ...
    case "JsonParseError" :
        return "Cannot parse string as JSON:\n" + x._0;
    case "CannotDownload" :
        return "Cannot downlaod file from GitHub:\n" + Connection__Method__GitHub__Download$AgdaModeVscode.$$Error.toString(x._0);
...
async function downloadLanguageServer(repo, callbacks, target) {
  ...
  var e = await Connection__Method__GitHub__Download$AgdaModeVscode.asFile(httpOptions, inFlightDownloadPath, callbacks.onDownload);
  var result;
  if (e.TAG === "Ok") {
    var e$1 = await rename(inFlightDownloadPath, inFlightDownloadPath + ".zip");
    if (e$1.TAG === "Ok") {
      await Connection__Method__GitHub__Unzip$AgdaModeVscode.run(inFlightDownloadPath + ".zip", destPath);
...
async function statModifyTime(path) {
  var stat;
  try {
    stat = await Nodefs.promises.lstat(path);
  }
  catch (raw_exn){
    var exn = Caml_js_exceptions.internalToOCamlException(raw_exn);
    if (exn.RE_EXN_ID === Js_exn.$$Error) {
      return {
              ...
              _0: {
                TAG: "CannotStatFile",
                _0: path,
                [Symbol.for("name")]: "CannotStatFile"
              },
              [Symbol.for("name")]: "Error"
            };
    }
...
async function isValid(globalStoragePath, cacheInvalidateExpirationSecs) {
  var path = Nodepath.join(globalStoragePath, "releases-cache.json");
  if (!Nodefs.existsSync(path)) {
    return false;
  }
  var lastModifiedTime = await statModifyTime(path);
  if (lastModifiedTime.TAG !== "Ok") {
    return false;
  }
  var currentTime = Date.now();
  var diff = (currentTime - lastModifiedTime._0) / 1000.0 | 0;
  return diff < cacheInvalidateExpirationSecs;
}

async function persist(self, releases) {
  var json = JSON.stringify(encodeReleases(releases));
  var path = Nodepath.join(self, "releases-cache.json");
  return await writeFile(path, json);
}

async function get(globalStoragePath) {
  var path = Nodepath.join(globalStoragePath, "releases-cache.json");
  var string = await readFile(path);
  var json;
  try {
    ...
    return {
            ...
            _0: {
              TAG: "JsonParseError",
              _0: string,
              [Symbol.for("name")]: "JsonParseError"
            },
            [Symbol.for("name")]: "Error"
          };
  }
...
async function getReleaseManifestFromGitHubRepo(repo) {
  ...
  var e = await Connection__Method__GitHub__Download$AgdaModeVscode.timeoutAfter(Connection__Method__GitHub__Download$AgdaModeVscode.asJson(httpOptions), 10000);
  if (e.TAG === "Ok") {
...
async function getReleaseManifest(repo) {
  var isValid$1 = await isValid(repo.globalStoragePath, repo.cacheInvalidateExpirationSecs);
  if (isValid$1) {
    var result = await get(repo.globalStoragePath);
    return [
            result,
            true
          ];
  ...
  if (result$1.TAG === "Ok") {
    await persist(repo.globalStoragePath, result$1._0);
  }
  return [
          result$1,
          false
