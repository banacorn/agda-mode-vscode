...
var Connection__Download__Assets$AgdaModeVscode = require("./Download/Connection__Download__Assets.bs.js");
...
function toString(connection) {
  switch (connection.TAG) {
    ...
    case "ALS" :
        ...
        var agdaVersion = match.agdaVersion;
        if (v !== undefined) {
          return "Agda v" + agdaVersion + " Language Server v" + v;
        } else {
          return "Agda v" + agdaVersion + " Language Server (version unknown)";
        }
    case "ALSWASM" :
        var match$1 = connection._3;
        var v$1 = match$1.alsVersion;
        var agdaVersion$1 = match$1.agdaVersion;
        if (v$1 !== undefined) {
          return "Agda v" + agdaVersion$1 + " Language Server v" + v$1 + " WASM";
        } else {
          return "Agda v" + agdaVersion$1 + " Language Server (version unknown) WASM";
        }
...
function resolvedPathForErrors(resolved) {
  if (resolved.resource.scheme === "file") {
    return resolved.resource.fsPath;
  } else {
    return resolved.resource.toString();
  }
}
...
async function probeCandidate(platformDeps, candidate, onProbeFlowOpt) {
  ...
  if (resolved.TAG === "Ok") {
    ...
    if (error.TAG === "Ok") {
      ...
      return {
              TAG: "Error",
              _0: Connection__Error$AgdaModeVscode.Establish.fromProbeError(resolvedPathForErrors(resolved$1), error._0, source)
            };
    }
  }
        ...
        if (error$1.TAG === "Ok") {
          return {
                  ...
                  _0: {
                    ...
                    _1: path,
                    _2: {
                      agdaVersion: agdaVersion._1,
                      alsVersion: agdaVersion._0,
        ...
        try {
          ...
          return {
                  ...
                  _0: {
                    ...
                    _2: path,
                    _3: {
                      agdaVersion: conn.agdaVersion,
                      alsVersion: conn.alsVersion,
                      lspOptions: undefined
                    }
                  }
                };
...
async function fromDownloads(platformDeps, memento, globalStorageUri, onEstablishFlowOpt) {
  ...
  switch (policy) {
    case "Yes" :
        ...
        var rememberWasmFallbackFromSource = function (source) {
          if (source.TAG !== "FromGitHub") {
            return ;
          }
          var descriptor = source._1;
          var sourceChannel = source._0;
          var nativeAgdaVersion = Core__Option.map(Connection__Download__DownloadArtifact$AgdaModeVscode.parseName(descriptor.asset.name), (function (a) {
                  return a.agdaVersion;
                }));
          wasmFallbackSource.contents = Core__Option.map(Connection__Download__Assets$AgdaModeVscode.wasm(descriptor.release).find(function (asset) {
                    var match = Connection__Download__DownloadArtifact$AgdaModeVscode.parseName(asset.name);
                    if (nativeAgdaVersion !== undefined && match !== undefined) {
                      return nativeAgdaVersion === match.agdaVersion;
                    } else {
                      return false;
                    }
                  }), (function (asset) {
        ...
        var tryWasmFallback = async function (path, source) {
          if (typeof source !== "object" && source === "FromManagedDownload") {
            var wasmPath = await Connection__Download__ManagedStorage$AgdaModeVscode.findWasmForManagedPath(globalStorageUri, path);
          ...
          var wasmSource = wasmFallbackSource.contents;
          ...
          var path$1 = await platformDeps.download(globalStorageUri, wasmSource, undefined);
          if (path$1.TAG === "Ok") {
            return {
                    TAG: "Ok",
                    _0: path$1._0
        ...
        var tryDownloadSource = async function (downloadSource) {
          var path = await platformDeps.download(globalStorageUri, downloadSource, undefined);
          if (path.TAG === "Ok") {
            return {
                    TAG: "Ok",
                    _0: path._0
          ...
          var error = path._0;
          if (downloadSource.TAG === "FromGitHub") {
            if (downloadSource._1.asset.name.includes("wasm")) {
              return {
                      TAG: "Error",
                      _0: error
                    };
          ...
          } else if (downloadSource._1.endsWith(".wasm")) {
            return {
                    TAG: "Error",
                    _0: error
                  };
        ...
        if (path !== undefined) {
          ...
          if (resolveError.TAG === "Ok") {
            var source = resolveError._0;
            if (source.TAG === "FromGitHub") {
              rememberWasmFallbackFromSource(source);
              var error = await tryDownloadSource(source);
              ...
              var error$1 = await tryDownloadSource(source);
        ...
        var path$1 = match[0];
        var connection = await make(path$1, source$1);
        if (connection.TAG === "Ok") {
          return {
                  TAG: "Ok",
                  _0: connection._0
...
async function makeWithFallback(platformDeps, memento, globalStorageUri, paths, logChannel) {
  ...
  var remainingPaths = preferredCandidate !== undefined ? paths.filter(function (path) {
          return path !== preferredCandidate;
        }) : paths;
