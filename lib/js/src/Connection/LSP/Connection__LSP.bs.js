...
function probe(tryTCP, name) {
  var probeStdIO = function (name) {
    return $$Promise.mapError($$Promise.mapOk(Process$AgdaModeVscode.PathSearch.run(name, "Please make sure that the language server is installed on the path"), (function (path) {
                      return {
                              TAG: 0,
                              _0: name,
                              _1: path.trim(),
