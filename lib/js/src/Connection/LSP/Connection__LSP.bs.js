...
async function sendRequestPrim(client, request) {
  var json;
  try {
    json = await Client__LSP$LanguageServerMule.sendRequest(client, encode(request));
  }
  catch (raw_exn){
...
async function make(client) {
  var error = await sendRequestPrim(client, "SYN");
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: error._0,
...
async function destroy(self) {
  var result;
  try {
    result = await Client__LSP$LanguageServerMule.destroy(self.client);
  }
  catch (raw_exn){
    var exn = Caml_js_exceptions.internalToOCamlException(raw_exn);
    if (exn.RE_EXN_ID === Js_exn.$$Error) {
