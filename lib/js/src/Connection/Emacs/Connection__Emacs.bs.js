...
async function make(path, args) {
  ...
  if (match === "win32") {
    var regex = /\\([a-zA-Z])\\/;
    var hasWeirdPath = regex.exec(path$2);
    if (hasWeirdPath == null) {
      path$3 = path$2;
    } else {
      var match$1 = Belt_Array.get(hasWeirdPath, 1);
      if (match$1 !== undefined) {
        var drive = Caml_option.valFromOption(match$1);
        if (drive !== undefined) {
          var drive$1 = drive.toUpperCase();
          path$3 = path$2.replace(regex, drive$1 + ":\\");
        } else {
          path$3 = path$2;
        }
      } else {
  ...
  var $$process = Client__Process$LanguageServerMule.make(path$3, ["-V"]);
  var match$2 = Util$AgdaModeVscode.Promise_.pending();
  var resolve = match$2[1];
  var destructor = Client__Process$LanguageServerMule.onOutput($$process, (function (output) {
          switch (output.TAG) {
            case "Stdout" :
                var match_ = Js_string.match_(/Agda version (.*)/, output._0);
                if (match_ === undefined) {
                  return resolve({
                              TAG: "Error",
                ...
                var match = Belt_Array.get(match_, 1);
                if (match === undefined) {
                  return resolve({
                              TAG: "Error",
                              _0: "Cannot read Agda version",
...
function wire(self) {
  ...
  listenerHandle = Client__Process$LanguageServerMule.onOutput(self.process, (function (x) {
          switch (x.TAG) {
            case "Stdout" :
                var rawText = x._0;
                if (Js_string.startsWith("Error:", rawText)) {
                  return Chan$AgdaModeVscode.emit(self.chan, {
                              TAG: "Error",
                              _0: {
                                TAG: "AgdaError",
                  ...
                  return Belt_Array.forEach(Parser$AgdaModeVscode.splitToLines(rawText), (function (extra) {
...
async function make$1(method) {
  ...
  var args = Js_array.concat(Config$AgdaModeVscode.Connection.getCommandLineOptions(), ["--interaction"]);
...
async function onResponse(conn, callback) {
  ...
  Belt_Option.forEach(listenerHandle, (function (destroyListener) {
          destroyListener();
        }));
  return result;
}
