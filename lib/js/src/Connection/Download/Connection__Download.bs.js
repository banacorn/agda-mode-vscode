...
function toString(order) {
  switch (order) {
    ...
    case "DevWASMALS" :
        return "Development Agda Language Server (WASM)";
...
function toVersionString(order) {
  ...
  switch (abstractOrder) {
    ...
    case "DevWASMALS" :
        return "WASM Language Server (dev build)";
...
async function getReleaseManifestFromGitHub(memento, repo, useCacheOpt) {
  ...
  var error = match[0];
  if (error.TAG === "Ok") {
    var manifest = error._0;
    Util$AgdaModeVscode.log("Fetched release manifest with ", manifest);
    return {
            TAG: "Ok",
            _0: manifest
          };
  ...
  Util$AgdaModeVscode.log("Cannot fetch release manifest: ", error$1);
  return {
          TAG: "Error",
          _0: {
