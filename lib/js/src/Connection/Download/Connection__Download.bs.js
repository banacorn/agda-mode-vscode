...
function toVersionString(order) {
  if (order.TAG === "FromGitHub") {
    ...
    var abstractOrder$1 = order._0;
    switch (abstractOrder$1) {
      case "LatestALS" :
      case "DevALS" :
          return toString(abstractOrder$1);
      case "Hardcoded" :
          if (order._1.endsWith(".wasm")) {
...
async function download(globalStorageUri, order) {
  if (order.TAG !== "FromGitHub") {
    return await downloadFromURL(globalStorageUri, order._1, order._2, "Agda Language Server");
  }
