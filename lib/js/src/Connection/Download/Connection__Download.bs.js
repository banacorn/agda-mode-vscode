...
async function download(logChannel, memento, globalStorageUri, fetchSpec) {
  ...
  await Config$AgdaModeVscode.Connection.addAgdaPath(logChannel, destPath);
  return {
          TAG: "Ok",
          _0: destPath
        };
...
async function downloadFromURL(logChannel, globalStorageUri, url, saveAsFileName, displayName) {
  ...
  if (match$1.TAG === "Ok") {
    ...
    await Config$AgdaModeVscode.Connection.addAgdaPath(logChannel, path);
    return {
            TAG: "Ok",
            _0: path
          };
