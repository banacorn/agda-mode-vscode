...
async function downloadFromURL(globalStorageUri, url, saveAsFileName, displayName) {
  ...
  try {
    ...
    var error = await Connection__Download__Util$AgdaModeVscode.asFile(httpOptions, tempFileUri, reportProgress, undefined, undefined, undefined, undefined);
    if (error.TAG === "Ok") {
      if (isWasm) {
...
async function download(globalStorageUri, channel, traceOpt) {
  ...
  var error = await Connection__Download__GitHub$AgdaModeVscode.download(downloadDescriptor, globalStorageUri, reportProgress, trace, undefined, undefined, undefined, undefined, undefined);
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: {
