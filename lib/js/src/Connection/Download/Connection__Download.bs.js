...
async function getReleaseManifest(memento, globalStorageUri) {
  var match = await Connection__Download__GitHub$AgdaModeVscode.ReleaseManifest.$$fetch(memento, makeRepo(globalStorageUri));
  var error = match[0];
  if (error.TAG === "Ok") {
    return {
            TAG: "Ok",
...
async function download(globalStorageUri, fetchSpec) {
  var reportProgress = await Connection__Download__Util$AgdaModeVscode.Progress.report("Agda Language Server");
  var error = await Connection__Download__GitHub$AgdaModeVscode.download(fetchSpec, globalStorageUri, reportProgress);
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: {
  ...
  var destUri = Vscode.Uri.joinPath(globalStorageUri, fetchSpec.saveAsFileName, "als");
