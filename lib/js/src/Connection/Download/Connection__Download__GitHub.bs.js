...
async function fetchFromCache(memento) {
  var string = Memento$AgdaModeVscode.Module.ALSReleaseCache.getReleases(memento);
  if (string === undefined) {
    return {
            TAG: "Ok",
            _0: []
...
async function writeToCache(memento, result) {
  if (result.TAG !== "Ok") {
    return ;
  }
  var json = JSON.stringify(encodeReleases(result._0));
  await Memento$AgdaModeVscode.Module.ALSReleaseCache.setTimestamp(memento, new Date());
  return await Memento$AgdaModeVscode.Module.ALSReleaseCache.setReleases(memento, json);
}

async function $$fetch(memento, repo) {
  var cacheAge = Memento$AgdaModeVscode.Module.ALSReleaseCache.getCacheAgeInSecs(memento);
  var cacheInvalidated = cacheAge !== undefined ? cacheAge > repo.cacheInvalidateExpirationSecs : true;
  if (!cacheInvalidated) {
    return [
            await fetchFromCache(memento),
            true
          ];
  }
...
async function download(fetchSpec, globalStorageUri, reportProgress) {
  var repo = {
    username: "agda",
