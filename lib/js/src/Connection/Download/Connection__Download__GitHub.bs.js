...
function toString(x) {
  ...
  switch (x.TAG) {
    ...
    case "CannotWriteInFlightFile" :
        return "Cannot write in-flight sentinel file:\n" + x._0;
    ...
    case "BinaryMissingAfterExtraction" :
        return "Binary missing after extraction: expected \"" + x._0 + "\"";
...
function toString$1(asset) {
  return "{" + Js_dict.entries(Js_dict.fromArray([
                      ...
                      [
                        "label",
                        Core__Option.getOr(Core__Option.map(asset.label, (function (s) {
                                    return "\"" + s + "\"";
                                  })), "null")
                      ],
                      [
...
var decode = Json_Decode$JsonCombinators.object(function (field) {
      return {
              ...
              label: field.required("label", Json_Decode$JsonCombinators.option(Json_Decode$JsonCombinators.string)),
...
function encode(asset) {
  return {
          ...
          name: asset.name,
          label: Json_Encode$JsonCombinators.option(function (prim) {
                  return prim;
                })(asset.label),
...
async function chmodExecutable(path) {
  Util$AgdaModeVscode.log("[ debug ] chmodExecutable: chmod 744 on", path);
  var val;
  try {
    val = await Nodefs.promises.chmod(path, 484);
  }
  catch (raw_e){
    var e = Caml_js_exceptions.internalToOCamlException(raw_e);
    if (e.RE_EXN_ID === Js_exn.$$Error) {
      Util$AgdaModeVscode.log("[ debug ] chmodExecutable: chmod failed", e._1);
      return {
              TAG: "Error",
              _0: {
                TAG: "CannotChmodFile",
  ...
  Util$AgdaModeVscode.log("[ debug ] chmodExecutable: chmod succeeded", path);
  return {
          TAG: "Ok",
          _0: undefined
        };
...
function assetStem(asset) {
  return asset.name.replace(/\\.(zip|wasm)$/, "");
}

function legacyDownloadDirectoryParts(downloadDescriptor) {
  if (downloadDescriptor.saveAsFileName === "dev-als") {
    return [
            downloadDescriptor.saveAsFileName,
            assetStem(downloadDescriptor.asset)
          ];
  } else {
    return [downloadDescriptor.saveAsFileName];
  }
}

function downloadDestination(downloadDescriptor) {
  var isWasm = downloadDescriptor.asset.name.includes("wasm");
  return {
...
async function downloadLanguageServer(repo, onDownload, downloadDescriptor, traceOpt, fetchFileOpt, unzipOpt, deleteZipOpt, downloadDestinationForDescriptorOpt) {
  ...
  var unzip = unzipOpt !== undefined ? Caml_option.valFromOption(unzipOpt) : undefined;
  var deleteZip = deleteZipOpt !== undefined ? Caml_option.valFromOption(deleteZipOpt) : undefined;
  var downloadDestinationForDescriptor = downloadDestinationForDescriptorOpt !== undefined ? downloadDestinationForDescriptorOpt : downloadDestination;
  var url = new URL(downloadDescriptor.asset.url);
  var httpOptions = {
    host: url.host,
    path: url.pathname,
    headers: {
  ...
  var destination = downloadDestinationForDescriptor(downloadDescriptor);
  var destPath = Caml_splice_call.spliceApply(Vscode.Uri.joinPath, [
        repo.globalStorageUri,
        destination.directoryParts
      ]);
  var doFetch = fetchFile !== undefined ? fetchFile : (function (uri, traceOpt) {
        var trace = traceOpt !== undefined ? traceOpt : Connection__Download__Trace$AgdaModeVscode.noop;
        return Connection__Download__Util$AgdaModeVscode.timeoutAfter(Connection__Download__Util$AgdaModeVscode.asFile(httpOptions, uri, onDownload, trace, undefined, undefined, undefined), Connection__Download__Util$AgdaModeVscode.artifactDownloadTimeoutMs);
      });
  var doUnzip = Core__Option.getOr(unzip, (function (a, b) {
          return Connection__Download__Unzip$AgdaModeVscode.run(a, b, undefined, undefined);
        }));
  var doDeleteZip = Core__Option.getOr(deleteZip, FS$AgdaModeVscode.$$delete);
  var e = await doFetch(inFlightDownloadUri, trace);
  var result;
  if (e.TAG === "Ok") {
    if (destination.isWasm) {
      await FS$AgdaModeVscode.createDirectory(destPath);
      var wasmDestUri = Vscode.Uri.joinPath(destPath, destination.executableName);
      ...
      if (e$2.TAG === "Ok") {
        ...
        var execUri = Vscode.Uri.joinPath(destPath, destination.executableName);
        var execPath = execUri.fsPath;
        var match = await FS$AgdaModeVscode.stat(execUri);
        if (match.TAG === "Ok") {
          var e$3 = await doDeleteZip(inFlightDownloadZipUri);
          result = e$3.TAG === "Ok" ? ({
                TAG: "Ok",
                _0: undefined
              }) : ({
...
async function download(downloadDescriptor, globalStorageUri, reportProgress, traceOpt, writeInFlightSentinelOpt, deleteInFlightSentinelOpt, fetchFileOpt, unzipOpt, deleteZipOpt, downloadDestinationForDescriptorOpt) {
  var trace = traceOpt !== undefined ? traceOpt : Connection__Download__Trace$AgdaModeVscode.noop;
  var writeInFlightSentinel = writeInFlightSentinelOpt !== undefined ? writeInFlightSentinelOpt : (function (uri) {
        return FS$AgdaModeVscode.writeFile(uri, new Uint8Array(0));
      });
  var deleteInFlightSentinel = deleteInFlightSentinelOpt !== undefined ? deleteInFlightSentinelOpt : FS$AgdaModeVscode.$$delete;
  var fetchFile = fetchFileOpt !== undefined ? Caml_option.valFromOption(fetchFileOpt) : undefined;
  var unzip = unzipOpt !== undefined ? Caml_option.valFromOption(unzipOpt) : undefined;
  var deleteZip = deleteZipOpt !== undefined ? Caml_option.valFromOption(deleteZipOpt) : undefined;
  var downloadDestinationForDescriptor = downloadDestinationForDescriptorOpt !== undefined ? downloadDestinationForDescriptorOpt : downloadDestination;
  ...
  var e = await writeInFlightSentinel(inFlightDownloadUri);
  if (e.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: {
  ...
  var destination = downloadDestinationForDescriptor(downloadDescriptor);
  var destUri = Caml_splice_call.spliceApply(Vscode.Uri.joinPath, [
        globalStorageUri,
        destination.directoryParts
      ]);
  var destFileUri = Vscode.Uri.joinPath(destUri, destination.executableName);
  var match = await FS$AgdaModeVscode.stat(destFileUri);
  if (match.TAG === "Ok") {
    var e$1 = await deleteInFlightSentinel(inFlightDownloadUri);
    if (e$1.TAG === "Ok") {
      return {
              TAG: "Ok",
              _0: true
  ...
  var error = await downloadLanguageServer(repo, reportProgress, downloadDescriptor, trace, Caml_option.some(fetchFile), Caml_option.some(unzip), Caml_option.some(deleteZip), downloadDestinationForDescriptor);
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: error._0
  ...
  var execPath = Nodepath.join(destPath, destination.executableName);
...
async function cleanupInFlightFiles(globalStorageUri) {
...
exports.downloadDestination = downloadDestination;
exports.download = download;
exports.isDownloading = isDownloading;
exports.cleanupInFlightFiles = cleanupInFlightFiles;
