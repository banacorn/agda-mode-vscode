...
function downloadDirectoryParts(downloadDescriptor) {
...
async function downloadLanguageServer(repo, onDownload, downloadDescriptor, traceOpt, fetchFileOpt, unzipOpt, deleteZipOpt) {
  ...
  var result;
  if (e.TAG === "Ok") {
    if (downloadDescriptor.asset.name.includes("wasm")) {
      ...
      if (e$2.TAG === "Ok") {
        ...
        var execUri = Vscode.Uri.joinPath(destPath, "als");
...
async function download(downloadDescriptor, globalStorageUri, reportProgress, traceOpt, writeInFlightSentinelOpt, deleteInFlightSentinelOpt, fetchFileOpt, unzipOpt, deleteZipOpt) {
  var trace = traceOpt !== undefined ? traceOpt : Connection__Download__Trace$AgdaModeVscode.noop;
  ...
  var destFileUri = Vscode.Uri.joinPath(destUri, fileName);
  ...
  var error = await downloadLanguageServer(repo, reportProgress, downloadDescriptor, trace, Caml_option.some(fetchFile), Caml_option.some(unzip), Caml_option.some(deleteZip));
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: error._0
...
var Module = {
  downloadDirectoryParts: downloadDirectoryParts,
  download: download,
...
exports.downloadDirectoryParts = downloadDirectoryParts;
exports.download = download;
