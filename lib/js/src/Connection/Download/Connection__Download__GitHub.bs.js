...
async function downloadLanguageServer(repo, onDownload, downloadDescriptor, fetchFileOpt, unzipOpt, deleteZipOpt) {
  ...
  var doFetch = Core__Option.getOr(fetchFile, (function (uri) {
          return Connection__Download__Util$AgdaModeVscode.asFile(httpOptions, uri, onDownload, undefined, undefined, undefined, undefined);
        }));
  ...
  var e = await doFetch(inFlightDownloadUri);
  var result;
  if (e.TAG === "Ok") {
...
async function download(downloadDescriptor, globalStorageUri, reportProgress, writeInFlightSentinelOpt, deleteInFlightSentinelOpt, fetchFileOpt, unzipOpt, deleteZipOpt) {
  ...
  var error = await downloadLanguageServer(repo, reportProgress, downloadDescriptor, Caml_option.some(fetchFile), Caml_option.some(unzip), Caml_option.some(deleteZip));
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: error._0
