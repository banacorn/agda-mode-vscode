...
async function isDownloading(globalStorageUri) {
  try {
    ...
    if (exists) {
      Vscode.Uri.joinPath(globalStorageUri, inFlightDownloadFileName);
      var readResult = await FS$AgdaModeVscode.readDirectory(globalStorageUri);
      if (readResult.TAG !== "Ok") {
        return false;
      }
      var fileNames = readResult._0.map(function (param) {
            return param[0];
          });
      var matched = fileNames.filter(function (fileName) {
            return fileName === inFlightDownloadFileName;
          });
