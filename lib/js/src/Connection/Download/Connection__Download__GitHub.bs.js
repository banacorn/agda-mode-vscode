...
async function downloadLanguageServer(repo, onDownload, target) {
  ...
  var e = await Connection__Download__Util$AgdaModeVscode.asFile(httpOptions, inFlightDownloadPath, onDownload);
  var result;
  if (e.TAG === "Ok") {
    var e$1 = await rename(inFlightDownloadPath, inFlightDownloadPath + ".zip");
    if (e$1.TAG === "Ok") {
      await Connection__Download__Unzip$AgdaModeVscode.run(inFlightDownloadPath + ".zip", destPath);
...
async function download(target, memento, globalStoragePath, reportProgress) {
  var repo = {
    username: "agda",
  ...
  var ifIsDownloading = await isDownloading(globalStoragePath);
  if (ifIsDownloading) {
    return {
            TAG: "Error",
  ...
  var error = await downloadLanguageServer(repo, reportProgress, target);
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: error._0,
  ...
  var shouldChmod = Nodeos.platform() !== "win32";
  if (shouldChmod) {
