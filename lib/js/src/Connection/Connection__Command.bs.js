...
async function searchWith2(command, name, timeoutOpt) {
  var timeout = timeoutOpt !== undefined ? timeoutOpt : 1000;
  var stdout = await Connection__Process__Exec$AgdaModeVscode.run(command, [name], timeout);
  if (stdout.TAG === "Ok") {
...
async function search(name, timeoutOpt) {
  ...
  if (OS$AgdaModeVscode.onUnix) {
    return await searchWith2("which", name, timeout);
  }
  var stdout = await searchWith2("which", name, timeout);
  if (stdout.TAG === "Ok") {
    return {
            TAG: "Ok",
            _0: stdout._0,
    ...
    return await searchWith2("where.exe", name, timeout);
  }
}

exports.$$Error = $$Error;
exports.searchWith = searchWith;
exports.searchWith2 = searchWith2;
