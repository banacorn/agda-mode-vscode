...
async function getPicked(memento, rawSuppliedPaths) {
  ...
  var rawPathFromMemento = Memento$AgdaModeVscode.get(memento, "pickedConnection");
  if (rawPathFromMemento === undefined) {
    return pickFromSuppliedEndpointsInstead;
  }
  var endpointFromMemento = await fromRawPath(Caml_option.valFromOption(rawPathFromMemento));
  if (endpointFromMemento.TAG === "Ok") {
    ...
    if (existsInSuppliedEndpoints) {
      ...
      await Memento$AgdaModeVscode.set(memento, "pickedConnection", undefined);
      return pickFromSuppliedEndpointsInstead;
    }
  }
  await Memento$AgdaModeVscode.set(memento, "pickedConnection", undefined);
...
function setPicked(memento, endpoint) {
  if (endpoint !== undefined) {
    return Memento$AgdaModeVscode.set(memento, "pickedConnection", Connection__URI$AgdaModeVscode.toString(toURI(endpoint)));
  } else {
    return Memento$AgdaModeVscode.set(memento, "pickedConnection", undefined);
  }
}
