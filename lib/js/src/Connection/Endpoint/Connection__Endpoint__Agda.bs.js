...
function wire(self) {
  ...
  Connection__Transport__Process$AgdaModeVscode.onOutput(self.process, (function (x) {
          switch (x.TAG) {
            case "Stdout" :
                var rawText = x._0;
...
async function make(path, version) {
  ...
  var conn = {
    process: Connection__Transport__Process$AgdaModeVscode.make(undefined, path, args),
    version: version,
    path: path,
...
async function onResponse(conn, callback) {
  ...
  await Connection__Scheduler$AgdaModeVscode.runLast(scheduler, (function (response) {
          return callback(response);
        }));
  return result;
}
