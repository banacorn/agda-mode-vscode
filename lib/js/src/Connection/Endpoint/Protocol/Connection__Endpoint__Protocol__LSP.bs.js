...
var Connection__Endpoint__Protocol__LSP__Binding$AgdaModeVscode = require("./Connection__Endpoint__Protocol__LSP__Binding.bs.js");
...
function onRequest(self, callback) {
  return Connection__Endpoint__Protocol__LSP__Binding$AgdaModeVscode.Disposable.toVSCodeDisposable(self.client.onRequest(self.id, callback));
}
...
async function make(id, name, method, serverInitOptions, clientInitOptions) {
  ...
  if (method.TAG === "ViaPipe") {
    serverOptions = Connection__Endpoint__Protocol__LSP__Binding$AgdaModeVscode.ServerOptions.makeWithCommand(method._0, method._1, serverInitOptions);
  } else {
    var url = method._1;
    serverOptions = Connection__Endpoint__Protocol__LSP__Binding$AgdaModeVscode.ServerOptions.makeWithStreamInfo(url.port, url.hostname);
  }
  ...
  var errorHandler = Connection__Endpoint__Protocol__LSP__Binding$AgdaModeVscode.ErrorHandler.make((function (exn, _msg, _count) {
          Chan$AgdaModeVscode.emit(errorChan, exn);
          return "Shutdown";
        }), (function () {
          return "DoNotRestart";
        }));
  var clientOptions = Connection__Endpoint__Protocol__LSP__Binding$AgdaModeVscode.LanguageClientOptions.make(documentSelector, synchronize, errorHandler, clientInitOptions);
