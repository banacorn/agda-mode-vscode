...
async function make(rawpath) {
  ...
  switch (agdaVersion.TAG) {
    ...
    case "IsALSWASM" :
        ...
        try {
          Util$AgdaModeVscode.log("WASM: Starting extension activation", extension$1);
          if (extension$1.isActive) {
            Util$AgdaModeVscode.log("WASM: Extension already active", undefined);
          } else {
            await extension$1.activate();
            Util$AgdaModeVscode.log("WASM: Extension activated", undefined);
          }
          Util$AgdaModeVscode.log("WASM: Reading file", uri);
          var error$3 = await FS$AgdaModeVscode.readFile(uri);
          if (error$3.TAG === "Ok") {
            var $$exports = extension$1.exports;
            Util$AgdaModeVscode.log("WASM: Extension exports", $$exports);
            var setupWasm = async function ($$exports, _raw) {
              Util$AgdaModeVscode.log("WASM: Starting ReScript setup", undefined);
              Util$AgdaModeVscode.log("WASM: Exports keys", Object.keys($$exports).join(", "));
              ...
              Util$AgdaModeVscode.log("WASM: AgdaLanguageServerFactory", agdaLanguageServerFactory);
              Util$AgdaModeVscode.log("WASM: WasmAPILoader", wasmAPILoader);
              Util$AgdaModeVscode.log("WASM: createUriConverters", createUriConverters);
              Util$AgdaModeVscode.log("WASM: Loading WASM API...", undefined);
              var wasm = wasmAPILoader.load();
              Util$AgdaModeVscode.log("WASM: API loaded", wasm);
              Util$AgdaModeVscode.log("WASM: Compiling WASM module...", undefined);
              var mod = await WebAssembly.compile(_raw);
              Util$AgdaModeVscode.log("WASM: Module compiled", mod);
              Util$AgdaModeVscode.log("WASM: Creating factory...", undefined);
              var factory = WASMLoader$AgdaModeVscode.createFactory(agdaLanguageServerFactory, wasm, mod);
              Util$AgdaModeVscode.log("WASM: Factory created", factory);
              Util$AgdaModeVscode.log("WASM: Creating memory file system...", undefined);
              var memfsAgdaDataDir = await wasm.createMemoryFileSystem();
              Util$AgdaModeVscode.log("WASM: Memory FS created", memfsAgdaDataDir);
              return {
                      factory: factory,
                      wasm: wasm,
            ...
            var result = setupWasm($$exports, error$3._0);
            Util$AgdaModeVscode.log("WASM: Awaiting setup...", undefined);
            var wasmSetup = await result;
            Util$AgdaModeVscode.log("WASM setup complete", wasmSetup);
            return {
                    TAG: "Ok",
                    _0: {
                      TAG: "ALSWASM",
          ...
          Util$AgdaModeVscode.log("WASM read error", error$3._0);
          return {
                  TAG: "Error",
                  _0: Connection__Error$AgdaModeVscode.Establish.fromProbeError(path, "CannotMakeConnectionWithALSWASMYet")
                };
          ...
          if (_error.RE_EXN_ID === Js_exn.$$Error) {
            Util$AgdaModeVscode.log("Error!", _error._1);
            return {
                    TAG: "Error",
                    _0: Connection__Error$AgdaModeVscode.Establish.fromProbeError(path, "CannotMakeConnectionWithALSWASMYet")
                  };
  ...
  switch (connection.TAG) {
    ...
    case "ALSWASM" :
        return {
                ...
                _0: {
                  TAG: "Establish",
                  _0: Connection__Error$AgdaModeVscode.Establish.fromProbeError(connection._0, "CannotMakeConnectionWithALSWASMYet")
                }
              };
