...
async function probeFilepath(path) {
  ...
  if (raw.endsWith(".wasm")) {
    var pathKey = vscodeUri.scheme === "file" ? vscodeUri.fsPath : raw;
    return {
            TAG: "Ok",
            _0: [
              pathKey,
...
async function make(rawpath, source) {
  ...
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: Connection__Error$AgdaModeVscode.Establish.fromProbeError(rawpath, error._0, source)
          };
  }
  var match = error._0;
        ...
        if (extension === undefined) {
          return {
                  ...
                  _0: Connection__Error$AgdaModeVscode.Establish.fromProbeError(path, {
                        ...
                        _0: "Extension 'qbane.als-wasm-loader' not found"
                      }, source)
                };
        }
...
async function fromPathsOrCommands(platformDeps, entries) {
  var fromPath = async function (path, source) {
    return await make(path, source);
  };
  var fromCommand = async function (command) {
  ...
  var tasks = entries.map(function (param) {
        ...
        return function () {
          if (isCommand(raw)) {
            return fromCommand(raw);
          } else {
            return fromPath(raw, source);
          }
        };
      });
...
async function fromCommandsWithWinner(platformDeps, commands) {
  var tasks = commands.map(function (command) {
        return async function () {
          ...
          var connection = await make(rawPath._0, {
                TAG: "FromCommandLookup",
                _0: command
              });
          if (connection.TAG === "Ok") {
  ...
  var success = await tryUntilSuccess(tasks);
  if (success.TAG === "Ok") {
    return {
            TAG: "Ok",
            _0: success._0
    ...
    return {
            TAG: "Error",
            _0: Connection__Error$AgdaModeVscode.Establish.mergeMany(success._0)
          };
  }
}
...
async function fromDownloads(platformDeps, memento, globalStorageUri) {
  ...
  switch (policy) {
    case "Yes" :
        ...
        var tryHardcodedWasmFallback = async function () {
        ...
        var tryDownloadSource = async function (source) {
          ...
          switch (source._0) {
            ...
            case "Hardcoded" :
                if (source._1.endsWith(".wasm")) {
                  return {
                          TAG: "Error",
                          _0: error
                        };
                } else {
                  return await tryHardcodedWasmFallback();
                }
            
          }
        };
        var path = await platformDeps.alreadyDownloaded(globalStorageUri, "Hardcoded");
        var downloadResult;
        if (path !== undefined) {
          ...
          var error = await platformDeps.resolveDownloadChannel("Hardcoded", true)(memento, globalStorageUri, platform$1);
          if (error.TAG === "Ok") {
            var error$1 = await tryDownloadSource(error._0);
            ...
            var path$1 = await tryHardcodedWasmFallback();
            downloadResult = path$1.TAG === "Ok" ? ({
                  TAG: "Ok",
                  _0: path$1._0
                }) : ({
                  TAG: "Error",
                  _0: Connection__Error$AgdaModeVscode.Establish.fromDownloadError(path$1._0)
                });
          }
        }
        ...
        var wasmPath = await tryHardcodedWasmFallback();
        if (wasmPath.TAG !== "Ok") {
          return {
                  TAG: "Error",
                  _0: {
...
async function makeWithFallback(platformDeps, memento, globalStorageUri, paths, commands, logChannel) {
  ...
  var shouldSkipAgdaCommand = true;
  if (!remainingPaths.includes("agda")) {
    var tmp;
    tmp = pickedConnection === "agda" ? true : false;
    shouldSkipAgdaCommand = tmp;
  }
  var shouldSkipAlsCommand = true;
  if (!remainingPaths.includes("als")) {
    var tmp$1;
    tmp$1 = pickedConnection === "als" ? true : false;
    shouldSkipAlsCommand = tmp$1;
  }
  var filteredCommands = commands.filter(function (command) {
        if (command === "agda") {
          return !shouldSkipAgdaCommand;
        } else if (command === "als") {
          return !shouldSkipAlsCommand;
        } else {
          return true;
        }
      });
  ...
  var step2Error = await fromCommandsWithWinner(platformDeps, filteredCommands);
  if (step2Error.TAG === "Ok") {
    var connection$4 = step2Error._0[0];
    logConnection(connection$4);
    return {
            TAG: "Ok",
            _0: connection$4
          };
  ...
  var connection$5 = await fromDownloads(platformDeps, memento, globalStorageUri);
  if (connection$5.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: {
              TAG: "Establish",
  ...
  var connection$6 = connection$5._0;
  logConnection(connection$6);
  var downloadedPath = getPath(connection$6);
