...
async function fromDownloads(platformDeps, memento, globalStorageUri) {
  ...
  switch (policy) {
    case "Yes" :
        ...
        var channel;
        channel = platform$1 === "Web" ? "DevALS" : "Hardcoded";
        ...
        if (path !== undefined) {
          ...
          if (platform$1 === "Web") {
            switch (channel) {
              case "DevALS" :
                  var error = await Connection__Download$AgdaModeVscode.downloadFromURL(globalStorageUri, "https://unpkg.com/agda-wasm@0.0.3-als.2.8.0/als/2.8.0/als.wasm", "dev-als", "Agda Language Server (WASM)");
          ...
          if (exit === 2) {
            var error$1 = await platformDeps.resolveDownloadChannel(channel, true)(memento, globalStorageUri, platform$1);
            if (error$1.TAG === "Ok") {
              var error$2 = await platformDeps.download(globalStorageUri, error$1._0);
              downloadResult = error$2.TAG === "Ok" ? ({
                    TAG: "Ok",
                    _0: error$2._0
                  }) : ({
                    TAG: "Error",
                    _0: Connection__Error$AgdaModeVscode.Establish.fromDownloadError(error$2._0)
                  });
            } else {
              downloadResult = {
                TAG: "Error",
        ...
        var error$3 = connection._0;
        return {
                TAG: "Error",
                _0: {
                  probes: error$3.probes,
                  commands: error$3.commands,
                  download: "Succeeded"
                }
              };
        break;
