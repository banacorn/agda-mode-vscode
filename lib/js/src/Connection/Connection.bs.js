...
function toStatus(conn) {
  ...
  var match$1 = Connection__LSP$AgdaModeVscode.getInfo(conn._0);
  return {
          TAG: 1,
          _0: match$1[0],
          _1: match$1[1],
...
function start(useLSP, viaTCP) {
  ...
  if (conn !== undefined) {
    ...
    return $$Promise.mapError($$Promise.mapOk(Connection__LSP$AgdaModeVscode.make(viaTCP), (function (conn) {
                      var match = Connection__LSP$AgdaModeVscode.getInfo(conn);
                      singleton.contents = {
                        TAG: 1,
                        _0: conn,
                        [Symbol.for("name")]: "LSP"
    ...
    return $$Promise.mapError($$Promise.mapOk(Connection__Emacs$AgdaModeVscode.make(undefined), (function (conn) {
                      singleton.contents = {
                        TAG: 0,
                        _0: conn,
                        [Symbol.for("name")]: "Emacs"
...
function sendRequest(useLSP, viaTCP, $$document, request, handler) {
  ...
  if (match.TAG === /* Emacs */0) {
    ...
    return $$Promise.mapError($$Promise.mapOk(Connection__Emacs$AgdaModeVscode.sendRequest(conn, encodeRequest($$document, match$1[0]), handler$1), (function (param) {
                      return toStatus({
                                  TAG: 0,
                                  _0: conn,
                                  [Symbol.for("name")]: "Emacs"
  ...
  var conn$1 = match._0;
  var match$2 = Connection__LSP$AgdaModeVscode.getInfo(conn$1);
  ...
  return $$Promise.mapError($$Promise.mapOk(Connection__LSP$AgdaModeVscode.sendRequest(conn$1, encodeRequest($$document, match$2[0]), handler$2), (function (param) {
                    return toStatus({
                                TAG: 1,
                                _0: conn$1,
                                [Symbol.for("name")]: "LSP"
