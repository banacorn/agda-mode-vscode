...
async function start_(target) {
  ...
  Connection__Target__ALS$AgdaModeVscode.getIPCMethod(conn$1);
  singleton.contents = {
    TAG: "ALS",
    _0: conn$1,
    _1: {
...
async function findAgda(errorFromFindingALS) {
  var error = await Connection__Resolver__Command$AgdaModeVscode.search("agda", undefined);
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: {
              ...
              [Symbol.for("name")]: "CannotFindAgda"
            },
            [Symbol.for("name")]: "Error"
          };
  }
  var path = error._0;
  var error$1 = await Connection__Target$AgdaModeVscode.probePath(path);
  if (error$1.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: error$1._0,
  ...
  return await start_(error$1._0);
}

async function findALSAndAgda() {
  var error = await Connection__Resolver__Command$AgdaModeVscode.search("als", undefined);
  if (error.TAG !== "Ok") {
    return await findAgda(error._0);
  }
  var path = error._0;
  var error$1 = await Connection__Target$AgdaModeVscode.probePath(path);
  if (error$1.TAG !== "Ok") {
    return await findAgda(error$1._0);
  }
  await Config$AgdaModeVscode.Connection.addAgdaPath(path);
  return await start_(error$1._0);
}
...
async function getALSReleaseManifest(state) {
  var match = await Connection__Resolver__GitHub$AgdaModeVscode.getReleaseManifest(makeAgdaLanguageServerRepo(state.memento, state.globalStorageUri.fsPath));
  var error = match[0];
  if (error.TAG === "Ok") {
    return {
            TAG: "Ok",
