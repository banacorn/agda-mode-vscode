...
async function make(memento, paths, commands, platform, getDownloadPolicy, downloadLatestALS) {
  ...
  var targetErrors$1 = Core__List.toArray(Core__List.zipBy(Core__List.fromArray(paths), Core__List.fromArray(targetErrors._0), (function (uri, error) {
              return {
                      uri: uri,
                      error: error
                    };
  ...
  var attempts = {
    targets: targetErrors$1,
    commands: attempts_commands
  };
  var askForDownloadPolicyAndHandleIt = async function () {
    var policy = await getDownloadPolicy();
    if (platform.TAG !== "Ok") {
      return {
              TAG: "Error",
              _0: {
    ...
    switch (policy) {
      ...
      case "Undecided" :
          return await askForDownloadPolicyAndHandleIt();
      
    }
  };
  var error = await askForDownloadPolicyAndHandleIt();
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: error._0,
  ...
  if (error$1.TAG === "Ok") {
    return await makeWithTarget(error$1._0);
  } else {
    return {
            TAG: "Error",
            _0: {
