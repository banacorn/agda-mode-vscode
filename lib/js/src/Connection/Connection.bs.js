...
async function makeWithFallback(platformDeps, memento, globalStorageUri, paths, commands, logChannel) {
  ...
  var commandsWithSource = filteredCommands.map(function (command) {
        return [
                command,
                {
                  TAG: "FromCommandLookup",
  ...
  var connection$4 = await fromPathsOrCommands(platformDeps, commandsWithSource);
  ...
  var connection$6 = await fromDownloads(platformDeps, memento, globalStorageUri);
  if (connection$6.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: {
              TAG: "Establish",
  ...
  var connection$7 = connection$6._0;
  logConnection(connection$7);
  await Config$AgdaModeVscode.Connection.addAgdaPath(logChannel, getPath(connection$7));
  await Memento$AgdaModeVscode.Module.PickedConnection.set(memento, getPath(connection$7));
  return {
          TAG: "Ok",
          _0: connection$7
        };
