...
function start(useLSP) {
  ...
  if (conn !== undefined) {
    ...
    return $$Promise.mapError($$Promise.mapOk($$Promise.flatMapOk($$Promise.flatMapOk($$Promise.flatMap(Connection__Probe$AgdaModeVscode.probeLSP("als", (function (param) {
...
function sendRequest(useLSP, $$document, request, handler) {
  ...
  var match = singleton.contents;
  if (match === undefined) {
    return $$Promise.flatMapOk(start(useLSP), (function (param) {
                  return sendRequest(useLSP, $$document, request, handler);
                }));
  }
  if (match.TAG === /* Emacs */0) {
    var conn = match._0;
