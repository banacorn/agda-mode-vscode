...
async function make(memento, paths, commands) {
  var errors = await Connection__Target$AgdaModeVscode.getPicked(memento, paths);
  if (errors.TAG === "Ok") {
    return await start_(errors._0);
  }
  ...
  var target = error._0;
  await Config$AgdaModeVscode.Connection.addAgdaPath(Connection__URI$AgdaModeVscode.toString(Connection__Target$AgdaModeVscode.toURI(target)));
