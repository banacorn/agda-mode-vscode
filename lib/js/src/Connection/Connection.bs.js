...
async function start(globalStorageUri, useALS, onDownload) {
  ...
  if (useALS) {
    var match = await Connection__Resolver$AgdaModeVscode.tryALS(globalStorageUri.toString(), onDownload);
    var result = match[0];
    if (result === undefined) {
      return {
              TAG: "Error",
  ...
  var match$1 = await Connection__Resolver$AgdaModeVscode.tryAgda();
