...
async function probeFilepath(path) {
  ...
  var url = match._1;
  var match$3 = await Connection__Transport__TCP$AgdaModeVscode.probe(url, undefined);
  if (match$3.TAG === "Ok") {
    return {
            TAG: "Ok",
            _0: [
        ...
        try {
          ...
          if (error$3.TAG !== "Ok") {
            return {
                    ...
                    _0: Connection__Error$AgdaModeVscode.Establish.fromProbeError(path, {
                          TAG: "CannotMakeConnectionWithALSWASMYet",
                          _0: "Failed to read WASM file: " + error$3._0
                        }, source)
                  };
          }
          ...
          if (error$5.RE_EXN_ID === Js_exn.$$Error) {
            var errorMessage = Core__Option.getOr(error$5._1.message, "Unknown error");
            return {
                    TAG: "Error",
                    _0: Connection__Error$AgdaModeVscode.Establish.fromProbeError(path, {
                          TAG: "CannotMakeConnectionWithALSWASMYet",
...
async function fromDownloads(platformDeps, memento, globalStorageUri) {
  ...
  switch (policy) {
    case "Yes" :
        ...
        var error$2 = connection._0;
        return {
                TAG: "Error",
                _0: {
                  probes: error$2.probes,
                  commands: error$2.commands,
                  download: "Succeeded"
                }
              };
    case "No" :
...
async function makeWithFallback(platformDeps, memento, globalStorageUri, paths, commands, logChannel) {
  ...
  if (step2Error.TAG === "Ok") {
    var match = step2Error._0;
    var connection$4 = match[0];
    var match$1 = Memento$AgdaModeVscode.Module.PickedConnection.get(memento);
    if (match$1 !== undefined) {
      
    } else {
      await Memento$AgdaModeVscode.Module.PickedConnection.set(memento, match[1]);
    }
  ...
  await Config$AgdaModeVscode.Connection.addAgdaPath(logChannel, getPath(connection$6));
  await Memento$AgdaModeVscode.Module.PickedConnection.set(memento, getPath(connection$6));
  return {
          TAG: "Ok",
          _0: connection$6
        };
