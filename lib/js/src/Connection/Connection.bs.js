...
async function findCommand(command) {
  var error = await Connection__Resolver__Command$AgdaModeVscode.search(command, undefined);
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: {
              TAG: "CannotFindCommand",
              _0: command,
              _1: error._0,
              [Symbol.for("name")]: "CannotFindCommand"
            },
            [Symbol.for("name")]: "Error"
          };
  }
...
async function findALSAndAgda() {
  var _error = await findCommand("agda");
  if (_error.TAG === "Ok") {
    return {
            TAG: "Ok",
            _0: undefined,
...
async function getALSReleaseManifest(state) {
  var match = await Connection__Resolver__GitHub$AgdaModeVscode.ReleaseManifest.$$fetch(makeAgdaLanguageServerRepo(state.memento, state.globalStorageUri.fsPath));
  var error = match[0];
  if (error.TAG === "Ok") {
    return {
            TAG: "Ok",
