...
async function findCommands(commands) {
  ...
  var step = async function (acc, commands) {
    ...
    var path = await Connection__Command$AgdaModeVscode.search(command, undefined);
    if (path.TAG === "Ok") {
      return {
              TAG: "Ok",
              _0: path._0,
  ...
  var errorPairs$1 = Core__List.map(errorPairs._0, (function (prim) {
          return prim[1];
        }));
  return {
          TAG: "Error",
...
async function make(memento, paths, commands, platform, getDownloadPolicyFromUser, downloadLatestALS) {
