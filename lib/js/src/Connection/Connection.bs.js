...
async function destroy(connection, logChannel) {
  ...
  switch (connection.TAG) {
    ...
    case "ALSWASM" :
        var error$1 = await Connection__Endpoint__ALS$AgdaModeVscode.destroy(connection._0);
        if (error$1.TAG === "Ok") {
          return {
                  TAG: "Ok",
                  _0: undefined
  ...
  switch (policy) {
    case "Yes" :
        ...
        var tryWasmFallback = async function () {
          ...
          var path = await platformDeps.download(globalStorageUri, wasmSource, undefined);
          if (path.TAG === "Ok") {
            return {
                    TAG: "Ok",
                    _0: path._0
        ...
        var tryDownloadSource = async function (source) {
          var path = await platformDeps.download(globalStorageUri, source, undefined);
          if (path.TAG === "Ok") {
            return {
                    TAG: "Ok",
                    _0: path._0
        ...
        var connection = await make(path$1, {
              TAG: "FromDownload",
              _0: channel
            });
        if (connection.TAG === "Ok") {
