...
async function sendRequest(connection, $$document, request, handler) {
  ...
  switch (connection.TAG) {
    ...
    case "ALS" :
        ...
        var payload = encodeRequest($$document, conn$1.agdaVersion);
        Util$AgdaModeVscode.log("Sending request to ALS: ", payload);
    ...
    case "ALSWASM" :
        ...
        var payload$1 = encodeRequestForWASM(wasmLoader, $$document, conn$2.agdaVersion);
        Util$AgdaModeVscode.log("Sending request to WASM ALS: ", payload$1);
        var error$3 = await Connection__Endpoint__ALS$AgdaModeVscode.sendRequest(conn$2, payload$1, handler);
        if (error$3.TAG === "Ok") {
          return {
                  TAG: "Ok",
                  _0: undefined
