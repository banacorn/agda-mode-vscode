...
async function makeWithEndpoint(endpoint) {
  if (endpoint.TAG === "Agda") {
    ...
    var error = await Connection__Endpoint__Agda$AgdaModeVscode.make(endpoint._1, version);
    if (error.TAG === "Ok") {
      return {
              TAG: "Ok",
              _0: {
  ...
  var error$1 = await Connection__Endpoint__ALS$AgdaModeVscode.make(endpoint._2, endpoint._3, getFromConfig());
  if (error$1.TAG === "Ok") {
    return {
            TAG: "Ok",
            _0: {
...
async function fromPathsAndCommands2(platformDeps, memento, paths, commands) {
  ...
  var pickedPath = await Connection__Endpoint$AgdaModeVscode.getPickedRaw(memento, paths);
...
async function fromDownloads2(platformDeps, memento, globalStorageUri, attempts) {
  ...
  switch (policy$1) {
    case "Yes" :
        await Config$AgdaModeVscode.Connection.DownloadPolicy.set("Yes");
        var rawPath = await platformDeps.alreadyDownloaded2(globalStorageUri)();
        if (rawPath !== undefined) {
          var connection = await makeWithRawPath(rawPath);
          if (connection.TAG !== "Ok") {
            return {
                    TAG: "Error",
                    _0: {
          ...
          await Config$AgdaModeVscode.Connection.addAgdaPath(Connection__URI$AgdaModeVscode.parse(rawPath));
          return {
                  TAG: "Ok",
                  _0: connection._0
                };
        ...
        var rawPath$1 = error._0;
        var connection$1 = await makeWithRawPath(rawPath$1);
        if (connection$1.TAG !== "Ok") {
          return {
                  TAG: "Error",
                  _0: {
        ...
        await Config$AgdaModeVscode.Connection.addAgdaPath(Connection__URI$AgdaModeVscode.parse(rawPath$1));
        return {
                TAG: "Ok",
                _0: connection$1._0
              };
...
async function make2(platformDeps, memento, globalStorageUri, paths, commands) {
  ...
  var error = await fromDownloads2(platformDeps, memento, globalStorageUri, connection._0);
  if (error.TAG === "Ok") {
    return {
            TAG: "Ok",
            _0: error._0
