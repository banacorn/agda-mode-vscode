...
async function fromDownloads(platformDeps, memento, globalStorageUri) {
  ...
  switch (policy) {
    case "Yes" :
        ...
        var downloadOrder;
        downloadOrder = platform$1 === "Web" ? "DevALS" : "LatestALS";
        var path = await platformDeps.alreadyDownloaded(globalStorageUri, downloadOrder);
        var downloadResult;
        if (path !== undefined) {
          downloadResult = {
            TAG: "Ok",
          ...
          var exit = 0;
          if (platform$1 === "Web" && downloadOrder !== "LatestALS") {
          ...
          if (exit === 2) {
            var error$1 = await platformDeps.resolveDownloadOrder(downloadOrder, true)(memento, globalStorageUri, platform$1);
            if (error$1.TAG === "Ok") {
...
async function makeWithFallback(platformDeps, memento, globalStorageUri, paths, commands, logChannel) {
  ...
  var pathsWithSource = pathsWithSelectedConnection.map(function (path) {
        return [
                path,
                "FromConfig"
              ];
      });
  var commandsWithSource = commands.map(function (command) {
        return [
                command,
                {
                  TAG: "FromCommandLookup",
  ...
  var allEntries = pathsWithSource.concat(commandsWithSource);
  var connection = await fromPathsOrCommands(platformDeps, allEntries);
  if (connection.TAG === "Ok") {
  ...
  var connection$2 = await fromDownloads(platformDeps, memento, globalStorageUri);
  ...
  await Config$AgdaModeVscode.Connection.addAgdaPath(logChannel, getPath(connection$3));
