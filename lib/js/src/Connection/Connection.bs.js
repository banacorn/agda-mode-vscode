...
async function fromDownloads(platformDeps, logChannel, memento, globalStorageUri) {
  ...
  switch (policy$1) {
    case "Yes" :
        ...
        await Config$AgdaModeVscode.Connection.addAgdaPath(logChannel, getPath(connection$1));
...
async function make(platformDeps, memento, globalStorageUri, paths, commands, logChannel) {
  ...
  var tasks = [
    ...
    (function () {
        return fromDownloads(platformDeps, logChannel, memento, globalStorageUri);
      })
  ];
  var connection = await tryUntilSuccess(tasks);
  if (connection.TAG !== "Ok") {
