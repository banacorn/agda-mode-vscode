...
async function fromCommands(platformDeps, commands) {
  var tryCommand = async function (command) {
    ...
    if (rawPath.TAG !== "Ok") {
      return {
              TAG: "Error",
              _0: Connection__Error$AgdaModeVscode.Construction.addCommandError(Connection__Error$AgdaModeVscode.Construction.make(), command, rawPath._0)
            };
    }
    var rawPath$1 = rawPath._0;
    var connection = await makeWithRawPath(rawPath$1);
    if (connection.TAG === "Ok") {
      return {
              TAG: "Ok",
              _0: connection._0
      ...
      return {
              TAG: "Error",
              _0: Connection__Error$AgdaModeVscode.Construction.addEndpointError(Connection__Error$AgdaModeVscode.Construction.make(), rawPath$1, connection._0)
            };
    }
  };
  ...
  if (connection.TAG === "Ok") {
    ...
    return {
            TAG: "Error",
            _0: Connection__Error$AgdaModeVscode.Construction.mergeMany(connection._0.map(function (prim) {
                      return prim[1];
                    }))
          };
  }
...
async function make(platformDeps, memento, globalStorageUri, paths, commands, logChannel) {
  ...
  var connection = await fromPathsAndCommands(platformDeps, memento, paths, commands);
  if (connection.TAG === "Ok") {
    var connection$1 = connection._0;
  ...
  var connection$2 = await fromDownloads(platformDeps, memento, globalStorageUri, connection._0);
