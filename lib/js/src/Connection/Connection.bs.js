...
async function getTarget(memento, globalStorageUri) {
  var _error = await getALSReleaseManifest(memento, globalStorageUri);
  if (_error.TAG !== "Ok") {
    return ;
  }
  ...
  return assets.toSorted(function (a, b) {
...
async function downloadLatestALS(memento, globalStorageUri, progressCallback) {
  var target = await getTarget(memento, globalStorageUri);
  if (target !== undefined) {
    await download(memento, globalStorageUri.fsPath, progressCallback, target);
    return target;
  }
