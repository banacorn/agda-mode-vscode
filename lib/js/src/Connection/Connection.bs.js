...
async function probeFilepath(path) {
  ...
  var path$1 = match._1.fsPath;
  var result = await Connection__Process__Exec$AgdaModeVscode.run(path$1, ["--version"], undefined);
  if (result.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: {
  ...
  var assetPath = await checkForPrebuiltDataDirectory(path$1);
  var lspOptions;
...
async function makeWithRawPath(rawpath) {
  ...
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: Connection__Error$AgdaModeVscode.Construction.fromEndpointError(rawpath, error._0)
          };
  }
  var match = error._0;
  ...
  if (agdaVersion.TAG === "Ok") {
    ...
    var connection = await Connection__Endpoint__Agda$AgdaModeVscode.make(rawpath, agdaVersion$1);
    return {
            TAG: "Ok",
            _0: {
              TAG: "Agda",
