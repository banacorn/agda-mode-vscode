...
async function make(platformDeps, memento, globalStorageUri, paths, commands) {
  var constructionError = await fromPathsAndCommands(platformDeps, memento, paths, commands);
  if (constructionError.TAG === "Ok") {
    var endpoint = constructionError._0;
    await Config$AgdaModeVscode.Connection.addAgdaPath(Connection__Endpoint$AgdaModeVscode.toURI(endpoint));
    return await makeWithEndpoint(endpoint);
  }
  var error = await fromDownloads(platformDeps, memento, globalStorageUri, constructionError._0);
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: error._0
...
async function make2(platformDeps, memento, globalStorageUri, paths, commands, logChannel) {
