...
async function findCommand(command) {
  ...
  if (_error.TAG === "Ok") {
    return await Connection__Target$AgdaModeVscode.fromRawPath(_error._0);
  } else {
    return {
            TAG: "Error",
...
function findCommands(commands) {
  ...
  var step = async function (commands) {
    if (!commands) {
      return {
              TAG: "Error",
              _0: "CannotFindALSorAgda",
    ...
    var result = await findCommand(commands.hd);
    if (result.TAG === "Ok") {
      return {
              TAG: "Ok",
              _0: result._0,
      ...
      return await step(commands.tl);
    }
  };
  return step(commands$1);
}

async function make(memento, paths, commands) {
  var target = await Connection__Target$AgdaModeVscode.getPicked(memento, paths);
  if (target !== undefined) {
    return await start_(target);
  }
  var error = await findCommands(commands);
  if (error.TAG !== "Ok") {
    return {
            TAG: "Error",
            _0: error._0,
  ...
  await Config$AgdaModeVscode.Connection.addAgdaPath(Connection__URI$AgdaModeVscode.toString(Connection__Target$AgdaModeVscode.toURI(target$1)));
  return await start_(target$1);
}
...
function chooseAssetFromRelease(release) {
...
async function download(memento, globalStoragePath, onDownload, target) {
...
async function downloadLatestALS(memento, globalStorageUri, progressCallback) {
  ...
  var target = error._0;
  var e = await download(memento, globalStorageUri.fsPath, progressCallback, target);
  if (e.TAG === "Ok") {
    return {
            TAG: "Ok",
            _0: target,
