...
function toString(connection) {
  ...
  if (match !== undefined) {
    return "ALS(" + path + ", " + match[0] + ", " + match[1] + ")";
  } else {
    return "ALS(" + path + ", unknown version)";
  }
}
...
async function fromDownloads(platformDeps, logChannel, memento, globalStorageUri) {
  ...
  switch (policy$1) {
    case "Yes" :
        ...
        if (path !== undefined) {
          ...
          var error = await platformDeps.downloadLatestALS(logChannel, memento, globalStorageUri)(platform._0);
...
async function makeWithFallback(platformDeps, memento, globalStorageUri, paths, commands, logChannel) {
  ...
  var tasks = [
    ...
    (function () {
        return tagFrom(fromDownloads(platformDeps, logChannel, memento, globalStorageUri), "FromDownloads");
      })
  ];
