...
function toString(x) {
  switch (x.TAG) {
    ...
    case "ProcessHanging" :
        return "process hanging for more than " + x._0.toString() + " milliseconds";
    case "NotFound" :
...
async function run(path, args, timeoutOpt, shellOpt) {
  var timeout = timeoutOpt !== undefined ? timeoutOpt : 10000;
  var shell = shellOpt !== undefined ? shellOpt : true;
  var $$process = Connection__Transport__Process$AgdaModeVscode.make(shell, path, args);
  ...
  var hangTimeout = {
    contents: Caml_option.some(setTimeout((function () {
                Connection__Transport__Process$AgdaModeVscode.destroy($$process);
                resolve({
                      TAG: "Error",
                      _0: {
                        TAG: "ProcessHanging",
          ...
          switch (output.TAG) {
            ...
            case "Event" :
                ...
                if (code.TAG === "OnError") {
                  var msg = code._0;
                  if (msg !== "") {
                    if (Connection__Transport__Process$AgdaModeVscode.errorMessageIndicatesNotFound(msg)) {
                      return resolve({
                                  TAG: "Error",
                                  _0: {
                                    TAG: "NotFound",
                    ...
                    if (Connection__Transport__Process$AgdaModeVscode.errorMessageIndicatesNotFound(stderr.contents)) {
                      return resolve({
                                  TAG: "Error",
                                  _0: {
                                    TAG: "NotFound",
                ...
                var code$1 = code._0;
                if (code$1 !== 0) {
                  if (code$1 !== 127 && !Connection__Transport__Process$AgdaModeVscode.errorMessageIndicatesNotFound(stderr.contents)) {
                    return resolve({
                                TAG: "Error",
                                _0: {
                                  TAG: "FromStderr",
