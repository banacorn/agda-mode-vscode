...
function toString(attempts) {
  return (
          attempts.endpoints.length === 0 ? "Tried to connect with the path from the configuration but there are none.\n" : "Tried to connect with the path from the configuration but all failed:\n" + attempts.endpoints.map(Connection__Endpoint$AgdaModeVscode.$$Error.toString).join("\n") + "\n"
        ) + (
