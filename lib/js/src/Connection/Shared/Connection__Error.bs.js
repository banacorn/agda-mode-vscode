...
function toString$4(x) {
  ...
  var downloadStr;
  if (error !== undefined) {
    downloadStr = "Tried to download the Agda Language Server but failed:\n" + Connection__Download$AgdaModeVscode.$$Error.toString(error);
  } else {
    var hasOtherAttempts = Object.entries(x.probes).length > 0 || Object.entries(x.commands).length > 0;
    downloadStr = hasOtherAttempts ? "" : "Opted not to download prebuilt Agda Language Server";
  }
