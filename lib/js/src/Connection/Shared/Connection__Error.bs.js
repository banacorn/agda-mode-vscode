...
function pathSourceToString(source) {
  if (typeof source !== "object") {
    if (source === "FromConfig") {
      ...
      return "from managed download";
    }
  } else if (source.TAG === "FromDownload") {
    if (source._0 === "LatestALS") {
      return "from LatestALS download";
    } else {
      return "from DevALS download";
    }
  } else {
    return "from PATH lookup (" + source._0 + ")";
  }
}
...
function toString$4(x) {
  ...
  downloadStr = typeof error !== "object" ? (
    ...
    ) : "Tried to download the Agda Language Server but failed:\n  " + Connection__Download__Error$AgdaModeVscode.toString(error._0);
