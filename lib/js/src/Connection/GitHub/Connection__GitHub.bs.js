...
var Connection__GitHub__Unzip$AgdaModeVscode = require("./Connection__GitHub__Unzip.bs.js");
var Connection__GitHub__Download$AgdaModeVscode = require("./Connection__GitHub__Download.bs.js");
...
function toString(x) {
  ...
  switch (x.TAG) {
    ...
    case "CannotDownload" :
        return "Cannot downlaod file from GitHub:\n" + Connection__GitHub__Download$AgdaModeVscode.$$Error.toString(x._0);
...
async function downloadLanguageServer(repo, callbacks, target) {
  ...
  var e = await Connection__GitHub__Download$AgdaModeVscode.asFile(httpOptions, inFlightDownloadPath, callbacks.onDownload);
  var result;
  if (e.TAG === "Ok") {
    var e$1 = await rename(inFlightDownloadPath, inFlightDownloadPath + ".zip");
    if (e$1.TAG === "Ok") {
      await Connection__GitHub__Unzip$AgdaModeVscode.run(inFlightDownloadPath + ".zip", destPath);
...
async function getReleaseManifestFromGitHubRepo(repo) {
  ...
  var e = await Connection__GitHub__Download$AgdaModeVscode.timeoutAfter(Connection__GitHub__Download$AgdaModeVscode.asJson(httpOptions), 10000);
  if (e.TAG === "Ok") {
