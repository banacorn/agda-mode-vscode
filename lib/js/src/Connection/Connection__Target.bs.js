...
function tryAgda() {
  ...
  return Connection__Resolver$AgdaModeVscode.searchMany([
              {
                TAG: "FromFile",
                _0: storedPath,
...
async function validateLocalInstallation(path) {
  var pathWithArgs = path + " --version";
  var error = await Connection__Target__Agda__Process$AgdaModeVscode.Validation.run(pathWithArgs, (function (a) {
          console.log(pathWithArgs + " => " + a);
          return {
                  TAG: "Ok",
                  _0: undefined,
                  [Symbol.for("name")]: "Ok"
  ...
  if (error.TAG === "Ok") {
    ...
    return {
            TAG: "Error",
            _0: Connection__Target__Agda__Process$AgdaModeVscode.Validation.$$Error.toString(error._0),
...
function getLocalInstallations() {
  var paths = [Config$AgdaModeVscode.Connection.getAgdaPath()];
  return Promise.all(paths.map(validateLocalInstallation));
}

var Module = {
  tryALS: tryALS,
  tryAgda: tryAgda,
  getLocalInstallations: getLocalInstallations
};
...
exports.getLocalInstallations = getLocalInstallations;
