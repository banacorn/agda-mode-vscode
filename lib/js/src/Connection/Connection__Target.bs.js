...
async function getPicked(memento, rawSuppliedPaths) {
  var suppliedTargets = Core__Array.filterMap(await Promise.all(rawSuppliedPaths.map(fromRawPath)), (function (target) {
          if (target.TAG === "Ok") {
            return target._0;
          }
  ...
  if (targetFromMemento.TAG === "Ok") {
    ...
    var existsInSuppliedTargets = suppliedTargets.includes(targetFromMemento$1);
    if (existsInSuppliedTargets) {
      return targetFromMemento$1;
    } else {
