...
async function downloadAgdaLanguageServer(memento, globalStoragePath, onDownload) {
  var repo = makeAgdaLanguageServerRepo(memento, globalStoragePath);
  console.log(repo);
  return await Connection__Resolver__GitHub$AgdaModeVscode.get(repo, {
...
async function getPicked(state) {
  ...
  var stillExists = Core__Array.reduce(fromConfig, false, (function (acc, target) {
          if (acc) {
            ...
            return Caml_obj.equal(toURI(target._0), fromMemento$1);
          } else {
            return false;
          }
        }));
...
var Module = {
  downloadAgdaLanguageServer: downloadAgdaLanguageServer,
  tryAgda: tryAgda,
...
exports.downloadAgdaLanguageServer = downloadAgdaLanguageServer;
