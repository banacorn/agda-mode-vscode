...
async function tryALS(memento, globalStoragePath, onDownload) {
  ...
  return await Connection__Resolver$AgdaModeVscode.searchMany([
              ...
              {
                TAG: "FromGitHub",
                _0: makeAgdaLanguageServerRepo(memento, globalStoragePath),
                _1: {
...
function getPath(target) {
  ...
  if (match.TAG === "ViaPipe") {
    ...
    return "lsp://" + match._1 + ":" + String(match._0);
  }
}

function getAll() {
  return Promise.all(Config$AgdaModeVscode.Connection.getAgdaPaths().map(probePath));
}

async function getFirstUsable() {
  var targets = await getAll();
...
async function getPicked(state) {
  ...
  if (stillExists) {
    var target = await probePath(fromMemento$1);
    if (target.TAG === "Ok") {
      return target._0;
    } else {
      return ;
  ...
  await State__Type$AgdaModeVscode.Memento.update(state.memento, "pickedConnection", undefined);
}

function setPicked(state, target) {
  return State__Type$AgdaModeVscode.Memento.update(state.memento, "pickedConnection", getPath(target));
}
