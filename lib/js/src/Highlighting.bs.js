...
async function triggerDocumentChangeEvent(editor) {
  ...
  if (succeed) {
    await Editor$AgdaModeVscode.$$Text.replace($$document, deleteRange, "");
  }
  await $$document.save();
}
...
function updateSemanticHighlighting(self, $$event) {
  ...
  self.semanticTokens = Core__Array.reduce(changes, self.semanticTokens, applyChange);
}
...
async function apply(self, tokens, editor) {
  if (Config$AgdaModeVscode.Highlighting.getHighlightWithThemeColors()) {
    ...
    return await triggerDocumentChangeEvent(editor);
  }
