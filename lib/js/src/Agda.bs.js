...
function render(x) {
  switch (x.TAG | 0) {
    ...
    case /* Underscore */2 :
        return RichText$AgdaModeVscode.string("_" + x._0);
...
function parseOthers(raw) {
  return Belt_Option.map(parse(raw), (function (raw$prime) {
                return {
                        TAG: 3,
                        _0: RichText$AgdaModeVscode.concatMany(Belt_Array.map(raw$prime, render)),
...
function render$2(value, $$location) {
  var $$location$1 = Belt_Option.mapWithDefault($$location, RichText$AgdaModeVscode.empty, RichText$AgdaModeVscode.srcLoc);
  switch (value.TAG | 0) {
    case /* OfType' */0 :
