...
function initialize(channels, extensionPath, globalStorageUri, memento, editor, fileName, semanticTokensRequest) {
  ...
  var state = State$AgdaModeVscode.make(channels, globalStorageUri, extensionPath, memento, editor, semanticTokensRequest);
  ...
  subscribe(Vscode.workspace.onDidChangeTextDocument(function ($$event) {
            ...
            var changes$1 = $$event.contentChanges.map(Tokens$AgdaModeVscode.Change.fromTextDocumentContentChangeEvent).toReversed();
...
function activateWithoutContext(subscriptions, extensionPath, globalStorageUri, memento) {
  ...
  subscribe(onOpenEditor(function (editor) {
            var fileName = Parser$AgdaModeVscode.filepath(editor.document.fileName);
            if (isAgda(fileName)) {
              return Core__Option.forEach(Registry$AgdaModeVscode.getState(fileName), (function (state) {
                            state.editor = editor;
  ...
  subscribe(Vscode.workspace.onDidChangeConfiguration(function ($$event) {
            ...
            var state = Registry$AgdaModeVscode.getAllStates()[0];
            if (state !== undefined) {
  ...
  subscribeMany(onTriggerCommand(async function (command, editor) {
            var fileName = Parser$AgdaModeVscode.filepath(editor.document.fileName);
            var showWebLimitationMessage = function (commandName) {
              Vscode.window.showWarningMessage("The \"" + commandName + "\" command requires desktop features (process spawning, file system) that are not available in VS Code for the Web. Please use the desktop version of VS Code for full agda-mode functionality.");
            };
            var isDesktopOnlyCommand;
            if (typeof command !== "object") {
              switch (command) {
                case "Load" :
                case "Compile" :
                    isDesktopOnlyCommand = true;
                    break;
                default:
                  isDesktopOnlyCommand = false;
              }
            } else {
              isDesktopOnlyCommand = false;
            }
            if (isDesktopOnlyCommand) {
              showWebLimitationMessage(Command$AgdaModeVscode.toString(command));
              return ;
            }
            if (typeof command !== "object") {
              switch (command) {
            ...
            if (typeof command === "object" && command.TAG === "InputMethod") {
              var tmp = command._0;
              if (typeof tmp !== "object" && tmp === "Activate") {
                var entry = Registry$AgdaModeVscode.getEntry(fileName);
                if (entry !== undefined) {
                  var match = entry.state;
                  if (match === undefined) {
                    var state = initialize(channels, extensionPath, globalStorageUri, memento, editor, fileName, Caml_option.some(entry.semanticTokens));
                    Registry$AgdaModeVscode.add(fileName, state);
                  }
                  
                } else {
                  var state$1 = initialize(channels, extensionPath, globalStorageUri, memento, editor, fileName, undefined);
                  Registry$AgdaModeVscode.add(fileName, state$1);
                }
              }
              
            }
            var state$2 = Registry$AgdaModeVscode.getState(fileName);
...
function activate(context) {
  ...
  var globalStorageUri = context.globalStorageUri.with({
        scheme: "file"
      });
  console.log("agda-mode: web environment - desktop features disabled");
  Vscode.window.showInformationMessage("agda-mode is running in web mode. Some features (Agda/ALS integration, file operations) require the desktop version of VS Code. Unicode input method and syntax highlighting are available.");
  return activateWithoutContext(subscriptions, extensionPath, globalStorageUri, Caml_option.some(context.workspaceState));
}
