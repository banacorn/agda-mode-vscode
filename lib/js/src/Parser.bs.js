...
function toString(s) {
  if (s.TAG) {
    return "[" + (Belt_Array.map(s._0, toString).join(", ") + "]");
  } else {
    return "\"" + (s._0 + "\"");
  }
}
...
function flatten(s) {
  if (s.TAG) {
    return Belt_Array.concatMany(Belt_Array.map(s._0, flatten));
  } else {
    return [s._0];
  }
}
...
function toString$1(param) {
  if (param.TAG) {
    return "Parse error code: R" + (String(param._0) + (" \"" + (toString(param._1) + "\"")));
  } else {
    return "Parse error code: S" + (String(param._0) + (" \"" + (param._1 + "\"")));
  }
}
