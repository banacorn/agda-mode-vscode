...
function parseError(raw) {
  ...
  if (hasBothErrorsAndWarnings) {
    ...
    dictionary = Emacs__Parser$AgdaModeVscode.Dict.update(Emacs__Parser$AgdaModeVscode.Dict.update(Emacs__Parser$AgdaModeVscode.Dict.partite(lines, predicate), "errors", (function (xs) {
                return [xs.slice(1).join("\n")];
              })), "warnings", (function (xs) {
            return Belt_Array.map(Emacs__Parser$AgdaModeVscode.Array_.mergeWithNext(Emacs__Parser$AgdaModeVscode.Array_.partite(xs.slice(1), markWarningStart), glueBack), (function (param) {
                          return Js_array.joinWith("\n", param);
                        }));
          }));
  } else {
    dictionary = Emacs__Parser$AgdaModeVscode.Dict.update(Emacs__Parser$AgdaModeVscode.Dict.partite(lines, (function (param) {
                if (param[1] === 0) {
                  return "errors";
                }
...
function parseAllGoalsWarnings(title, body) {
  ...
  var x = partiteMetas(partiteAllGoalsWarnings(title, body));
  return partiteWarningsOrErrors(partiteWarningsOrErrors((console.log(body), console.log(x), x), "warnings"), "errors");
}

function renderAllGoalsWarnings(dictionary) {
  return Js_array.concatMany(Belt_Array.map(Js_dict.entries(dictionary), (function (param) {
                    var lines = param[1];
                    switch (param[0]) {
                      case "errors" :
...
exports.renderAllGoalsWarnings = renderAllGoalsWarnings;
