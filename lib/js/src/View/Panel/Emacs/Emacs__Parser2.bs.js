...
function parseOutputs(raw) {
  var lines = Emacs__Parser$AgdaModeVscode.aggregateLines(Js_string.split("\n", raw));
  return Belt_Array.map(Belt_Array.keepMap(Belt_Array.map(lines, Agda$AgdaModeVscode.Output.parse), (function (x) {
                    return x;
                  })), (function (output) {
                return Agda$AgdaModeVscode.Output.renderItem(output);
...
function parseAndRenderSearchAbout(raw) {
  var lines = Js_string.split("\n", raw);
  var outputs = Belt_Array.map(Belt_Array.keepMap(Belt_Array.map(Emacs__Parser$AgdaModeVscode.aggregateLines(Belt_Array.map(Js_array.sliceFrom(1, lines), (function (param) {
