...
function unindent(lines) {
  var newLineIndices = Belt_Array.map(Belt_Array.keep(Belt_Array.mapWithIndex(lines, (function (index, line) {
              ...
              var isDelimeter = delimeter.test(line);
              var isSort = sort.test(line);
              var isCompleteJudgement = completeJudgement.test(line);
              var isReallyLongTermIdentifier = reallyLongTermIdentifier.test(line) && Belt_Option.mapWithDefault(nextLine, false, (function (line) {
                      return restOfTheJudgement.test(line);
                    }));
              if (isDelimeter || isSort || isCompleteJudgement) {
                return true;
              } else {
                return isReallyLongTermIdentifier;
              }
            })), (function (param) {
          return param[2];
        }));
