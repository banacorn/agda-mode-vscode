...
function parse(raw) {
  return concatMany(Belt_Array.mapWithIndex(Belt_Array.keepMap((function (__x) {
                          return Js_string.splitByRe(/([^\(\)\s]+\:(?:\d+\,\d+\-\d+\,\d+|\d+\,\d+\-\d+|\d+\,\d+))/, __x);
                        })(raw), (function (x) {
                        return x;
                      })), (function (i, token) {
                    var match = i % 2;
                    if (match !== 1) {
                      ...
                      return Belt_Option.mapWithDefault(Common$AgdaModeVscode.AgdaRange.parse(token), string(token), (function (loc) {
                                    return srcLoc(loc);
                                  }));
                    }
                  })));
