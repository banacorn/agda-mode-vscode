...
function recv(reqChan, resChan, handler) {
  React.useEffect((function () {
          return Chan$AgdaModeVscode.on(reqChan, (function (req) {
                        handler(req).then(function (res) {
                              Chan$AgdaModeVscode.emit(resChan, res);
                            });
                      }));
        }), []);
}
...
function useFocus() {
  ...
  var setFocus = function (param) {
    Core__Option.forEach(Core__Option.flatMap(Caml_option.nullable_to_opt(htmlElRef.current), Webapi__Dom__Element.asHtmlElement), (function (prim) {
            prim.focus();
          }));
  };
  return [
