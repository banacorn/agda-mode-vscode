...
function codeUnitEndingOffset(editor, utf16offset) {
  var range = new Vscode.Range(new Vscode.Position(0, 0), editor.document.positionAt(utf16offset + 1 | 0));
  var textWithLookahead = editor.document.getText(Caml_option.some(range));
  var textWithoutLookahead = textWithLookahead.substring(0, utf16offset);
  var utf8offsetWithLookahead = Sig$AgdaModeVscode.characterWidth(textWithLookahead);
  var utf8offsetWithoutLookahead = Sig$AgdaModeVscode.characterWidth(textWithoutLookahead);
  if (utf8offsetWithLookahead === utf8offsetWithoutLookahead) {
    return {
            utf8: utf8offsetWithoutLookahead,
...
function fromAgdaOffset(editor, offset) {
  var _utf16offset = offset;
  while(true) {
    var utf16offset = _utf16offset;
    var current = codeUnitEndingOffset(editor, utf16offset);
    if (offset === current.utf8) {
      return current.utf16;
    }
    var next = codeUnitEndingOffset(editor, (current.utf16 + offset | 0) - current.utf8 | 0);
