...
function lineEndingIsCRLF($$document) {
  var match = VSCode$AgdaModeVscode.TextDocument.eol($$document);
  if (match) {
    return false;
  } else {
    return true;
...
function registerProvider(definitionProvider, hoverProvider) {
  ...
  var definitionProvider$1 = {
    provideDefinition: (function (textDocument, point, param) {
        return VSCode$AgdaModeVscode.ProviderResult.map(Curry._2(definitionProvider, textDocument.fileName, point), (function (pairs) {
                      return VSCode$AgdaModeVscode.LocationLinkOrLocation.locationLinks(Belt_Array.map(pairs, (function (param) {
  ...
  var hoverProvider$1 = {
    provideHover: (function (textDocument, point, param) {
        return VSCode$AgdaModeVscode.ProviderResult.map(Curry._2(hoverProvider, textDocument.fileName, point), (function (param) {
...
function registerTestingProvider(prodider, legend) {
  var documentSelector = VSCode$AgdaModeVscode.MaybeArray.singular(VSCode$AgdaModeVscode.DocumentFilterOrString.string("agda"));
  var documentSemanticTokensProvider = {
    provideDocumentSemanticTokens: (function (textDocument, param) {
        return VSCode$AgdaModeVscode.ProviderResult.map(Curry._1(prodider, textDocument.fileName), (function (prim) {
                      return new Vscode.SemanticsTokens(prim);
                    }));
      })
  };
  var semanticTokensLegend = new Vscode.SemanticTokensLegend(legend);
