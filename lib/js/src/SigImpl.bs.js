...
function codeUnitEndingOffset(editor, offset) {
  var range = new Vscode.Range(new Vscode.Position(0, 0), editor.document.positionAt(offset + 1 | 0));
  var textWithLookahead = editor.document.getText(Caml_option.some(range));
  var textWithoutLookahead = textWithLookahead.substring(0, offset);
  var charOffsetWithLookahead = Sig$AgdaModeVscode.characterWidth(textWithLookahead);
  var charOffsetWithoutLookahead = Sig$AgdaModeVscode.characterWidth(textWithoutLookahead);
  if (charOffsetWithLookahead === charOffsetWithoutLookahead) {
    return /* tuple */[
            offset + 1 | 0,
            charOffsetWithoutLookahead
          ];
  } else {
    return /* tuple */[
            offset,
