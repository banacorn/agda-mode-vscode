...
function highlightBackground(editor, style, ranges) {
  return highlightBackgroundPrim(editor, VSCode$AgdaModeVscode.ThemeColor.themeColor(new Vscode.ThemeColor(style)), ranges);
}

function highlightBackgroundWithColor(editor, color, ranges) {
  return highlightBackgroundPrim(editor, VSCode$AgdaModeVscode.ThemeColor.string(color), ranges);
}
...
function decorateText(editor, style, ranges) {
  return decorateTextPrim(editor, VSCode$AgdaModeVscode.ThemeColor.themeColor(new Vscode.ThemeColor(style)), ranges);
}

function decorateTextWithColor(editor, color, ranges) {
  return decorateTextPrim(editor, VSCode$AgdaModeVscode.ThemeColor.string(color), ranges);
}
...
function registerProvider(defnProvider) {
  var documentSelector = VSCode$AgdaModeVscode.MaybeArray.singular(VSCode$AgdaModeVscode.DocumentFilterOrString.string("agda"));
  var definitionProvider = VSCode$AgdaModeVscode.DefinitionProvider.makeWithLocations((function (textDocument, point, param) {
          return Belt_Option.map(Curry._2(defnProvider, textDocument.fileName, point), (function (param) {
                        return $$Promise.resolved([new Vscode.Location(Vscode.Uri.file(param[0]), param[1])]);
                      }));
        }));
  return Vscode.languages.registerDefinitionProvider(documentSelector, definitionProvider);
}
