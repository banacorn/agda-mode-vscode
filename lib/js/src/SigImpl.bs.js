...
function codeUnitEndingOffset(editor, init, utf16offset) {
  var range = new Vscode.Range(editor.document.positionAt(init.utf16), editor.document.positionAt(utf16offset + 1 | 0));
  var textWithLookahead = editor.document.getText(Caml_option.some(range));
  var textWithoutLookahead = textWithLookahead.substring(init.utf16, utf16offset);
  ...
  if (utf8widthWithLookahead === utf8widthWithoutLookahead) {
    return {
            utf8: init.utf8 + utf8widthWithoutLookahead | 0,
...
function fromAgdaOffset(editor, initial, targetUTF8) {
  var initial$1 = initial !== undefined ? initial : ({
        utf8: 0,
        utf16: 0
      });
  ...
  while(true) {
    ...
    var current = codeUnitEndingOffset(editor, initial$1, utf16offset);
    if (targetUTF8 === current.utf8) {
      return current.utf16;
    }
    var next = codeUnitEndingOffset(editor, initial$1, (current.utf16 + targetUTF8 | 0) - current.utf8 | 0);
...
function toAgdaOffset(editor, offset) {
...
exports.codeUnitEndingOffset = codeUnitEndingOffset;
exports.fromAgdaOffset = fromAgdaOffset;
exports.toAgdaOffset = toAgdaOffset;
