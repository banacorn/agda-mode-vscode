...
function compileUTF8OffsetIntervals(text) {
  ...
  if (match !== undefined) {
    return Belt_Array.concat(intervals, [/* tuple */[
                  match[1] + 1 | 0,
                  lastEndpoint
                ]]);
  } else {
...
function normalizeUTF16Offset(editor, cachedStart, utf16offset) {
  if (utf16offset < cachedStart.utf16) {
    cachedStart.utf8 = 0;
    cachedStart.utf16 = 0;
  }
  var start = editor.document.positionAt(cachedStart.utf16);
  var end_ = editor.document.positionAt(utf16offset + 1 | 0);
  var range = new Vscode.Range(start, end_);
  var textWithLookahead = editor.document.getText(Caml_option.some(range));
  var textWithoutLookahead = textWithLookahead.substring(0, utf16offset - cachedStart.utf16 | 0);
  var utf8widthWithLookahead = Sig$AgdaModeVscode.characterWidth(textWithLookahead);
  var utf8widthWithoutLookahead = Sig$AgdaModeVscode.characterWidth(textWithoutLookahead);
  var result = utf8widthWithLookahead !== utf8widthWithoutLookahead ? ({
        utf8: cachedStart.utf8 + utf8widthWithoutLookahead | 0,
        utf16: utf16offset
      }) : ({
        utf8: cachedStart.utf8 + utf8widthWithoutLookahead | 0,
        utf16: utf16offset + 1 | 0
      });
...
function fromUTF8Offset(editor, cachedStart, targetUTF8) {
  var cachedStart$1 = cachedStart !== undefined ? cachedStart : ({
        utf8: 0,
        utf16: 0
      });
  var _utf16offset = targetUTF8;
  while(true) {
    var utf16offset = _utf16offset;
    var current = normalizeUTF16Offset(editor, cachedStart$1, utf16offset);
    if (targetUTF8 === current.utf8) {
      return current.utf16;
    }
    var next = normalizeUTF16Offset(editor, cachedStart$1, (current.utf16 + targetUTF8 | 0) - current.utf8 | 0);
    if (current.utf8 === next.utf8) {
      return next.utf16;
    }
    _utf16offset = next.utf16;
    continue ;
  };
}
...
function replaceText(editor, range, text) {
  ...
  workspaceEdit.replace(editor.document.uri, range, text, undefined);
...
exports.compileUTF8OffsetIntervals = compileUTF8OffsetIntervals;
exports.normalizeUTF16Offset = normalizeUTF16Offset;
