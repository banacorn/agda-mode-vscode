...
function getSemanticHighlighting(param) {
  var raw = Vscode.workspace.getConfiguration("agdaMode", undefined).get("semanticHighlighting");
  if (raw === "true") {
    return true;
  } else {
    return false;
...
function registerTestingProvider(prodider, param) {
  ...
  var documentSemanticTokensProvider = {
    provideDocumentSemanticTokens: (function (textDocument, param) {
        ...
        return VSCode$AgdaModeVscode.ProviderResult.map(Curry._2(prodider, textDocument.fileName, pushLegend), (function (param) {
